...
### 11 AM Session Note
- No trades. No "### Approved Trades (verified)" section existed in today's RESEARCH-LOG (market-open's APA buy was verified/logged directly, not staged as a list), so per STEP 3 the buy side stayed HOLD; this week (Aug 24 start) holds at core 2/6 (XYZ 8/26, APA 8/28), satellite 0/4 new trades — valid per the Patience Rule.
- Risk sweep: no position at/beyond its sleeve hard-cut — APA -0.59%, BAX -4.79% (closest, vs. -7% core cut), XYZ +1.54%, MNKD -0.75%, RIGL +10.73% (vs. -15% satellite cut). All 8 open positions confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial dust flagged every prior sweep). No stop tightening — XYZ/RIGL both below their +15%/+25% thresholds; no stop moved down.
- No thesis break: nothing moved sharply enough to warrant the optional Gemini check. BAX remains off-watchlist a 2nd consecutive week — its thesis-broken decision point is today's separate weekly-review screen-refresh (not yet run as of this session, confirmed via WATCHLIST.md still showing "Week of 2026-08-21" as current), not this 11am window. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2. Cash 20.17%, at/above the 20% floor, no sweep needed.
- Lesson: none new this window.

### 3 PM Session Note
- No trades. No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (market-open's APA buy was verified/logged directly, not staged as a list) — per STEP 3 the buy side stayed HOLD. This week (Aug 24 start) closes at core 2/6 (XYZ 8/26, APA 8/28), satellite 0/4 new trades — valid per the Patience Rule.
- Risk sweep: no position at/beyond its sleeve hard-cut — APA -0.31%, BAX -4.13% (closest, vs. -7% core cut, improved from -4.79% at 11am), XYZ +0.37%, MNKD -1.38%, RIGL +10.76% (vs. -15% satellite cut). All 8 open positions confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74.5 dust flagged every prior sweep, not a new gap). No stop moved down.
...
### Account
- Equity: $95,743.94 | Cash: $19,309.35 (20.17%, at/above the 20% floor) | Buying power: $287,366.26 (margin-inflated 4x multiplier, no margin used or planned) | Daytrade count: not returned by `alpaca.sh account` this session (field absent from the response) — not fabricated, flagged as a data gap
- Core exposure: $24,099.10 (25.17%) — APA $5,259.87 (+2.02% unrealized) / BAX $9,302.28 (-4.18% unrealized) / XYZ $9,536.95 (-0.46% unrealized) | Satellite exposure: $10,158.72 (10.61%) — MNKD $4,860.00 (0.00% unrealized) / RIGL $5,298.72 (+11.09% unrealized) | Income exposure: $42,176.77 (44.05%) — EDGX $20,117.83 / SGOV $2,088.46 (20.740468683 sh, 20 sh stop-covered) / SPHY $19,970.48
- All 8 open positions confirmed via `alpaca.sh positions`/`orders`, no auth errors: all carry live GTC trailing stops (APA 10% $38.655 hwm $42.95; BAX 10% $24.705 hwm $27.45; XYZ 10% $78.228 hwm $86.92; MNKD 15% $3.74 hwm $4.40; RIGL 15% $40.341 hwm $47.46; EDGX 5% $25.9825 hwm $27.35; SPHY 5% $22.23 hwm $23.40; SGOV 5% $95.665 hwm $100.70 covering 20 of 20.740468683 sh — same immaterial ~$74.5 fractional dust flagged every prior sweep, not a new gap). None missing, none moved down.
- No position at/beyond its sleeve hard-cut: APA +2.02%, BAX -4.18% (closest to core -7% cut), XYZ -0.46%; MNKD 0.00%, RIGL +11.09% (both vs. -15% satellite cut).
- This week (Aug 31 start, new weekly-trade-count window): core 0/6, satellite 0/4 new trades so far.
- Watchlist: current "Week of 2026-08-28" (sixth weekly refresh, not empty/PENDING). **BAX drops off the core list for a 2nd consecutive week — this triggers TRADING-STRATEGY.md's mandatory thesis-broken exit, due at today's market-open regardless of P&L** (BAX -4.18% unrealized, well clear of its -7% cut — the exit is thesis-driven, not P&L-driven). **APA is week 1 off the core list** (bought 8/28, watchlist qualified it that day but the same 8/28 refresh's fresh fundamentals check found analyst-rating deterioration) — no forced exit yet; if APA is still off the list at the 9/4 refresh, exit regardless of P&L.

### Data-quality check — consolidated Gemini call opened mid-document; resubmitted once for the missing pieces
The first consolidated Gemini Deep Research call (submitted per STEP 3) completed (exit 0) after ~11 minutes, but the captured output **starts mid-document** on a "Cybersecurity: The Necessity of Platformization" sub-section — section 1 (S&P 500 futures direction / VIX level) and the top-catalysts/economic-releases discussion (section 2) never appear before it, and only 2 of the 8 held tickers (APA, XYZ) are addressed incidentally within sector commentary; BAX, MNKD, RIGL, EDGX, SGOV, SPHY get no explicit overnight-news check in this pass. This matches the exact truncation pattern logged repeatedly in prior refreshes (see TRADING-STRATEGY.md/WATCHLIST.md history). Per the data-quality guard, none of the missing figures were fabricated. What this first pass **did** deliver cleanly: full satellite-watchlist catalyst-proximity confirmation (see Satellite Trade Ideas below) and core-watchlist sector commentary for CRWD/PANW/PYPL/STT/HOOD/ADP/MPC/VLO/TGT/EXPE/CRL/NTAP/VEEV/ZBRA/IQV/CNC/RVTY/INCY.

A second, narrower Gemini call was submitted (submit+poll, ~12 more minutes) asking specifically for the missing pieces: (1) S&P futures/VIX, (2) top catalysts, (3) a one-paragraph thesis-break check per held ticker. This completed successfully and delivered (2) and (3) in full — see Market Context and Held-Position Thesis Check below — but again did **not** return an explicit S&P 500 futures point/percent figure or a VIX level/reading; it substituted 2-year Treasury yield and oil-price moves instead. **No VIX level or S&P futures figure is used anywhere in this entry** — that specific data point remains an open gap after two consolidated attempts; not resubmitted a third time per the "don't loop many small queries" guidance in scripts/gemini_research.sh. Flagging the VIX/futures gap specifically for the next session to try a native WebSearch fallback if it recurs.

### Market Context
- **S&P 500 futures / VIX:** not available this session — see data-quality note above. No figure fabricated.
- **Geopolitical / macro (from the second Gemini pass, cited):** U.S. military action targeted Iranian rocket launchers near the Strait of Hormuz over the weekend, prompting retaliatory Iranian missile strikes — a real, significant escalation. Brent crude surged >5% to breach $90/barrel. Fed Chair Kevin Warsh's Jackson Hole remarks read mildly hawkish, moving the market-implied September rate-hike probability from ~35% to ~60% and pushing the 2-year Treasury yield up 12bp to 4.36%. Econ calendar today is thin — Dallas Fed Texas Manufacturing Outlook (10:30am ET), short-term T-bill auctions (11:30am ET) — with markets positioning cautiously ahead of Friday's Non-Farm Payrolls print. China NBS Manufacturing PMI and Germany's preliminary August CPI were also cited as overnight inputs into the European/Asian session tone.
- **Sector momentum (from the first Gemini pass):** Cybersecurity (CRWD +13.78% last week post-Q2 beat; PANW near a $300B market cap on its platformization thesis) framed as structurally insulated from the (separately reported, last week's) weak Chicago PMI. Energy refiners (MPC, VLO) framed as a direct hedge against the current oil-price spike — relevant tailwind context for held APA. Consumer-discretionary names (TGT, EXPE) flagged as vulnerable to oil-driven erosion of discretionary spending. Fintech (PYPL, STT, HOOD, ADP) described as bifurcating, with PYPL's Advent/Stripe M&A-interest collapse (-12.82% last week) cited as a legacy-processor warning sign — relevant sector-risk context for held XYZ, not a direct XYZ thesis break. Life-sciences/CRO names (CRL, VEEV, IQV, RVTY, CNC, INCY) and NTAP framed as low-beta/defensive this session; ZBRA flagged as exposed to the same industrial softness underlying the Chicago PMI miss.

### Held-Position Thesis Check
- **APA (core, +2.02% unrealized):** No thesis-breaking news found (2nd Gemini pass). Pre-market tailwind from the Iran-driven oil spike; Citigroup recently raised its price target to $44. Week 1 off the core watchlist (see Account section) — watch item for the 9/4 refresh, no action today.
- **BAX (core, -4.18% unrealized):** No thesis-breaking news found (2nd Gemini pass) — Q2 beat, raised FY26 guidance, new CFO (effective Oct 1), $600M debt tender offer all cited as positive/neutral, cefazolin recall unchanged. **This is week 2 off the core watchlist — TRADING-STRATEGY.md's thesis-broken exit rule is triggered regardless of P&L. Top actionable item for today's market-open session** (this pre-market workflow does not place trades itself).
- **XYZ (core, -0.46% unrealized):** No thesis-breaking news found (2nd Gemini pass) — Q2 beat ($1.02 EPS on $6.62B revenue), BTIG price-target raise to $100, a new $6.77M institutional position disclosed. Sector-risk context from PYPL's M&A collapse (see Market Context) is a watch item, not a thesis break, consistent with prior sessions' treatment.
- **MNKD (satellite, 0.00% unrealized):** No thesis-breaking news found (2nd Gemini pass). Near-term dilution concern noted (12.85M share shelf tied to a $50M private placement at $3.89/share) but framed as not affecting the core commercial thesis (Furoscix/Afrezza revenue +43% YoY). Satellite catalyst-proximity table (1st pass) confirms no binary event inside the 5-trading-day window.
- **RIGL (satellite, +11.09% unrealized):** No thesis-breaking news found (2nd Gemini pass). Q2 beat, FY26 guidance raised to $285-295M, a newly disclosed 10.49% BlackRock stake cited as supportive. No binary catalyst inside the 5-day window (1st pass). Continues approaching, not yet at, the +25% satellite tighten threshold.
- **EDGX (income):** No thesis-breaking news found. Trading ex-dividend today (8/31) for a $0.0466/share distribution payable 9/3 — routine, not a thesis event.
- **SGOV (income):** No thesis-breaking news found. Pricing normally (~$100.69); elevated short-term yields from the hawkish Fed repricing continue to support its ~3.74% trailing yield.
- **SPHY (income):** No thesis-breaking news found. ~7.19% yield, no credit-shock or liquidity event reported overnight; hawkish-rate repricing is a mild valuation headwind, not a thesis break.

### Core Trade Ideas (from current WATCHLIST.md core list, Week of 2026-08-28)
Nothing here is pre-approved — the full momentum/FCF/rating gate still runs fresh at market-open.
- **CRWD, PANW:** No adverse overnight news; both framed as structurally insulated from the industrial-cycle weakness (Chicago PMI) given cybersecurity's non-discretionary budget status. CRWD's post-Q2 channel-check momentum (Stifel survey) is a positive but stale (last week's print), not new information.
- **MPC, VLO:** Positive tailwind context from the Iran-driven oil spike (>5% Brent move) — refining crack-spread expansion thesis, same treatment as APA above.
- **TGT, EXPE:** Flagged as vulnerable to oil-driven consumer-discretionary erosion — a caution flag for market-open's momentum re-screen, not a signal either way on its own.
- **PYPL, STT, HOOD, ADP:** No adverse ticker-specific news for the watchlist name PYPL beyond the already-known M&A-collapse selloff (relevant mainly as XYZ sector-risk context, PYPL itself not held). STT/HOOD/ADP framed as rate/labor-sensitive; no idiosyncratic overnight news.
- **CRL, NTAP, VEEV, ZBRA, IQV, CNC, RVTY, INCY:** No idiosyncratic overnight news; framed as defensive/low-beta this session except ZBRA, flagged (again) as industrial-cycle-exposed.
- **DELL, MRVL, HPE, ANET, PLTR:** No fresh session-specific overnight news beyond last week's already-known post-earnings moves; not independently re-verified this session.

### Satellite Trade Ideas (from current WATCHLIST.md satellite list, Week of 2026-08-28)
- **MNKD, RIGL** (held) — thesis-intact per above; already at 2/4 satellite slots.
- **CGEM, TWIN, SHIP, ZYME, DCO, PLPC** (not held) — Gemini's mandatory 5-day catalyst-proximity review (1st pass) confirms **zero** satellite-watchlist tickers have a documented binary catalyst inside the August 31–September 5, 2026 window. TWIN, SHIP, DCO all have already-resolved Q2/Q4 earnings behind them (next prints in Oct/Nov, outside the window); DCO's Investor Day (9/17) and PLPC's product-launch PR are informational, not binary; CGEM and ZYME have no FDA AdCom/PDUFA scheduled.
- No documented, verified binary catalyst inside the 5-trading-day window for any watchlist satellite ticker that would require entry/exit action today.

### Risk Factors
- **BAX's mandatory thesis-broken exit (2 weeks off the core watchlist) is due at today's market-open — top actionable item, not yet executed by this workflow.**
- APA is week 1 off the core watchlist — watch item for the 9/4 refresh, no action yet.
- Gemini data-quality: the consolidated call opened mid-document (missing sections 1-2 and most of the held-ticker check) and required one resubmit; even after the resubmit, no explicit VIX level or S&P 500 futures figure was returned by either pass — flagged, not fabricated, and worth a WebSearch fallback next time it recurs.
- Real, significant geopolitical escalation (U.S.-Iran military action near the Strait of Hormuz) driving a >5% Brent crude spike to $90+/barrel — a live tailwind for held APA and watchlist MPC/VLO, but a broader risk-off/stagflation-shock input for the rest of the market pending today's open reaction.
- Hawkish Fed repricing (Warsh Jackson Hole remarks, September rate-hike odds ~35%→~60%, 2yr yield +12bp to 4.36%) — a modest tailwind for SGOV/SPHY yields, a headwind for risk assets generally; no held position's thesis is broken by it today.
- Cash at 20.17%, only 0.17pp above the 20% floor — watch for a floor breach at today's BAX exit (should push cash up) or on continued price drift elsewhere, per TRADING-STRATEGY.md's price-drift floor-breach rule.
- No held position at/beyond its sleeve hard-cut; no verified satellite catalyst inside the gap-risk window requiring forced action.
...
### 3 PM Session Note
- No trades. No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (market-open's BAX exit and HPE buy were both verified/logged directly, not staged as a list) — per STEP 3 the buy side stayed HOLD. This week (Aug 31 start) closes at core 1/6 (HPE 8/31), satellite 0/4 new trades — valid per the Patience Rule.
- Risk sweep: no position at/beyond its sleeve hard-cut — APA +0.52%, HPE -0.57%, XYZ -1.62% (all vs. -7% core cut); MNKD -1.38%, RIGL +9.92% (both vs. -15% satellite cut). All 8 open positions confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74.5 dust flagged every prior sweep, not a new gap). No stop moved down.
- Thesis check: no new research run this window — nothing moved sharply enough since 11am (largest intraday mover RIGL -1.05%) to warrant a fresh WebSearch/Gemini check, and watchlist status doesn't change intraday. HPE/XYZ remain current on this week's (Aug 28) core watchlist; APA stays week-1-off-list (no forced exit until the 9/4 refresh). MNKD/RIGL catalysts remain resolved — MNKD's Wells Fargo conference (Sept 4) is a non-binary IR event 4 trading days out, RIGL's VEPPANU launch resolved 8/13 — neither inside the 1-2 trading-day gap-risk window. No stop tightening this window — RIGL below the +25% satellite threshold, APA/HPE/XYZ all below the core +15% threshold. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2. Cash 20.09% ($19,129.47 of $95,252.10 equity), at/above the 20% floor, no sweep needed.
- Risky positions closed today: none (BAX's thesis-broken exit was already executed at 9:30am — see TRADE-LOG).
- Lesson: none new this window; the 9:30am same-day SGOV round-trip lesson stands as the session's open item.
...
### 9:30 AM Session Note
- **Buy-side: HOLD.** No core or satellite candidate — pre-market's Decision explicitly sourced none (Gemini truncation left the overnight watchlist sweep unconfirmed). Nothing new surfaced at market-open. This week (Aug 31 start) stays core 1/6 (HPE 8/31), satellite 0/4 — valid per the Patience Rule.
- **Income sweep executed:** BUY SGOV 94 sh @ $100.470613, funded from XYZ's overnight stop-out proceeds — cash was 30.55% of equity pre-trade (well above the 20% floor), now 20.26% post-trade. Replaced the SGOV stop with a 114-share 5% trailing GTC (trigger $95.39425). Full detail in TRADE-LOG.md.
- **Data-integrity flag:** EDGX's quote/mark froze at market-open (stale $26.61 bid, $0 ask, timestamp stuck at yesterday's 19:59:30Z close) while the position mark showed a -13.72%/$23.00 print that never refreshed across 3 re-polls. No halt/NAV news found via WebSearch. EDGX's 5% trailing stop (trigger $25.9825) did not fire despite the mark being well below it — treated as a frozen paper-feed print, not a real move, per the zero-ask/halted-ticker skip rule. No EDGX trade taken. Logged in TRADE-LOG.md; re-check next session.
- **Risk sweep:** no other position near its hard-cut (APA +3.9%, HPE -3.5% vs -7% core cut; MNKD +1.8%, RIGL +17.0% vs -15% satellite cut). All non-EDGX stops confirmed live via `alpaca.sh orders`. No stop tightening this window — RIGL still below the +25% satellite threshold. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- **Lesson:** a frozen/zero-ask quote can show a large fake intraday loss without the underlying stop ever firing — treat a mark that doesn't refresh across repeated polls, with no ask and no news, as a data artifact before assuming a real move or a stop failure. Worth a standing check (does the quote actually update between polls) whenever a position looks like it slammed through its stop without filling.

### 11 AM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG — per STEP 3 this window trades from that list only, so no buys placed. This week (Aug 31 start) stays core 1/6 (HPE 8/31), satellite 0/4 — valid per the Patience Rule.
- **Risk sweep:** all 7 open positions (APA, HPE, MNKD, RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stops via `alpaca.sh orders`, none missing. No position at/near its hard-cut (APA +3.85%, HPE -4.97% vs -7% core cut; MNKD +0.63%, RIGL +14.17% vs -15% satellite cut). No stop tightening — RIGL still below the +25% satellite threshold, APA/HPE not in sufficient gain. No stop moved down. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- **HPE earnings correction:** pre-market's note reads ambiguously — confirmed via WebSearch that HPE's Q3 FY26 print is scheduled for **today (9/2) after market close**, not yesterday's close. No reaction to assess yet; remains tomorrow's top event-risk item.
- **Cash floor:** 19.72% of equity (18,684.57 / 94,772.46), 0.28pp under the 20% floor — a pure price-appreciation drift (cash unchanged since market-open's sweep, equity rose) of well under 1% of equity. Per the Income sleeve drift rule, immaterial and requires no same-day action; will act only if this persists past 3 consecutive sessions.
- **Data-integrity flags (2):** EDGX's quote endpoint remains frozen (bid $26.61/ask $0, timestamp still stuck at 2026-09-01T19:59:30Z) though its position mark has normalized to a plausible $26.87 (vs. the fake -13.7%/$23.00 mark seen at market-open) — continued monitoring, no trade needed. **New this window:** RIGL's quote also came back frozen/implausible (bid $41.11/ask $54.86, ~25% spread, identical timestamp across two polls 10s apart) while its position mark ($47.77) tracks normally against yesterday's $47.50 close — same stale-quote pattern as EDGX, not a real spread widening. Neither is actionable today (no trade needed on either name), but both are logged in case a future session needs to trade one of these tickers and must apply the wide/frozen-quote skip rule.
- **Lesson:** the frozen-quote pattern isn't unique to EDGX — it can hit any ticker's quote endpoint independently of whether its position mark/last-trade price has refreshed normally. Always cross-check a suspicious quote (bid/ask, spread) against the position's `current_price`/`lastday_price` and a repeat poll before treating it as a real market condition.

### 3 PM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section appeared in today's RESEARCH-LOG at any window (pre-market, 9:30am, 11am, 3pm) — Gemini truncation left the overnight watchlist sweep unconfirmed all session, so no new core or satellite candidate was sourced. This week (Aug 31 start) closes at core 1/6 (HPE 8/31), satellite 0/4 new trades — valid per the Patience Rule.
- **Risk sweep:** no position at/beyond its sleeve hard-cut (APA +5.63%, HPE -1.05% vs -7% core cut; MNKD +1.13%, RIGL +17.56% vs -15% satellite cut); all 7 open positions confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep, not a new gap). No stop moved down. No satellite catalyst inside the 1-2 trading-day gap-risk window (MNKD's Wells Fargo conference is 9/4, explicitly non-binary IR event per WATCHLIST.md; RIGL's VEPPANU launch resolved 8/13).
- **Thesis check:** HPE remains current on this week's (8/28) core watchlist. APA stays week 1 off-list (dropped 8/28) — the mandatory two-week exit decision falls at the 9/4 Friday refresh, not before; no forced action this window. No new adverse news found for either since 11am (no fresh WebSearch run — nothing moved sharply enough to warrant one; largest mover was RIGL +3.5% intraday). HPE reports Q3 FY26 earnings after today's close — no action pre-print, tomorrow's reaction is the top event-risk item.
- **Stop tightening:** none — APA/HPE below the core +15% threshold, RIGL (+17.56%) still below the satellite +25% threshold. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- **Cash floor:** 19.59% of equity ($18,684.57 / $95,366.76), 0.41pp under the 20% floor — pure price-appreciation drift (cash unchanged since this morning's sweep, equity rose further). First flagged under-floor at 11am today (day 1) — per the Income sleeve drift rule this is immaterial at <1% of equity and requires no same-day action; only acts if the drift persists past 3 consecutive trading sessions. No sweep this window.
- **Data-integrity follow-up:** EDGX's and RIGL's position marks (via `alpaca.sh positions`) both show normal, sane intraday changes this window (EDGX +0.52%, RIGL +3.55%, both consistent with plausible price action) — the frozen-quote artifacts flagged at market-open/11am appear resolved; no trade needed on either, no further action.
- Risky positions closed today: none.

## 2026-09-03 — Pre-market Research

### Account
- Equity: $94,895.19 | Cash: $18,684.56 (19.69%) | Buying power: $284,238.16 | Daytrade count: not present in account payload (same pattern as prior sessions)
- Core exposure: $14,403.80 (15.18%: APA $5,396.60 / HPE $9,007.20) | Satellite exposure: $10,496.19 (11.06%: MNKD $4,862.31 / RIGL $5,633.88) | Income exposure: $51,310.65 (54.07%: EDGX $19,980.27 / SGOV $11,522.54 / SPHY $19,807.84)
- All 7 open positions (APA, HPE, MNKD, RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stops via `alpaca.sh orders`, none missing: APA 10%/$40.6125 (hwm $45.125), HPE 10%/$47.799 (hwm $53.11), MNKD 15%/$3.74 (hwm $4.40), RIGL 15%/$42.245 (hwm $49.70), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.399 on 114 covered sh (hwm $100.42; the 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep).
- **This week (Aug 31 start):** core 1/6 new trades used (HPE 8/31), satellite 0/4.

### Data-quality flag — Gemini opened mid-document again
The consolidated Gemini Deep Research call returned a substantive report on HPE's earnings, the core/satellite watchlist sweep, and the satellite 5-day catalyst audit, but it opened directly on "Analyst Reaction and The Margin Dilemma" (HPE) — section 1 (S&P futures direction / VIX level) never arrived as a standalone figure, and the held-ticker overnight check (section 3) covered HPE in depth but never explicitly addressed APA. Same recurring truncation pattern flagged repeatedly in this log since July. Per the data-quality guard, filled the two gaps via native WebSearch fallback below rather than inventing either figure.

### Market Context (Gemini + WebSearch fallback for the VIX/futures figure and APA check)
- **VIX futures ~16.55–16.75** (WebSearch fallback, Investing.com) — down from 9/2's elevated ~17.25 reading, though still above the pre-risk-off ~14.43 baseline from Friday 8/28. S&P 500 sits ~1.3% below its all-time high per the same search; no explicit premarket S&P futures % figure was available from either source — not fabricated.
- 10-year Treasury yield near ~4.8%, driving a violent, systematic valuation reset in high-multiple software/cybersecurity (CRWD -7% to $214.56, PANW -6% to $360.81) despite both beating expectations — a rate-driven multiple-compression story, not a fundamentals failure (IGV -3% vs. QQQ -0.9% over the same stretch, per Gemini).
- Today's calendar (per Gemini): ISM Services PMI, with Friday's Nonfarm Payrolls and mid-September CPI print flagged as the next two catalysts that will "seize control" of the tape. A recent ADP private-sector employment report also missed expectations (38K vs. 47K est.) per the same report.
- Energy refiners (MPC, VLO — both core-watchlist names) continue to benefit from a geopolitical risk premium as the Iran/Strait-of-Hormuz-driven crude strength persists.

### Held-Position Thesis Check
- **HPE (core, held) — Q3 FY26 earnings, reported after 9/2 close:** Beat-and-raise on every headline number — revenue $12.2B vs. $11.97B consensus (+2.26%), non-GAAP EPS $1.11 vs. $0.93 consensus (+20.65%), Q4 guide $13.9-14.8B revenue / $1.20-1.30 EPS vs. $13.04B / $1.07 consensus (a material raise on both). Non-GAAP operating margin nearly doubled YoY to a record 16.2% (from 8.5%). Despite this, shares fell ~4-5% after-hours (to ~$49.72 from the $51.83 close) — management guided **sequential** operating-margin decline as high-volume, lower-margin AI server sales outgrow the higher-margin networking/software mix, compounded by flagged supply-chain constraints. Live quote this morning: $50.04 (-3.45% vs. yesterday's $51.83 close), consistent with a partial recovery off the after-hours low. **Assessment: fundamentals and forward guidance are strong — this reads as a margin-mix/valuation reaction, not a thesis break.** HPE remains current on this week's (8/28) core watchlist. Unrealized P&L -5.03%, well clear of the -7% core cut; stop (10% trailing, $47.799, hwm $53.11) is unaffected and well below the current price. No action.
- **APA (core, held):** No overnight news found (Gemini's report did not address APA directly this session; WebSearch fallback also found nothing dated 9/2-9/3, most recent substantive news remains the 8/5 Q2 beat). **APA is week 2 of 2 off the core watchlist (dropped 8/28) — tomorrow's 9/4 Friday refresh is the mandatory thesis-broken exit decision point if APA is still unlisted, regardless of P&L.** Flagging prominently for tomorrow. No action today (research-only workflow; the decision itself falls to Friday's refresh/market-open).
- **MNKD (satellite, held):** Per Gemini's 5-trading-day catalyst audit, no confirmed binary regulatory/clinical catalyst falls inside the current window for any satellite watchlist name. MNKD's Sept 4 Wells Fargo Healthcare Conference remains a non-binary IR appearance, consistent with WATCHLIST.md. No new thesis-breaking news. No action.
- **RIGL (satellite, held):** No 5-day-window catalyst (per the same audit). **New forward-looking item:** Gemini's report notes RIGL completed primary data collection for a Phase 3 trial in May 2026, and under standard reporting timelines a top-line readout "could materialize at any point in Q4 2026" — no company-confirmed date yet. Not a near-term gap-risk trigger today, but flagging for ongoing surveillance and for the weekly-review to fold into RIGL's tracked catalyst-date field. No action.
- **EDGX / SGOV / SPHY (income, held):** No idiosyncratic ETF-level news; not thesis-screened per TRADING-STRATEGY.md. No action.

### Core Watchlist Context (informational — not a re-screen; Friday's weekly refresh owns momentum verification)
Notable overnight moves among watchlist names per Gemini, none independently re-verified via fresh Alpaca bars this session: **DELL** +~8% on an AI-server earnings/guidance blowout (122% YoY server/networking-storage growth); **ANET** +6.2%; **NTAP** beat (EPS $2.58); **PLTR** raised Q3 revenue guidance to $2.160-2.164B on continued hyper-growth. **MRVL** fell ~10% on weak Q3 gross-margin guidance — same margin-scrutiny dynamic hitting HPE. **CRWD/PANW** both sold off (-7%/-6%) on rate-driven software-multiple compression despite beating estimates (see Market Context). **TGT** launched Target Beauty Studio (incremental, not thesis-altering); **PYPL** range-bound, no idiosyncratic catalyst; **ADP** flagged as sensitive to the weak private-payrolls print. None of this changes today's Decision — no fresh momentum/FCF re-verification was run on any name this session, so no new core candidate is sourced regardless of these moves.

### Core Trade Ideas (from current WATCHLIST.md core list)
None. This session's role is overnight-news/thesis confirmation on held names, not a momentum re-screen — no candidate was independently re-verified via `alpaca.sh bars` today, so none is sourced regardless of the moves noted above.

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
None. MNKD and RIGL (both held) have no new entry signal (see Held-Position Thesis Check). **CGEM (not held):** Gemini flags an imminent, undated Q3 2026 multi-dose CLN-978 (rheumatoid arthritis) data readout as a "severe volatility vector" — not inside the 5-trading-day window per the audit, so no gap-risk cap applies today, but worth flagging for the weekly-review's satellite catalyst tracking given how close the quarter is to closing. No other satellite watchlist ticker (TWIN, SHIP, ZYME, DCO, PLPC) has fresh news this session.

### Risk Factors
- HPE's post-earnings margin-mix selloff (-3.45% this morning) is a real, live valuation risk on a currently-held core position, even though the underlying beat-and-raise thesis is intact — worth continued monitoring over the next few sessions, not an action today.
- APA's mandatory two-week thesis-broken exit decision is due at tomorrow's (9/4) Friday refresh — the top event-risk item into that session.
- VIX futures still elevated (~16.55-16.75) versus the pre-risk-off ~14.43 baseline, though cooling from 9/2's ~17.25.
- 10-year yield near ~4.8% is structurally capping high-multiple software/cyber names (CRWD, PANW) on the core watchlist regardless of execution quality — a macro overhang to keep in mind before considering any re-entry in that cohort.
- Cash 19.69%, 0.31pp under the 20% floor — pure price-appreciation drift, day 2 of tracking (first flagged 11am 9/2). Immaterial per the Income sleeve drift rule; will need action if it persists past 3 consecutive trading sessions (i.e., check closely tomorrow).
- RIGL's next Phase 3 data readout (potentially Q4 2026, undated) and CGEM's imminent-but-undated Q3 CLN-978 readout are both forward gap-risk items to track, not immediate triggers.
- Gemini's report did not explicitly address APA or give a numeric VIX/futures figure this session — filled via WebSearch fallback per the data-quality guard; no figure above is fabricated.

### Decision
HOLD — no core or satellite trade sourced this session (no fresh momentum re-screen run; HPE's earnings reaction and the MRVL/CRWD/PANW moves are informational, not independently re-verified). No held position broken or at its hard cut; no satellite catalyst inside the gap-risk window. Valid per the Patience Rule. Top items for market-open/tomorrow: (1) HPE's continued post-earnings price action — no thesis break, but worth a fresh look at market-open; (2) APA's mandatory 9/4 Friday two-week thesis-broken exit decision — the top item; (3) monitor whether the cash-floor drift reaches day 3 tomorrow; (4) RIGL's and CGEM's newly-flagged forward catalyst timelines for the weekly-review to incorporate; (5) no satellite catalyst inside the gap-risk window currently.

### 9:30 AM Session Note
- **Buy-side: HOLD.** No core or satellite candidate — pre-market's Decision sourced none (no fresh momentum re-screen run this session). No "Approved Trades (verified)" list existed. This week (Aug 31 start) stays core 1/6 (HPE 8/31), satellite 0/4 — valid per the Patience Rule.
- **HPE stopped out overnight.** The 10% GTC trailing stop filled mechanically at 9:32am ET (before this session ran), 180 sh @ $46.116944 vs. entry $52.690889, realized -$1,183.31 (-12.48%) — a real overnight/pre-market gap past the -7% hard-cut, not a same-day trigger. This follows through on the post-Q3-earnings margin-mix selloff flagged this morning as a live valuation risk (fundamentals beat-and-raise, but management guided sequential margin compression) — no thesis break in the business, the stop caught a valuation/gap reaction. Core sleeve is now down to 1 open position (APA). Full detail in TRADE-LOG.md.
- **Income sweep executed:** BUY SGOV 79 sh @ $100.43, funded from HPE's freed cash (cash was 28.65% of equity pre-trade, well above the 20% floor, with no core/satellite buy this session to fund instead) — now 20.22% post-trade. Replaced the SGOV stop with a 193-share 5% trailing GTC (trigger $95.399095). Full detail in TRADE-LOG.md.
- **Risk sweep:** all 6 remaining open positions (APA, MNKD, RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stops via `alpaca.sh orders`, none missing. No position at/near its hard-cut (APA +4.1% vs. -7% core cut; MNKD +0.75%, RIGL +17.8% vs. -15% satellite cut). No stop tightening — RIGL still below the +25% satellite threshold. No stop moved down. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- **Flag for tomorrow:** APA remains week 2 of 2 off the core watchlist — the 9/4 Friday refresh is the mandatory thesis-broken exit decision point regardless of P&L, now the top open item alongside rebuilding core exposure (1 of 6 slots) after today's HPE exit.

### 11 AM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section in today's RESEARCH-LOG (pre-market/9:30am both sourced none) — no buys placed. This week (Aug 31 start) stays core 1/6 (HPE 8/31, since stopped out), satellite 0/4 — valid per the Patience Rule.
- **Risk sweep:** APA +4.11% (core, vs -7% cut), MNKD +0.38%, RIGL +17.33% (satellite, vs -15% cut) — none near a hard-cut, no cuts taken. All 6 open positions (APA, MNKD, RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stops via `alpaca.sh orders`, none missing.
- **No stop tightening:** APA below core +15% threshold; RIGL (+17.33%) still below satellite +25% threshold; MNKD not in meaningful gain. No stop moved down.
- **Thesis check:** APA remains week 2 of 2 off the core watchlist — tomorrow's 9/4 Friday refresh is the mandatory thesis-broken exit decision point, no action due today. MNKD/RIGL theses intact, no new news, no satellite catalyst inside the gap-risk window (MNKD's Wells Fargo conference 9/4 is non-binary IR). No positions moved sharply enough intraday (all under ~0.7%) to warrant a Gemini consolidated check.
- **Cash floor:** 20.23% ($19,051.64 / $94,171.55 equity), back above the 20% floor — resolved by this morning's market-open SGOV sweep-back after the HPE stop-out. No drift action needed.
- 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.

### 3 PM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section appeared in today's RESEARCH-LOG at any window (pre-market, 9:30am, 11am, 3pm) — no fresh momentum re-screen sourced a candidate. This week (Aug 31 start) stays core 1/6 (HPE 8/31, since stopped out), satellite 0/4 — valid per the Patience Rule.
- **Risk sweep (re-run since 11am):** no position at/beyond its sleeve hard-cut (APA +3.81% vs. -7% core cut; MNKD +0.25%, RIGL +15.72%, both vs. -15% satellite cut). All 6 open positions (APA, MNKD, RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep, not a new gap). No stop moved down.
- **No stop tightening this window:** APA below the core +15% threshold; RIGL (+15.72%) still below the satellite +25% threshold; MNKD not in meaningful gain.
- **Thesis check:** APA remains week 2 of 2 off the core watchlist (dropped 8/28) — tomorrow's 9/4 Friday refresh is the mandatory two-week thesis-broken exit decision point, not today; no forced action this window. MNKD/RIGL theses intact per this morning's research, nothing moved sharply enough intraday (largest mover RIGL, -2.03%) to warrant a fresh consolidated check. No satellite catalyst inside the 1-2 trading-day gap-risk window (MNKD's Wells Fargo conference 9/4 remains non-binary IR; RIGL has no confirmed near-term date). No closes.
- **Cash floor:** 20.21% ($19,051.64 / $94,268.54 equity), at/above the 20% floor — no sweep needed this window.
- 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- **Risky positions closed today: none.** (HPE's overnight mechanical stop-out at market-open was a standard trailing-stop fill, not a discretionary risk-sweep catch this window.)
- Pattern for Friday's review: HPE's 9/2 post-earnings gap past its 10% trail (fundamentals beat-and-raise, stop caught a valuation/margin-mix reaction) is the second earnings-gap stop-out this phase (after AMD/KLAC's 7/17 sector-selloff gap) — worth a review-level note on whether core positions should avoid holding through a confirmed earnings date the way the satellite binary-catalyst rule already requires.

## 2026-09-04 — Pre-market Research

### Account
- Equity: $94,122.35 | Cash: $19,051.41 (20.24%) | Buying power: $282,564.88 (margin-inflated by the standing 4x multiplier flag — no margin/leverage used or planned)
- Core exposure: $5,263.50 (5.59%: APA) | Satellite exposure: $10,328.25 (10.97%: MNKD $4,799.25 / RIGL $5,529.00) | Income exposure: $59,479.19 (63.19%: EDGX $20,181.54 / SGOV $19,464.14 / SPHY $19,833.52)
- All 6 open positions (APA, MNKD, RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stop orders via `alpaca.sh orders`, none missing: APA 10%/$40.6125 (hwm $45.125), MNKD 15%/$3.74 (hwm $4.40), RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.4085 on 193 covered sh (hwm $100.43; the 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep).
- Core down to 1 open position since HPE's 9/3 overnight stop-out; satellite steady at 2/4 (MNKD, RIGL).
- **This week (Aug 31 start):** core 1/6 new trades used (HPE 8/31, since stopped out), satellite 0/4.

### Data-quality flag — Gemini truncated again (opened mid-document)
The consolidated Gemini Deep Research call first attempt failed outright (`curl` HTTP 500 from the Gemini API, exit 22) — retried once per the workflow's fallback allowance and the retry succeeded, but the returned report **opened mid-sentence** ("Despite leading its competitive group with an operating margin of 43%...", mid-way through an ANET discussion) — sections 1-2 (explicit S&P futures direction / VIX numeric level, today's economic-release calendar) and the explicit overnight-news check for held tickers APA, EDGX, SGOV, and SPHY never arrived as their own sections. Same recurring truncation pattern flagged repeatedly in this log since July. Per the data-quality guard, nothing below is fabricated from a source not actually returned — the VIX figure and APA check were filled via native WebSearch fallback (see below); EDGX/SGOV/SPHY have no idiosyncratic news to report either way (not thesis-screened per TRADING-STRATEGY.md, and no negative artifact found). Core watchlist names **DELL, MRVL, MPC, VLO, ZBRA** were not addressed anywhere in the returned report (their section apparently fell in the same truncated gap) — treated as data-unavailable, not screened, consistent with the guard.

### Market Context (Gemini + WebSearch fallback for the VIX figure)
- **VIX futures ~16.55–16.75** (WebSearch, Investing.com/Barchart) — consistent with Gemini's own later reference to a "sub-17 VIX," continuing to cool from 9/2's elevated ~17.25 peak, though still above the pre-risk-off ~14.43 baseline (Fri 8/28).
- Gemini's report (via its Strategic Synthesis section, the one part of the top-of-document material that did come through) flags a live Strait-of-Hormuz-driven energy shock: diesel crack spreads blown out to ~$106/bbl, a real secondary-inflation risk that could push the Fed toward "higher for longer" — a macro overhang on the market's rate-cut-dependent valuation premium heading into the Labor Day long weekend (Mon 9/7 is a scheduled market holiday; next trading session is Tue 9/8).
- No explicit numeric S&P 500 futures direction or a same-day economic-release calendar was recoverable from either source this session — not fabricated, flagged as a gap.
- Refiners MPC and VLO (both core watchlist, not currently held) are called out as structural cash-flow winners of the same geopolitical energy setup, per Gemini.

### Held-Position Thesis Check
- **APA (core, held) — this is the mandatory two-week thesis-broken decision point (off the core watchlist since the 8/28 refresh; today, 9/4, is the second consecutive Friday).** Gemini's report did not address APA directly (truncated gap). WebSearch fallback found **no adverse news** — the most recent items are constructive: Raymond James raised its APA price target to $54 from $50 on 9/2/2026, and the standing thesis (Q2 beat, $738M FCF, ≥60% of FCF returned to shareholders, $700M cost-cut program, >5% annual oil growth, Suriname 2028 buildout, $3B net-debt target) remains intact per public coverage. Unrealized P&L +2.09%, well clear of the -7% hard-cut. **This is a research-only session — it does not place trades.** Per TRADING-STRATEGY.md's thesis-broken rule, APA being off the current WATCHLIST.md two Fridays running (since 8/28) triggers a mandatory exit regardless of P&L once today's weekly screen-refresh confirms whether it clears or stays off the list — flagging prominently for the next trade-window session (market-open) to execute if APA is still unlisted after the refresh.
- **MNKD (satellite, held):** No new binary news. Gemini's satellite catalyst-window audit confirms MNKD has **no scheduled catalyst within the next 5 trading days** (next trading days span through Fri 9/11, given Mon 9/7's market holiday). Sept 4 Wells Fargo Healthcare Conference remains a non-binary IR appearance per standing WATCHLIST.md notes. Thesis intact, well clear of the -15% hard-cut (unrealized -1.25%).
- **RIGL (satellite, held):** No thesis-breaking news. Gemini flags RIGL **is** inside the 5-trading-day window — Citi (9/9) and Cantor Global Healthcare (9/10) conference presentations — explicitly characterized as informational/marketing appearances, not a binary regulatory or clinical event; does not trigger the gap-risk cap. Unrealized +15.9%, approaching but still below the +25% satellite tighten threshold — continue watching.
- **EDGX / SGOV / SPHY (income, held):** No idiosyncratic ETF-level news found in either source; not thesis-screened per TRADING-STRATEGY.md.

### New data point flagged for Friday's screen refresh (not actionable today)
Gemini's satellite catalyst-window audit surfaced a **near-term binary catalyst for CGEM (not currently held)**: a Phase 3 REZILIENT3 interim-data presentation for zipalertinib (EGFR exon 20 NSCLC) at the WCLC Presidential Symposium on **Sept 7, 2026** — inside the 5-trading-day window, and reported as having already met its primary PFS endpoint at interim analysis with an early-unblinding recommendation. This is a materially different (and much nearer-dated) event than CGEM's previously logged catalyst framing on WATCHLIST.md (FDA NDA target action Feb 27, 2027) — worth independently verifying via WebSearch and folding into the next screen refresh's satellite catalyst tracking; not actionable this session (CGEM is not held, no fresh momentum re-verification was run today, and this workflow does not source new candidates outside a confirmed momentum check). Similarly, ZYME (not held) has Wells Fargo (9/8) and Citi (9/9) conference appearances inside the window — both non-binary IR events per Gemini, consistent with ZYME's catalyst having already resolved 8/25 per the standing watchlist note.

### Core Trade Ideas (from current WATCHLIST.md core list)
None. Gemini's report covered ANET, PYPL, XYZ, HOOD, CRWD, PANW, NTAP, VEEV, PLTR, RVTY, IQV, CRL, EXPE, TGT, STT, CNC, ADP, INCY with no adverse news found on any, but this was an overnight-news/commentary sweep, not a fresh momentum/FCF re-screen (`alpaca.sh bars` was not re-run this session) — per STEP 4, no core candidate is sourced regardless. DELL, MRVL, MPC, VLO, ZBRA were not addressed in the returned report at all (truncation gap, not screened).

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
None. MNKD and RIGL (both held) have no new entry signal (see Held-Position Thesis Check). CGEM's newly-flagged near-term catalyst (see above) is a real, material event but not actionable this session — no fresh momentum re-verification was run, and per STEP 4 no new candidate is sourced from research/commentary alone. TWIN, SHIP, DCO, PLPC (not held) had no near-term catalyst inside the 5-day window per Gemini's audit; no new entry signal for any.

### Risk Factors
- **APA's mandatory two-week thesis-broken exit decision is due today (9/4 Friday refresh)** — the top event-risk item; no adverse news found, but the process rule is off-list-two-Fridays-running regardless of P&L, not discretionary.
- VIX still elevated (~16.55-16.75) vs. the pre-risk-off ~14.43 baseline, alongside a live Strait-of-Hormuz energy-shock/inflation risk per Gemini's synthesis — a real, sustained risk-off undertone into the Labor Day long weekend (Mon 9/7 market holiday, thin post-holiday liquidity a possible amplifier of any headline risk).
- CGEM's newly-surfaced Sept 7 Phase 3 data readout is a high-volatility event for a name currently on the satellite watchlist but not held — worth flagging now so a future session isn't caught off guard if considering an entry before the data drops.
- No held position is near its hard-cut: APA +2.09% (vs. -7% core cut), MNKD -1.25%, RIGL +15.9% (both vs. -15% satellite cut).
- 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- Gemini's report did not deliver an explicit numeric VIX/futures figure, a same-day economic calendar, or an explicit APA/EDGX/SGOV/SPHY overnight-news section this session — filled via WebSearch fallback or logged as "no adverse item found," not fabricated.

### Decision
HOLD — no core or satellite trade sourced this session (no fresh momentum re-screen run; the returned Gemini report was a commentary/overnight-news sweep with an inline satellite catalyst-window audit, not a re-verification of momentum/FCF gates). No held position broken or at its hard cut; MNKD/RIGL both thesis-intact, RIGL's upcoming conference appearances are non-binary and don't trigger gap-risk sizing. Valid per the Patience Rule. Top items for market-open: (1) **APA's mandatory two-week thesis-broken exit decision (today's 8/28-vintage-watchlist two-Friday rule) — execute the exit at market-open if today's weekly screen-refresh confirms APA is still off the core watchlist, regardless of P&L**; (2) rebuild core exposure (1 of 6 slots) once fresh candidates clear the momentum/FCF/spread gates — core sits well under its 85% target; (3) continue watching RIGL's approach toward its +25% satellite tighten threshold; (4) independently verify CGEM's newly-flagged Sept 7 Phase 3 data catalyst ahead of Friday's screen refresh; (5) monitor for any Strait-of-Hormuz/energy-shock headline risk over the long weekend.

### 9:30 AM Session Note
- HOLD confirmed — no fresh core/satellite candidate this window (pre-market decision stands); account/positions re-validated live (`alpaca.sh account`/`positions`/`orders`), all 6 positions (APA, MNKD, RIGL, EDGX, SGOV, SPHY) still carrying live GTC trailing stops, cash $19,051.41 = 20.25% of $94,072.33 equity, at/above the 20% floor — no sweep needed.
- **APA's mandatory two-week thesis-broken exit did NOT fire this session.** Per `routines/README.md`'s cron schedule, weekly-review (the workflow that runs the actual watchlist re-screen) fires at 4:00 PM Fridays — *after* today's 9:30am/11am/3pm windows, not before. memory/WATCHLIST.md's persisted "Current" section is still the 08-28 vintage (only one confirmed weekly absence for APA so far); today's would-be second confirming refresh hasn't run yet. Executing the exit now would have been acting on a refresh that doesn't exist yet in the record. Correct sequencing: after today's 4pm weekly-review confirms APA still off-list, the exit executes at the next trade window that follows it — Tue 9/8 market-open (Mon 9/7 is the Labor Day holiday, no session).
- Lesson for the runbook: pre-market's "execute at market-open if today's refresh confirms" phrasing reads as same-day-morning-actionable but the actual cron order (weekly-review last, after daily-summary) means a Friday morning session can never be the one that acts on that day's own refresh — worth tightening that phrasing next time to say "next trade window after weekly-review," not "market-open."

### 11 AM Session Note
- HOLD — no "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG, so per STEP 3 no buys placed this window; this week (Aug 31 start) stays core 1/6 (HPE 8/31, since stopped out), satellite 0/4. No cuts (no position at/beyond its sleeve hard-cut: APA +2.32% vs. -7% core cut; MNKD +1.50%, RIGL +15.32%, both vs. -15% satellite cut) and no tightens (APA below the core +15% threshold, RIGL +15.32% still below the satellite +25% threshold). All 6 positions (APA, MNKD, RIGL, EDGX, SGOV, SPHY) reconfirmed via `alpaca.sh account`/`positions`/`orders` carrying live GTC trailing stops, none missing, none moved down; cash $19,051.41 = 20.23% of $94,202.15 equity, at/above the 20% floor — no sweep needed. No sharp unexplained intraday move on any held name (largest: MNKD intraday +2.53%) — no ad hoc Gemini call this window.

### 3 PM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section appeared at any window today (pre-market, 9:30am, 11am, this window) — per STEP 3 nothing was quoted or gated this session. This week (Aug 31 start) stays at core 1/6 (HPE 8/31, since stopped out), satellite 0/4 — valid per the Patience Rule. No buys placed.
- **Risk sweep, live via `alpaca.sh account`/`positions`/`orders`:** equity $94,224.92, cash $19,051.41 (20.22%, at/above the 20% floor, no funding sweep needed). No position at/beyond its sleeve hard-cut — APA +0.94% (vs. -7% core cut); MNKD +1.88%, RIGL +16.59% (both vs. -15% satellite cut). All 6 open positions confirmed carrying live GTC trailing stops via `alpaca.sh orders`, none missing (APA 10%/$40.6125, MNKD 15%/$3.74, RIGL 15%/$42.415, EDGX 5%/$25.9825, SPHY 5%/$22.23, SGOV 5%/$95.4465 on 193 covered sh — hwm ticked up to $100.47 intraday, the 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep, not a new gap). No stop moved down.
- **No stop tightening this window** — APA (+0.94%) well below the core +15% threshold; MNKD (+1.88%) and RIGL (+16.59%) both below the satellite +25% threshold (RIGL continues approaching it — watch).
- **APA's mandatory two-week thesis-broken exit did not execute this window either**, consistent with the 9:30am note's sequencing finding: `routines/README.md`'s weekly-review (the workflow that actually re-screens and persists WATCHLIST.md) runs at 4:00pm Fridays, after this 3pm window. Acting now would mean exiting on a refresh that hasn't happened yet. No adverse news on APA found across any window today (Raymond James PT raise to $54 on 9/2 was the most recent item, per pre-market WebSearch). The exit remains queued for the next trade window after today's 4pm weekly-review confirms the list — Tue 9/8 market-open (Mon 9/7 is the Labor Day holiday, no session).
- No satellite catalyst inside the 1-2 trading-day gap-risk window as of this close: MNKD's Wells Fargo Healthcare Conference was today (9/4), already logged as a non-binary IR event, not a trigger; RIGL's Citi (9/9)/Cantor (9/10) conference appearances remain outside the window measured from the next trading day (Tue 9/8) and are non-binary per this morning's Gemini audit.
- Thesis check: no held position's thesis broke intraday; nothing moved sharply enough (largest mover RIGL, well within normal range) to warrant a fresh ad hoc check beyond this morning's research. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- **Risky positions closed today: none.**
...
### Account
- Equity: $94,210.02 | Cash: $19,051.41 (20.22%) | Buying power: $282,722.88 (margin-inflated by the standing 4x multiplier flag — no margin/leverage used or planned) | Daytrade count: not present in account payload (same pattern as prior sessions)
- Core exposure: $5,238.09 (5.56%: APA) | Satellite exposure: $10,456.98 (11.10%: MNKD $4,908.60 / RIGL $5,548.38) | Income exposure: $59,463.54 (63.12%: EDGX $20,171.54 / SGOV $19,467.04 / SPHY $19,824.96)
- Cash 20.22%, at/above the 20% floor — no sweep needed.
- All 6 open positions (APA, MNKD, RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stop orders via `alpaca.sh orders`, none missing: APA 10%/$40.6125 (hwm $45.125), MNKD 15%/$3.74 (hwm $4.40), RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.4465 on 193 covered sh (hwm $100.47; the 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep). No auth errors on account/positions/orders calls.
- **First live trading session since Friday 9/4** (Monday 9/7 was the Labor Day market holiday). This week's counters reset fresh: core 0/6, satellite 0/4.

### Data-quality flag — Gemini truncated again (missing sections 1-3)
The consolidated Gemini Deep Research call completed (exit 0) but the captured output opens mid-document on the tail of the Technology cohort's CRWD writeup — section 1 (S&P futures/VIX), section 2 (top market-moving catalysts/economic releases), and section 3 (held-ticker APA/MNKD/RIGL/EDGX/SGOV/SPHY overnight-thesis check) never arrived as dedicated sections. Section 4's per-ticker coverage (all 24 core watchlist tickers + all 7 satellite tickers) did come through complete. Same recurring truncation pattern flagged in this log since July. Per the data-quality guard, sections 1-3 below are filled via native WebSearch fallback, not fabricated from anything Gemini didn't actually return. One embedded Gemini figure is flagged as lower-confidence and not used: its "Strategic Synthesis" paragraph cites a "14.53 spot VIX" — this conflicts with an independently WebSearch-confirmed 15.12 VIX close for 9/7 (up 4.06%), so the WebSearch figure is used instead below.

### Market Context (WebSearch fallback — Gemini's sections 1-2 unrecoverable this session)
- **VIX:** confirmed close of **15.12 on 9/7** (up 4.06% on the day) — a real uptick from the 14.32 close logged 9/4, consistent with a risk-off tilt into today's open, not yet an elevated-fear level.
- **S&P 500 futures/broad market:** S&P 500 closed 9/7 at 7,718.60 (-0.38%); Dow, S&P, Nasdaq all finished 9/7 in the red on broad selling pressure. Dow futures edging lower again pre-market for 9/8.
- **Iran/Strait of Hormuz — still live, but a partial de-escalation signal overnight:** oil extended its gain (WTI >$92/bbl, Brent settled ~$97 Monday 9/7) as traders watch for details of an Iran-Oman accord reportedly in its "final stages" that would establish a temporary safe shipping route through Hormuz. This is a materially different framing than 9/6-9/7's "strait effectively closed" language — a managed-corridor deal, if finalized, would be a genuine easing step — but Iran simultaneously warned that ships face continued attack risk near Oman, so the underlying conflict remains unresolved and the risk premium in oil is holding, not unwinding. Treat as: real news, ambiguous net effect, not yet a confirmed resolution.
- **Economic releases:** ADP National Employment Report (the payroll-processor's private-sector print, not the ticker) showed a weak +38k private-sector jobs for August, but base pay rose 3.2% and gross pay rose 4.7% — a soft-jobs/sticky-wages divergence that keeps the ~60% probability of a 9/15-16 FOMC hike as the market's working baseline (per Gemini's synthesis section, which did come through), a genuine headwind for long-duration growth names and for held SPHY's high-yield-credit exposure, while continuing to support SGOV's short-duration-safe-haven thesis.
- No other single-name-moving economic release found for today specifically beyond the above.

### Held-Position Thesis Check (WebSearch fallback for APA — Gemini's section 3 unrecoverable; MNKD/RIGL/income cross-checked against Gemini's complete section 4)
- **APA (core, held, +1.60% unrealized: $42.61 → $43.29):** No new adverse company-specific news. One new data point: Seaport Research initiated coverage at Neutral (previously logged: Raymond James raised PT to $54 on 9/2 — unchanged). Oil-price tailwind (Brent ~$97, ongoing Hormuz risk premium) remains on-thesis and, if anything, strengthens fundamentally. **This does not change the mechanical exit**: APA has now been off the core watchlist for two consecutive Fridays (dropped 8/28, still off 9/4) — TRADING-STRATEGY.md's two-week thesis-broken exit rule is due **today at market-open, to execute regardless of P&L**. This pre-market research workflow does not place trades — flagging for the market-open workflow to execute.
- **MNKD (satellite, held, -0.49% unrealized today: $4.06 → $4.04; +1.00% since entry):** Gemini's satellite section (complete) confirms MNKD is the only held/watched satellite name with a catalyst inside the 5-trading-day window: Cantor Global Healthcare Conference fireside chat, Thursday 9/10 (2 trading days out). Consistent with every prior investor-conference appearance logged for this name (Wells Fargo 9/4, upcoming H.C. Wainwright 9/15), this is a non-binary IR event, not a binary regulatory/trial readout — no gap-risk position-sizing trigger per TRADING-STRATEGY.md. One unverified WebSearch item surfaced a claim that MNKD "completed the acquisition of scPharmaceuticals Inc. in October 2026" — this is internally inconsistent (dated after today, 9/8/2026) and not independently corroborated by any other source this session; **not treated as confirmed, not actionable, flagged for a clean re-check next session** rather than assumed real or fabricated further. No adverse news found. Well clear of the -15% hard-cut.
- **RIGL (satellite, held, +0.00% unrealized today: $48.67 → $48.67; +16.32% since entry):** Gemini's satellite section confirms no catalyst inside the 5-trading-day window (next earnings ~11/3, estimated). No adverse news found via WebSearch. Continuing to approach (not yet at) the +25% satellite tighten threshold. Well clear of the -15% hard-cut.
- **EDGX / SGOV / SPHY (income, held):** No idiosyncratic ETF-level news found; not thesis-screened per TRADING-STRATEGY.md. Macro backdrop (elevated Fed-hike odds, rising yields) continues to favor SGOV's short-duration thesis and pressure SPHY's high-yield-credit exposure — same standing risk flag as prior sessions, not a new development. SPHY currently -0.86% unrealized, well inside its 5% trailing stop.

### New data points flagged for the next screen refresh (not actionable today)
- **RVTY (core watchlist, not held):** Upgraded to Zacks Rank #2 (Buy) on steady upward EPS-estimate revisions (FY26 consensus +2.1% over 3 months to $5.36/sh); also integrating Anthropic Claude AI into its Signals R&D software platform. Reinforces, doesn't change, its existing watchlist status.
- **CRWD (core watchlist, not held):** Surpassed $2B lifetime TCV with partner Optiv, reached in under half the time of the prior $1B milestone; also integrated Falcon with the Anthropic Claude Marketplace. Positive but not a new entry trigger by itself — momentum/FCF/rating recheck still required at the buy-side gate.
- **ABNB (core watchlist, not held):** Post-earnings volatility continues — Q4 revenue beat ($2.78B, 20th beat in 21 quarters) and strong Q1 guide, but stock fell ~4.5% after-hours on an EPS miss ($0.56 vs. $0.66 est., partly a one-time $90M tax item). Worth a fresh momentum check at the next live buy-side evaluation given the two-sided signal.
- **ZBRA (core watchlist, not held):** Raised FY non-GAAP EPS guide to $20.75-$21.25 and reaffirmed >=$1B FCF target on a record $1.56B Q2; flagged a $73M tariff-recovery benefit and ongoing memory-supply constraints as a growth cap. No change to prior positive treatment.
- **DASH (core watchlist, not held):** Flagged margin-risk exposure to the ongoing oil/gasoline price spike (driver-incentive cost pressure) — a real but modest headwind, not a fundamentals reversal.

### Core Trade Ideas (from current WATCHLIST.md core list)
None sourced this session. Today's Gemini/WebSearch pass surfaced no new adverse fundamentals for any of the 24 core watchlist tickers, but this pre-market workflow doesn't place trades regardless — a live momentum/FCF/rating recheck against the buy-side gate is required at market-open before any entry, and none of today's overnight items (RVTY upgrade, CRWD milestone, ABNB volatility, ZBRA guide raise) rises to a standalone entry signal without that recheck.

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
None. MNKD and RIGL (held) have no new entry signal — see Held-Position Thesis Check. CGEM, PLPC, SHIP, ZYME, GNK: Gemini's complete satellite section confirms no catalyst inside the 5-trading-day window for any of the five; no material overnight news for any. No satellite candidate qualifies for a new entry this session.

### Risk Factors
- **APA's mandatory two-week thesis-broken exit is due at today's (9/8) market-open, execute regardless of P&L** — off the core watchlist two consecutive Fridays (8/28, 9/4); this pre-market workflow only flags it, doesn't execute.
- Iran/Hormuz conflict remains live and unresolved despite an overnight report of a possible Iran-Oman temporary-corridor deal in its "final stages" — genuine uncertainty either direction (deal finalizes and oil eases, or talks stall/another strike occurs and oil extends its spike); Iran itself warned of continued attack risk near Oman even as the corridor talk circulates. Continues to be a real gap-risk driver for the whole book, not just energy names.
- Elevated Fed rate-hike risk into the 9/15-16 FOMC (~60% odds, per Gemini's synthesis) compounds with the weak-jobs/sticky-wages ADP print and sustained oil-driven inflation pressure — a genuine headwind for long-duration tech and, directly, for held SPHY's high-yield-credit exposure.
- No held position is near its hard-cut: APA +1.60%, MNKD +1.00%, RIGL +16.32%, all well clear of -7%/-15%; income sleeve stops all intact (SPHY -0.86%, well inside its 5% trail).
- Cash 20.22%, at/above the 20% floor — no sweep action needed this session.
- Gemini's report this session was missing dedicated sections 1-3 (market context, catalysts, held-position check) though section 4 (all watchlist tickers) came through complete — market-context and held-position gaps filled via WebSearch above.
- Unverified WebSearch item on MNKD (a claimed scPharmaceuticals acquisition dated after today) is internally inconsistent and not corroborated — flagged for re-check, not acted on.

### Decision
**HOLD.** No core or satellite trade sourced this session — valid per the Patience Rule. This is a research-only workflow and does not place trades regardless. Top items for today's market-open session: (1) **execute APA's mandatory thesis-broken exit regardless of P&L** — this is the session's one required mechanical action; (2) re-verify momentum/FCF/rating live at the buy-side gate before considering any new core entry (RVTY, CRWD, ABNB, ZBRA are the names with fresh overnight items worth a first look, not pre-approved); (3) monitor the Iran/Hormuz situation for whether the reported Oman-corridor deal actually finalizes (de-escalation) or stalls/breaks down further (continued spike risk) — a real fork, not a settled trend either way; (4) continue watching RIGL's approach toward its +25% satellite tighten threshold; (5) independently re-verify (or drop) the unconfirmed MNKD/scPharmaceuticals WebSearch item next session.

### Notification sent
None — no held position at/near a hard-cut, no satellite catalyst resolved negatively overnight, and while the Iran/Hormuz situation remains serious, last session's (9/7) email already covered the direct-military-escalation development; today's incremental item (a reported, unconfirmed de-escalation corridor deal still in "final stages") is not itself a fresh major escalation warranting a second alert per STEP 5's urgent-only bar. Silent per STEP 5 default.

### 9:30 AM Session Note
- **Decided: executed APA's mandatory core thesis-broken exit, sold 121 sh, +$202.19 (+3.92%) realized.** Off the core watchlist two consecutive Fridays (8/28, 9/4) per TRADING-STRATEGY.md's two-week rule — exit is rule-driven, not a reaction to APA's (favorable) price action. No new core or satellite entries this window — pre-market's Decision was HOLD and no fresh candidate cleared the buy-side gate; valid per the Patience Rule.
- Cash-floor sweep-back: freed cash swept $5,325.44 (53 sh) into SGOV, restoring cash to 20.26% of equity (from a post-exit high of 25.91%), combined SGOV stop updated to cover 246 whole shares.
- **Pattern worth flagging for Friday's review:** the APA sell order stalled mid-fill (partially filled 45/121, then made no further progress for several minutes) — a first for this account; every prior market order in the trade log filled instantly and completely. Also caught the `alpaca.sh orders` (open-orders list) endpoint returning stale status/filled_qty for that order while a direct per-order GET showed the correct partially_filled state — a second, independent data-quality issue. Resolved by canceling the stalled remainder and resubmitting for the balance (filled cleanly ~60s later). Worth watching whether this recurs on other symbols/order types before treating it as an isolated blip.
- Core exposure now 0% of equity (all 6 slots open) — rebuilding from fresh momentum/FCF/spread-qualified candidates is the top priority for the next session.

### 11 AM Session Note
- **Decided: HOLD, no action.** No "Approved Trades (verified)" section exists in today's RESEARCH-LOG (pre-market's Decision was HOLD; market-open sourced no new entry either) — per STEP 3, no buys placed. Core sits at 0 open positions (5 of 6 slots plus the one from APA's exit this morning open); satellite unchanged at 2/4 (MNKD, RIGL).
- Risk sweep: no core positions to check. Satellite — MNKD +7.00% ($4.00→$4.28) and RIGL +12.79% ($41.84→$47.19), both well clear of the -15% hard-cut and below the +25% tighten threshold; RIGL eased back from Friday's +16.32% mark, not a thesis concern. No stop tightened, none moved down. All 5 open positions (EDGX, MNKD, RIGL, SGOV, SPHY) confirmed carrying live GTC trailing stops via `alpaca.sh orders`, none missing.
- Thesis check: MNKD and RIGL both intact, nothing new since the 9:30 AM note; no satellite catalyst inside the 1-2-trading-day gap-risk window (MNKD's Cantor conference is Thu 9/10, still non-binary IR, 2 trading days out). 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- Cash 20.24% ($19,083.97 / $94,323.82 equity), at/above the 20% floor — no sweep needed.
- No sharp/unexplained intraday moves warranting a Gemini catalyst check — MNKD's and RIGL's moves are within normal single-name volatility for this book, not the "no obvious cause" pattern STEP 7 is for.

### 3 PM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section appeared in today's RESEARCH-LOG at any window (pre-market, 9:30am, 11am, 3pm) — pre-market's Decision was HOLD and no fresh candidate cleared the buy-side gate at market-open or 11am either. This week (Sep 8 start, first session since 9/4 given the Labor Day holiday) stays core 0/6, satellite 0/4 new trades — valid per the Patience Rule (APA's exit this morning was a rule-driven thesis-broken sell, not a new entry, so it doesn't count against the weekly cap).
- **Risk sweep:** no core positions to check (0 open, all 6 slots free since APA's exit). Satellite — MNKD +4.38% ($4.00→$4.175) and RIGL +14.08% ($41.84→$47.73), both well clear of the -15% hard-cut and below the +25% tighten threshold. All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep, not a new gap). No stop moved down.
- **Satellite catalyst gap-risk check:** MNKD's Cantor Global Healthcare Conference fireside chat is Thu 9/10 — 2 trading days out, inside the 1-2 trading-day window. Confirmed non-binary IR event (consistent with every prior investor-conference appearance logged for this name, e.g. Wells Fargo 9/4) — not a regulatory/trial-readout binary catalyst, so the binary-catalyst 5%-cap/max-loss-logged rule does not apply. Conviction intact (no adverse news since 11am). **Holding through is valid — stated explicitly here per STEP 4, not silently doing nothing.** RIGL has no catalyst inside the window (next earnings ~11/3, estimated).
- **Thesis check:** MNKD and RIGL both intact, nothing new since 11am. No core positions to check. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- **Stop tightening:** none — MNKD/RIGL both below the satellite +25% tighten threshold (RIGL closest at +14.08%, easing back from the intraday high). No core positions.
- **Cash floor:** 20.24% ($19,083.97 / $94,277.28 equity), at/above the 20% floor — no sweep needed.
- Risky positions closed today: none this window (APA's thesis-broken exit was executed at 9:30am market-open, already logged above — not a 3pm risk-sweep catch).
- Lesson: none new this window — same open items as the 9:30 AM note (rebuild core exposure, watch the stalled-order/stale-orders-list pattern for recurrence, verify the unconfirmed MNKD/scPharmaceuticals item).

## 2026-09-09 — Pre-market Research

### Account
- Equity: $94,351.50 | Cash: $19,083.81 (20.23%) | Buying power: $282,876 (margin-inflated by the standing 4x multiplier flag — no margin/leverage used or planned)
- Core exposure: $0.00 (0.00% — 0 open positions, all 6 slots free since APA's 9/8 thesis-broken exit) | Satellite exposure: $10,661.13 (11.30%: MNKD $5,260.95 / RIGL $5,400.18) | Income exposure: $64,606.56 (68.47%: EDGX $20,003.77 / SGOV $24,794.95 / SPHY $19,807.84)
- All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stop orders via `alpaca.sh orders`, none missing: MNKD 15%/$3.74 (hwm $4.40), RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.456 on 246 covered sh (hwm $100.48; the 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep, not a new gap). No auth errors on account/positions/orders calls.
- This week (Sep 8 start): core 0/6, satellite 0/4 new trades so far (APA's exit and the SGOV sweep were not new entries).

### Data-quality flag — Gemini output missing Section I (S&P futures/VIX)
The consolidated Gemini Deep Research call completed (exit 0) but the captured output opens at Section II (market-moving catalysts) — Section I (S&P 500 futures direction and VIX level) never arrived. Sections II-V are complete and not truncated mid-document. Per the data-quality guard, Section I below is filled via native WebSearch fallback, not fabricated from anything Gemini returned.

### Market Context
- **VIX:** confirmed close of **15.30 on 9/8** (up modestly from 15.12 on 9/7) — WebSearch fallback for Gemini's missing Section I. VIX futures trading elevated (~$16.60-16.73 range intraday 9/9) relative to spot, consistent with the market pricing in near-term event risk (FOMC 9/15-16, ongoing Hormuz conflict) rather than a level indicating acute panic yet.
- **Geopolitical — Iran/Strait of Hormuz:** Gemini's report frames this as a "severe escalation" with kinetic attacks on U.S. and Iranian vessels and a declared Iranian restricted zone forcing tanker rerouting. Independent WebSearch confirms this references the known 9/5-9/6 escalation (CENTCOM strikes on 3 IRGC-linked tankers 9/5; Iran's anti-ship ballistic missile launches at U.S. Navy vessels same day; IRGC's 9/6 claim of hitting an unmanned U.S. vessel, which the U.S. called "a total lie") — already covered by the 9/6/9/7 notification emails, not a fresh escalation since yesterday's research log entry. The Iran-Oman temporary shipping-corridor talks (Iran's FM spokesman said 9/7 a deal was "days away," 60-day interim corridor with separate inbound/outbound lanes) remain unresolved as of this morning — still "close," not finalized, same ambiguous-fork status as 9/8's note. Net: real, live, unresolved conflict: continues to be a background gap-risk driver for the whole book, but nothing materially new today beyond what prior sessions' emails already covered.
- **Economic calendar today:** China August PPI hotter than expected (+3.8% YoY vs. +3.6% consensus) — upstream-inflation-export risk, not directly actionable for this book. French industrial production badly missed (-0.4% MoM vs. +0.2% consensus) — Eurozone weakness, not a U.S.-book driver. U.S. ADP employment report (1:15pm GMT) and a 10-Year Treasury auction (6:01pm GMT) are today's key U.S. releases — a hot ADP print would reinforce hawkish Fed odds into the 9/15-16 FOMC, a genuine headwind for long-duration tech (relevant background for core rebuild candidates) and for held SPHY's high-yield-credit exposure; the auction's bid-to-cover/indirect-bidder share will signal duration demand amid rising yields.

### Held-Position Thesis Check (Gemini Section III, complete)
- **MNKD (satellite, held, +8.25% unrealized: $4.00 → $4.33):** No thesis-breaking news. Mild pre-market weakness noted in the report ($3.94 print) ahead of tomorrow's (9/10) Cantor Global Healthcare Conference fireside chat — confirmed non-binary IR event, consistent with every prior conference appearance for this name. Newly disclosed 5.83% passive stake from Rubric Capital Management — informational, not a catalyst. No adverse news.
- **RIGL (satellite, held, +13.22% unrealized: $41.84 → $47.37):** No adverse news. Actively presenting at Citi's Biopharma Back to School Conference today (9/9), ahead of Cantor (9/10) and H.C. Wainwright (9/15) — all non-binary IR events. TAVALISSE commercial execution and R289 Phase 1b MDS enrollment cited as ongoing value drivers, unchanged from prior weeks. Continuing to approach (not yet at) the +25% satellite tighten threshold.
- **EDGX (income, held, +0.13%):** Thesis intact. Ex-dividend 9/8 paid $0.0466/share as scheduled. Note: heavy overnight coverage of an unrelated "Cboe EDGX" 23x5-trading-hours SEC approval — same ticker acronym, different entity (a Cboe exchange venue, not this ETF) — flagged as a headline-confusion risk, not acted on.
- **SPHY (income, held, -0.94%):** No adverse credit-market or SPHY-specific news. Thesis remains subject to the standing macro caveat (elevated Fed-hike odds pressure high-yield credit), unchanged from prior sessions. Well inside its 5% trailing stop.
- **SGOV (income, held, +0.03%):** Thesis reinforced, not just intact — fixed-income ETF inflows hit a record $325B YTD with SGOV alone pulling $29B, and the ongoing oil/inflation-driven push in intermediate Treasury yields (19-month highs per the report) strengthens the case for SGOV's short-duration insulation. No stop-relevant news (a 5% move on SGOV would itself be a data-integrity flag, not expected).

### Core Trade Ideas (from current WATCHLIST.md core list)
None sourced as approved — this workflow does not place trades regardless, and every name below still requires a live momentum/FCF/rating recheck at the buy-side gate before any entry. Flagged for a first look at market-open given today's Gemini commentary (Section IV, complete):
- **DELL, CRWD** — both hit hard by the broader mega-cap-tech/high-growth-software rotation this week (DELL -7.63%, CRWD -6.57%, no company-specific negative news); DELL's ISG/AI-server backlog thesis and CRWD's ongoing Fal.Con 2026 conference (Falcon Platform integrated into the Anthropic Claude Marketplace, new Agentic SOC product) both reported as fundamentally unchanged. A rotation-driven pullback, not a thesis break — worth a fresh momentum check given the price move, not a buy signal by itself.
- **MPC, VLO, PSX** — no company-specific news, but the Hormuz-driven oil spike (Brent approaching $100, WTI ~$95) is a systemic tailwind for refining crack spreads across this whole cohort — reinforces, doesn't newly trigger, prior positive treatment.
- **DXCM** — standout: fresh 52-week highs (~$92.34) on continued institutional accumulation (13F filings from Danske Bank, Silvant Capital, Trillium) and CONNECT trial data expanding the G7 CGM system's addressable market. Strongest positive datapoint of today's core scan — still requires the live buy-side gate check, not pre-approved.
- **NTAP** — showing relative strength (+0.11%) against the tech-sector rotation, consistent with its existing FCF-decline-flagged-but-fundamentally-sound treatment.
- No adverse news for any other core watchlist name (HUM, CRL, VEEV, ZBRA, IQV, HOOD, WDAY, ABNB, STT, CRM, CNC, RVTY, INCY, MRK, ADP, TRV) — healthcare/defensive names noted as benefiting from a rotation out of rate-sensitive tech.

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
None. MNKD and RIGL (held) — see Held-Position Thesis Check; both confirmed within the 5-trading-day catalyst window (non-binary IR conferences, no sizing-rule trigger). CGEM, ZYME — no documented catalyst inside the window, no adverse news. PLPC — no catalyst inside the window (next earnings late Oct/early Nov), shares elevated near $398 on continued Q2/Delta Star integration strength, no new entry signal. SHIP — no catalyst inside the window (ex-date 9/25, earnings early Nov); reported a dividend increase to $0.35/share (7.4% forward yield) on strong Q2, reinforces not newly triggers prior treatment. GNK — no catalyst inside the window (next dividend declaration ~early Nov per WATCHLIST); Diana Shipping's hostile takeover bid **confirmed formally withdrawn** (consistent with the 9/4 refresh's already-corrected catalyst framing), removing an M&A overhang — informational, not a new entry trigger. No satellite candidate qualifies for a new entry this session.

### Risk Factors
- Iran/Hormuz conflict remains live and unresolved — real, ongoing gap-risk driver for the whole book; the reported Iran-Oman corridor deal is still "close," not finalized, an unresolved fork in either direction. Nothing materially new since yesterday's log entry.
- Elevated Fed rate-hike odds into the 9/15-16 FOMC, compounded by hot Chinese PPI and today's ADP report/10-Year auction — a genuine headwind for long-duration tech (relevant to any core rebuild candidate) and for held SPHY's high-yield-credit exposure; a continuing tailwind for held SGOV.
- Tech-sector rotation is pressuring core-watchlist names like DELL/CRWD on valuation grounds, not fundamentals — a real price move to note before any live buy-side check, not a thesis break.
- No held position is near its hard-cut: MNKD +8.25%, RIGL +13.22% (both vs. -15% satellite cut), EDGX +0.13%, SPHY -0.94%, SGOV +0.03% (all vs. -5% income cut).
- Cash 20.23%, at/above the 20% floor — no sweep action needed this session.
- Gemini's report this session was missing Section I (S&P futures/VIX) though Sections II-V came through complete — filled via WebSearch above.

### Decision
**HOLD.** No core or satellite trade sourced this session — valid per the Patience Rule; core exposure remains at 0% (all 6 slots open) and rebuilding it from a fresh momentum/FCF/rating-qualified candidate stays the top priority, but this research-only workflow does not place trades regardless. Top items for today's market-open session: (1) re-verify momentum/FCF/rating live at the buy-side gate for DELL, CRWD, DXCM, NTAP, MPC/VLO/PSX before considering any entry — none are pre-approved; (2) monitor whether the Iran-Oman Hormuz corridor deal finalizes (de-escalation) or the talks stall/another strike occurs (continued spike risk); (3) continue watching RIGL's approach toward its +25% satellite tighten threshold; (4) watch today's ADP print and 10-Year auction results for their read-through to the 9/15-16 FOMC odds.

### Notification sent
None — no held position at/near a hard-cut, no satellite catalyst resolved negatively overnight, and the Iran/Hormuz situation, while still serious and unresolved, is not a fresh escalation beyond what the 9/6/9/7 emails already covered (today's Gemini framing describes the same known 9/5-9/6 events, independently confirmed via WebSearch, not a new development). Silent per STEP 5 default.

### 9:30 AM Session Note
- **Decided: HOLD, no core or satellite trade.** Live-rechecked all 7 names pre-market flagged for a "first look" (DELL, CRWD, DXCM, NTAP, MPC, VLO, PSX) against live Alpaca quotes ~3 min after the open: 6 of 7 failed the bid/ask spread check outright (DELL ~10.4%, CRWD ~10.3%, MPC ~11.0%, VLO ~10.5%, PSX ~6.8%, DXCM ~5.5% — all wide, same thin-opening-auction-liquidity pattern as WDC/STX/AMAT on 7/24). NTAP alone cleared the momentum gate on fresh Alpaca bars (3M rel. +10.70pp, 6M rel. +82.05pp vs SPY, above both 50-day ($179.43) and 200-day ($131.03) MA, its already-verified 22.4% 05-29 move excepted) and the fundamentals bar (Hold/no-Sell rating per the 09-04 screen; FCF decline remains the documented deliberate-reinvestment flag, not an exclusion) — but its own live quote came back with a wide ~6.65% spread (bid $179.55/ask $191.90) that did not move or refresh across two rechecks ~60s apart (identical timestamp both times) — treated as a data-quality/stale-quote flag, not a tradeable liquid market, and skipped on the same wide-spread basis as the other six. No core buy this session.
- **Data-quality pattern worth flagging for Friday's review:** this is a second, independent stale/non-refreshing-data incident on this account within 2 sessions — 9/8's note flagged `alpaca.sh orders` returning stale filled_qty on a partial fill; today a `quote` call returned an identical timestamp+bid+ask across calls ~60s apart during active market hours. Worth checking whether this is a market-data-feed lag specific to post-open illiquid names, or something to escalate/watch more broadly before trusting a marginal spread read at market-open again.

### 11 AM Session Note
- **Decided: HOLD, no core or satellite trade.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market and 9:30am both HOLD) — per STEP 3 this window trades from that list only, so no buys placed. This week (Sep 8 start) stays core 0/6, satellite 0/4 new trades.
- **Risk sweep:** no position at/beyond its sleeve hard-cut — core has 0 open positions; satellite MNKD +0.13% (down from pre-market's +8.25% on a ~3.7% intraday pullback) and RIGL +13.2%, both vs. -15% satellite cut. All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial dust flagged every prior sweep). No stop moved down.
- **No tightening this window** — RIGL still below the satellite +25% threshold (continuing to approach); MNKD well below. Thesis check: MNKD/RIGL both intact, no adverse news since the 9:30am note — MNKD's intraday pullback is consistent with the pre-market-flagged mild weakness ahead of tomorrow's (9/10) Cantor conference (confirmed non-binary IR event), not a thesis break. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- Cash 20.33% ($19,083.81 of $93,895.66 equity), at/above the 20% floor — no sweep needed. No Gemini call this window (MNKD's pullback has an established, non-adverse explanation; nothing else moved sharply without cause).
- This week (Sep 8 start): core 0/6, satellite 0/4 new trades — unchanged, valid per the Patience Rule. Core exposure stays at 0% (all 6 slots open); satellite unchanged (MNKD, RIGL held, no gate-triggering event this window). No position changes, no stop changes.

### 3 PM Session Note
- **Decided: HOLD, no core or satellite trade.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market, 9:30am, 11am all HOLD) — per STEP 3 no buy source existed, so no buys placed this window either. This week (Sep 8 start) closes at core 0/6, satellite 0/4 new trades — valid per the Patience Rule.
- **Risk sweep:** core has 0 open positions, nothing to check. Satellite — MNKD +0.13% ($4.00 -> $4.005), RIGL +12.79% ($41.84 -> $47.19), both well clear of the -15% satellite hard-cut. Income — EDGX -0.49%, SPHY -1.05%, SGOV +0.02%, all well inside the 5% income stop. All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep, not a new gap). No stop moved down.
- **Thesis check (fresh WebSearch, since prices barely moved since 11am):** MNKD — no adverse news; only item is tomorrow's (9/10) Cantor Global Healthcare Conference, confirmed non-binary IR appearance, consistent with every prior conference for this name. RIGL — no adverse news; CFO presenting today at Citi's Biopharma Back to School Conference (one-on-one meetings), confirmed non-binary IR, plus the already-known Cantor (9/10)/H.C. Wainwright (9/15) appearances. Both theses intact, nothing closed.
- **Gap-risk check:** MNKD's Cantor conference is 1 trading day out (inside the 1-2-day window) but confirmed non-binary IR, so the binary-catalyst sizing rule doesn't apply — conviction intact, holding through is valid, stated explicitly per STEP 4. RIGL has no binary catalyst inside the window.
- **No stop tightening this window** — RIGL (+12.79%) still below the satellite +25% threshold; MNKD not in meaningful gain. Income sleeve has no gain-based tightening schedule.
- 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- Cash 20.35% ($19,083.81 of $93,789.37 equity), at/above the 20% floor — no sweep needed (no trade this window).
- **Risky positions closed today: none.**
- This week (Sep 8 start): core 0/6, satellite 0/4 new trades — final for the week so far, valid per the Patience Rule. Core exposure stays at 0% (all 6 slots open, top priority remains rebuilding core exposure from a fresh momentum/FCF/rating-qualified candidate); satellite unchanged (MNKD, RIGL, 2 of 4 slots). Tomorrow: re-verify momentum/FCF/rating live at the buy-side gate for any core candidate (DELL, CRWD, DXCM, NTAP, MPC/VLO/PSX flagged pre-market, all skipped 9:30am on wide bid/ask spreads — worth a fresh liquidity check after the open settles); MNKD's Cantor conference (9/10) is the top event to monitor (non-binary, no sizing action expected); continue watching RIGL's approach toward its +25% satellite tighten threshold; monitor the Iran/Hormuz corridor-deal situation and today's ADP/10-Year auction read-through into the 9/15-16 FOMC.

## 2026-09-10 — Pre-market Research

### Account
- Equity: $93,769.43 | Cash: $19,083.81 (20.36%) | Buying power: $281,615.57 (margin-inflated by the standing 4x multiplier flag — no margin/leverage used or planned)
- Core exposure: $0.00 (0.00% — 0 open positions, all 6 slots free since APA's 9/8 thesis-broken exit) | Satellite exposure: $10,182.33 (10.86%: MNKD $4,799.25 / RIGL $5,383.08) | Income exposure: $64,503.29 (68.79%: EDGX $19,925.51 / SGOV $24,795.62 / SPHY $19,782.16)
- Cash 20.36%, at/above the 20% floor — no sweep needed.
- All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stop orders via `alpaca.sh orders`, none missing: MNKD 15%/$3.74 (hwm $4.40), RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.4655 on 246 covered sh (hwm $100.49; the 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep). No auth errors on account/positions/orders calls.
- This week (Sep 8 start): core 0/6, satellite 0/4 new trades so far.

### Data-quality notes
Gemini's consolidated report this session was complete — all 4 requested sections (market context, catalysts, held-position check, watchlist overnight news + satellite catalyst-proximity table) arrived, no truncation. Two items still needed independent verification per the standing data-quality/pending-catalyst guards rather than being taken at face value:
- **Geopolitical framing:** Gemini's report described "military strikes... specifically involving the United States and Iran around the Persian Gulf and Larak Island" as if newly overnight. WebSearch cross-check found the Larak Island strike exchange itself dates to 8/30-8/31 (already resolved, old news), but confirms a **genuinely fresh escalation dated 9/8-9/9**: the US military destroyed five Iranian tankers overnight in response to an Iranian ballistic-missile attempt on a US Navy warship, and Brent crude broke above $100/bbl for the first time since July on this news (confirmed via CNBC, Bloomberg, Yahoo Finance, all dated 9/8-9/9). VIX confirmed closed 16.31 on 9/9 (+3.75%), consistent with Gemini's reported 16.36 pre-market read. Used the WebSearch-verified framing below, not Gemini's conflated date.
- **CGEM catalyst discrepancy:** Gemini's report surfaced a Sept 14, 2026 IASLC World Conference on Lung Cancer Presidential Symposium 2 presentation of zipalertinib's Phase 3 REZILIENT3 data — materially different from WATCHLIST.md's currently-documented catalyst ("zipalertinib NDA FDA-accepted, target action Feb 27, 2027, not imminent," last confirmed 9/4). Independently verified via WebSearch (BioSpace, StockTitan, Investing.com press coverage): **real and confirmed** — REZILIENT3 already met its primary PFS endpoint (announced via press release), and the detailed data presentation is scheduled Monday 9/14/2026, 8am, at WCLC in Seoul (Dr. Daniel Tan Shao Weng presenting). This is 4 trading days out — inside the 5-trading-day satellite catalyst window. Note this is **not a blind binary pass/fail** — the topline (trial met its primary endpoint) is already public; 9/14 is a magnitude/safety-detail readout, which still carries volatility risk but a different risk profile than an unresolved FDA decision. CGEM is watchlist-only, not currently held — flagging for the next screen refresh to update its documented catalyst; not actionable via this research-only workflow regardless.

### Market Context
- **VIX:** confirmed 16.31 close 9/9 (+3.75%), pre-market reading ~16.36 — a real but modest uptick from 15.30 (9/8) and 15.12 (9/7), still below acute-panic levels.
- **S&P 500 futures:** per Gemini, +0.22% to ~7,653 pre-market, a tentative bounce after Wednesday's S&P 500 close of -0.48%; Nasdaq 100 futures roughly flat (semis weak, hyperscalers firm), Dow futures +0.35%. Not independently re-verified (non-critical secondary color) but consistent with the broader risk-off-but-not-panicked tone below.
- **Brent crude — confirmed >$100/bbl, first time since July** (WebSearch: CNBC, Bloomberg, Yahoo Finance, all dated 9/8-9/9), driven by the fresh US-Iran escalation described above (five Iranian tankers destroyed overnight in response to an Iranian ballistic-missile attempt on a US Navy warship). This is materially beyond what the 9/8/9/9 research-log entries had priced in (Brent was ~$97, WTI ~$95 as of 9/9's pre-market note) — a genuine, fresh multi-month high directly tied to escalating conflict, not a repeat of the already-covered 9/5-9/7 events.
- **Rates:** 10-Year Treasury yield ~4.859%, 30-Year ~5.309% (per Gemini, not independently re-verified) — a hostile backdrop for long-duration assets; Treasury's $6B long-bond buyback reportedly failed to stabilize the move. A $22B 30-year auction is scheduled this afternoon — a weak bid-to-cover could extend the equity selloff.
- **Economic calendar:** August PPI due this morning (consensus +0.4% MoM headline, +0.2% core) — the direct precursor to Friday's CPI print; a hot energy-driven PPI would reinforce hawkish Fed odds into the 9/15-16 FOMC. ECB meets today with a ~25bp hike priced in, against European gas prices at their highest since late 2022.
- **Corporate:** Apple hardware event today (iPhone 18 Pro, reportedly a foldable design plus Gemini-model AI integration) — not a portfolio-relevant name. Oracle and Adobe report earnings after today's close — a read-through for software/AI-monetization sentiment relevant to core watchlist software names (CRM, WDAY, CRWD, DELL).

### Held-Position Thesis Check
- **MNKD (satellite, held, -1.25% unrealized: $4.00 → $3.95):** No adverse news, thesis intact. Insulated from the broader tech/rate-driven volatility as a commercial-stage biotech with an established revenue stream (Tyvaso DPI ramp).
- **RIGL (satellite, held, +12.86% unrealized: $41.84 → $47.22):** Thesis intact and reinforced. CFO Dean Schorno presenting at the Cantor Global Healthcare Conference today, 9/10, 11:30am ET — confirmed **non-binary IR event**, consistent with every prior conference appearance for this name (Wells Fargo 9/4, Citi 9/9). Market will watch for forward color on VEPPANU commercial ramp and the IRAK1/4/PROTAC pipeline; no gap-risk sizing rule applies. No catalyst date change from prior sessions.
- **EDGX (income, held, -0.26% unrealized):** No thesis break. Gemini's writeup again conflated this ticker with the unrelated Cboe EDGX exchange venue (same acronym, different entity) — same headline-confusion risk flagged 9/9, not acted on. No idiosyncratic adverse news for the actual Global X Income Edge ETF holding.
- **SGOV (income, held, +0.03% unrealized):** Thesis reinforced — the fresh flight-to-safety impulse from the oil/geopolitical shock and ongoing long-duration bond-market stress both favor SGOV's zero-duration positioning. No stop-relevant news.
- **SPHY (income, held, -1.07% unrealized):** Thesis intact but the standing macro watch-item is sharper today: a sustained oil-driven inflation shock forcing the Fed to hold rates higher for longer would pressure high-yield credit spreads. Not yet a break — well inside its 5% trailing stop — but worth monitoring the ICE BofA HY OAS spread if the Brent move extends.

### Core Trade Ideas (from current WATCHLIST.md core list)
None sourced as approved — this workflow does not place trades regardless, and every name still requires a live momentum/FCF/rating recheck at the buy-side gate before any entry. Notable overnight items for a first look at market-open: **MRK** (HSBC upgraded to Buy overnight); **DXCM** (pulled back from 52-week highs on Executive Chair Kevin Sayer's 10b5-1 share sale, offset by institutional accumulation — a two-sided signal, not a new trigger by itself); **ADP** (new AWS partnership to accelerate AI in its HCM platform — incrementally positive); **HOOD** (Goldman Sachs raised PT to $142, Buy maintained); **DASH** (lost a Tractor Supply/Petsense same-day-delivery contract to Instacart — a minor competitive item, not a thesis break); **MPC/VLO/PSX** (direct tailwind from Brent's break above $100 — crack-spread upside, reinforces prior positive treatment, not a new signal). No adverse news for any other core watchlist name.

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
None. MNKD and RIGL (held) — see Held-Position Thesis Check, no new entry signal. **CGEM** — see Data-quality notes above: a real, confirmed, near-dated catalyst (9/14 WCLC data presentation, 4 trading days out) not yet reflected in WATCHLIST.md's stale Feb-2027 framing, but this pre-market workflow doesn't source new entries outside the standing watchlist notes and CGEM would still need a full live buy-side-gate pass (momentum, YoY growth/analyst rating, catalyst documentation) before any entry could be considered — flagging for market-open's awareness and the next screen refresh, not sourcing as an approved idea. PLPC, SHIP, ZYME, GNK — no catalyst inside the 5-trading-day window, no adverse news.

### Risk Factors
- **Genuine fresh geopolitical escalation:** US destroyed five Iranian tankers overnight after an Iranian ballistic-missile attempt on a US Navy warship; Brent crude broke above $100/bbl for the first time since July as a direct result. This is beyond what the 9/6-9/9 log entries and emails already covered — see Notification sent below.
- Elevated Fed rate-hike risk into the 9/15-16 FOMC, now compounded by an oil-driven inflation shock ahead of this morning's PPI and Friday's CPI — a headwind for long-duration tech (any core rebuild candidate) and for held SPHY's high-yield-credit exposure; a tailwind for held SGOV.
- $22B 30-year Treasury auction this afternoon — a weak bid-to-cover could extend the bond-market rout and pressure equities further.
- No held position is near its hard-cut: MNKD -1.25%, RIGL +12.86% (both vs. -15% satellite cut), EDGX -0.26%, SGOV +0.03%, SPHY -1.07% (all vs. -5% income cut).
- Cash 20.36%, at/above the 20% floor — no sweep action needed this session.
- CGEM's near-dated (9/14) satellite catalyst is confirmed real but not yet reflected in WATCHLIST.md — flagged for the next screen refresh, not actionable today.
- **SendGrid credential failure:** `scripts/sendgrid.sh` returned an HTTP 401 (Unauthorized) on send — not a missing-credential case (all SENDGRID_* vars are present per this session's STEP 0 check), the API key itself appears invalid/expired. The urgent alert below was written to `DAILY-SUMMARY.md`'s local fallback format by hand since the script's automatic fallback only triggers on missing vars, not on a live send failure. **Owner action needed:** verify/rotate the SendGrid API key — email notifications are currently non-functional.

### Decision
**HOLD.** No core or satellite trade sourced this session — valid per the Patience Rule; core exposure remains at 0% (all 6 slots open) and rebuilding it from a fresh momentum/FCF/rating-qualified candidate stays the top priority, but this research-only workflow does not place trades regardless. Top items for today's market-open session: (1) re-verify momentum/FCF/rating live at the buy-side gate for DELL, CRWD, DXCM, NTAP, MPC/VLO/PSX before considering any entry — none are pre-approved; (2) monitor whether the US-Iran conflict continues to escalate (fresh tanker/warship exchange overnight) or shows signs of the previously-reported Oman-corridor de-escalation talks resuming; (3) continue watching RIGL's approach toward its +25% satellite tighten threshold, and note today's Cantor conference presentation (non-binary); (4) be aware CGEM's confirmed 9/14 catalyst for the next screen refresh; (5) escalate the SendGrid 401 to the owner — notifications are not being delivered.

### Notification sent
**Sent** (via local fallback after a live SendGrid failure — see Risk Factors) — qualifies as a "major geopolitical event" under STEP 5's urgent bar: a fresh US-Iran military exchange overnight (five Iranian tankers destroyed after an Iranian missile attempt on a US Navy warship) pushed Brent above $100/bbl for the first time since July, materially beyond what the 9/6-9/9 notifications already covered. Message: "Pre-market alert 2026-09-10: Brent crude broke 100 dollars/bbl (first time since July) on escalating US-Iran military strikes near the Strait of Hormuz; VIX up to 16.36. No held position (MNKD, RIGL, EDGX, SGOV, SPHY) is near its hard-cut. Core sleeve remains at 0 open positions. No trade action taken; HOLD per Patience Rule." Delivery mechanism failed (SendGrid 401) — content preserved in `DAILY-SUMMARY.md` and this log entry; owner should be notified of the escalation and the broken email pipeline through this session's own notification channel.

### 9:30 AM Session Note
- Re-checked the 7 core names pre-market flagged for a live look (DELL, CRWD, DXCM, NTAP, MPC, VLO, PSX) — all on the current core watchlist, all cleared momentum/FCF/rating in the 9/4 screen. Every one showed a wide/unstable bid-ask spread at the open (DELL ~11.0%, CRWD ~5.3%, DXCM ~5.4%, MPC ~4.8%, VLO ~10.3%, PSX ~10.4%); NTAP looked briefly tight (0.99%) but widened to 4.14% on a second quote pulled seconds later — treated as noisy/stale opening-auction liquidity, not a real tradeable spread. All 7 skipped per STEP 2's wide/illiquid-spread rule; no core buy-side gate reached. No satellite candidates were sourced pre-market either. Buy side HOLD this window — core 0/6, satellite 0/4 new trades this week, valid per the Patience Rule.
- Risk re-check: account equity $93,554.40, cash $19,083.81 (20.4%, at/above the floor) — no sweep needed. All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing; no core positions open (0 of 6 slots).
- Pattern worth flagging for Friday's review: this is the same 7-name group that failed the identical wide-spread check at 9:30am on 9/9 — two sessions running now. Worth checking whether an 11am/later-window requote is consistently tighter, or whether this points to a feed-specific issue in the first few minutes after the open rather than genuine illiquidity in these large, normally tight-spread names.

### 11 AM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG — per STEP 3 this window trades from that list only, so no buys placed. This week (Sep 8 start) stays at core 0/6, satellite 0/4 new trades.
- **Account/positions/stops, live via `alpaca.sh account`/`positions`/`orders`:** Equity $93,412.85, cash $19,083.81 (20.43%, at/above the 20% floor — no sweep needed). No core positions (0/6 slots, flat since APA's 9/8 thesis-broken exit). Satellite: MNKD 1215 sh, -4.13% unrealized ($4.00 → $3.835); RIGL 114 sh, +11.90% unrealized ($41.84 → $46.82). Income: EDGX -0.86% ($26.78 → $26.5501), SGOV +0.03%, SPHY -1.33% ($23.36 → $23.0499). All 5 open positions confirmed carrying live GTC trailing stops via `alpaca.sh orders`, none missing: MNKD 15%/$3.74 (hwm $4.40), RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.475 on 246 covered sh (hwm $100.50; the 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep).
- **Cuts:** none. No core positions to evaluate. MNKD -4.13% and RIGL +11.90% both well clear of the satellite -15% hard-cut.
- **Stop tightening:** none. RIGL (+11.90%) remains below the satellite +25% tighten threshold (continuing to approach it — watch). MNKD is underwater, no gain-based tightening applies. Income sleeve has no gain-based schedule. No stop moved down.
- **Thesis check (fresh WebSearch on both held satellite names):** MNKD — no adverse news; today's Cantor Global Healthcare Conference fireside chat (11:30am ET) independently confirmed as a standard IR presentation, consistent with every prior conference appearance for this name — non-binary, no gap-risk sizing rule triggered. RIGL — no adverse news; CFO Dean Schorno's Cantor conference appearance today also confirmed non-binary IR, same treatment. Both theses intact, nothing closed. EDGX/SGOV/SPHY — no idiosyncratic news, no thesis to break.
- **Step 7 (optional Gemini check):** skipped — no held position moved sharply without cause (MNKD -2.67% and RIGL -0.85% intraday are both normal drift within their trailing-stop bands, consistent with the broader tape given this morning's oil/geopolitical-driven risk-off tone already logged pre-market).
- **2-strike sub-sector tracker unchanged:** biotech 1/2 (OCUL 7/23), industrials 0/2.
- No action taken this window — zero trades, zero cuts, zero tightens, zero stop moves.
- SendGrid still broken (401, flagged pre-market) — no notification sent this window since no trade fired; owner action to rotate the key remains outstanding.

### 3 PM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market, 9:30am, 11am, 3pm all HOLD) — per STEP 3 no buy source existed, so no buys placed this window. This week (Sep 8 start) closes at core 0/6, satellite 0/4 new trades — valid per the Patience Rule.
- **Risk sweep:** no core positions to check (0/6 slots). Satellite: MNKD -5.75% and RIGL +11.76%, both well clear of the -15% hard-cut. Income: EDGX -0.78%, SPHY -1.52%, SGOV +0.03%, all well inside their 5% stops. All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial dust flagged every prior sweep). No stop moved down. MNKD's Cantor Global Healthcare Conference (11:30am ET) resolved during the session as a standard non-binary IR appearance, consistent with prior conferences — no adverse news, no gap-risk sizing action triggered. Thesis check: MNKD/RIGL both intact.
- **No stop tightening this window** — RIGL (+11.76%) remains below the satellite +25% threshold; MNKD is underwater, no gain-based tightening applies; Income sleeve has no gain-based schedule.
- **2-strike sub-sector tracker unchanged:** biotech 1/2 (OCUL 7/23), industrials 0/2. **Risky positions closed today: none.** SendGrid still returned 401 Unauthorized on the 3pm EOD send (same failure flagged pre-market) — content preserved in `DAILY-SUMMARY.md`'s local fallback per the standing pattern; owner action to rotate the key remains outstanding.

## 2026-09-11 — Pre-market Research

### Account
- Equity: $93,391.54 | Cash: $19,083.81 (20.43%) | Buying power: $280,713.01 (margin-inflated by the standing 4x multiplier flag — no margin/leverage used or planned)
- Core exposure: $0.00 (0.00% — 0 open positions, all 6 slots free since APA's 9/8 thesis-broken exit) | Satellite exposure: $9,993.63 (10.70%: MNKD $4,604.85 / RIGL $5,388.78) | Income exposure: $64,314.10 (68.86%: EDGX $19,826.22 / SGOV $24,799.88 / SPHY $19,688.00)
- Cash 20.43%, at/above the 20% floor — no sweep needed.
- All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stop orders via `alpaca.sh orders`, none missing: MNKD 15%/$3.74 (hwm $4.40), RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.475 on 246 covered sh (hwm $100.50; the 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep, not a new gap). No auth errors on account/positions/orders calls.
- This week (Sep 8 start, 4 trading days Tue-Fri): core 0/6, satellite 0/4 new trades — today is the last session of the week.

### Data-quality notes
The single consolidated Gemini Deep Research call returned but **started mid-document** — it opens directly at "### Enterprise Technology, Cybersecurity, and Hardware," with sections 1 (S&P 500 futures/VIX) and 2 (top market-moving catalysts/economic releases) entirely absent from the captured output. Per STEP 3's data-quality guard, no figure for those two sections was taken from Gemini — independently filled the gap via WebSearch instead:
- **VIX:** confirmed closed 16.46 on 9/10 (Yahoo Finance/Cboe).
- **S&P 500 futures:** +0.44% to ~7,630 pre-market; Dow futures +0.46% to ~52,350; Nasdaq 100 futures -0.45% to ~29,260 (FXStreet, dated 9/11 07:30 — mixed tape, cyclicals/value firm, tech soft) — consistent with Gemini's own (visible) note that software/SaaS names (CRWD, CRM, WDAY, VEEV) face Nasdaq-beta headwinds today.
- **Fed rate-hike odds:** ~70% for the 9/15-16 FOMC (down slightly from 72%), per CNBC/Polymarket coverage — in the same ballpark as the 66% figure Gemini's (visible) synthesis section cited; used the independently-sourced 70% as primary.
- **Key economic release today:** August CPI, released this morning — the last major data point before the 9/15-16 FOMC (confirmed via FXStreet/CNBC). This is the top scheduled catalyst missing from Gemini's absent section 2.

The rest of Gemini's report (held-position/watchlist overnight news, satellite catalyst-proximity table) read as complete and internally consistent, not truncated — no other gaps found.

### Market Context
- **VIX 16.46** (9/10 close, WebSearch-confirmed), **S&P 500 futures +0.44% (~7,630)** pre-market (WebSearch-confirmed, see Data-quality notes) — modest risk-on tone despite the oil shock, likely CPI-relief positioning.
- **Middle East / oil — continuing and, on fresh detail, more severe than previously logged:** Brent remains above $100/bbl. WebSearch turned up sharper specifics than the 9/9-9/10 entries captured: Iran conducted "the war's heaviest declared day of attacks on merchant shipping" (10 ships attacked near the Strait of Hormuz), Houthi strikes on Saudi Arabia wounded 70+ people and ignited fires at oil facilities, and shipping through the Strait of Hormuz (normally ~20% of world oil supply) is described as **nearly halted** — one source characterizes this as "the largest oil supply shock ever," 3-4x the scale of the 1973/1979 crises. This detail refines rather than replaces the 9/9-9/10 findings (same conflict, same >$100/bbl Brent level already known and already notified on 9/10) — not treated as a fresh discrete trigger for today's notification decision (see Notification below), but logged in full given the severity of the characterization.
- **Rates/CPI:** August CPI (the last major print before the 9/15-16 FOMC) releases this morning — the single most consequential scheduled event today; a hot print reinforces the ~70% hike-odds backdrop above, a cool one could unwind it quickly. No independent read of the actual number available pre-market (not yet released at research time).
- **Baltic Dry Index / dry-bulk shipping (relevant to satellite watchlist SHIP, GNK, not held):** BDI +13.9% in a week to 3,628, highest since Oct 2021, led by Capesize rates surging past $51-58k/day on Pacific typhoon disruption plus robust Atlantic demand — a strong tailwind for both watchlist names, not a new entry signal by itself (still needs a live buy-side-gate pass).

### Held-Position Thesis Check
- **MNKD (satellite, held, -5.25% unrealized: $4.00 → $3.79):** No adverse news, thesis intact. Not oil/rate-sensitive as a commercial-stage biotech.
- **RIGL (satellite, held, +12.98% unrealized: $41.84 → $47.27):** No adverse news, thesis intact. Continuing to approach (not yet at) the satellite +25% tighten threshold.
- **EDGX (income, held, -0.76% unrealized):** No idiosyncratic news, no thesis break.
- **SGOV (income, held, +0.05% unrealized):** Thesis reinforced — flight-to-safety and short-duration positioning both favored by the ongoing geopolitical/oil shock and elevated-rate backdrop.
- **SPHY (income, held, -1.54% unrealized):** Thesis intact, well inside its 5% stop. Standing watch item unchanged: a sustained oil-driven inflation shock forcing a higher-for-longer Fed would pressure high-yield credit spreads — worth watching the ICE BofA HY OAS spread if the escalation continues, not yet a break.

### Core Trade Ideas (from current WATCHLIST.md core list)
None sourced as approved — this workflow does not place trades regardless, and every name still requires a live momentum/FCF/rating recheck at the buy-side gate before any entry. Notable overnight items for a first look at market-open: **CRWD** (pre-market -1.45% on profit-taking ahead of its imminent earnings report plus a disclosed 10b5-1 insider sale by CEO George Kurtz — elevated ~180x forward-earnings valuation makes it rate-sensitive today; not a thesis break, fundamentals unchanged); **HUM** (weak after-hours on sector-wide medical-loss-ratio warnings from peer CVS at the Wells Fargo Healthcare Conference, compounding existing anxiety about HUM's 2027 Medicare Advantage plan exits — a sentiment overhang, not new fundamental deterioration); **IQV** (disclosed insider option-exercise/sale by an executive, offset by raised FY26 revenue guidance — two-sided, not a trigger); **MPC/VLO/PSX** (continued crack-spread tailwind from Brent above $100, reinforces prior positive treatment, not a new signal); **STT/ADP/TRV** (rate-sensitive, a tailwind if the CPI print firms up hike odds). No adverse or thesis-breaking news for any other core watchlist name (DELL, NTAP, VEEV, CRM, WDAY, CRL, RVTY, INCY, MRK, DXCM, ZBRA, HOOD, ABNB, DASH, CNC).

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
None. MNKD and RIGL (held) — see Held-Position Thesis Check, no new entry signal. **CGEM** — same discrepancy flagged in the 9/10 entry persists: WATCHLIST.md still documents "target action Feb 27, 2027, not imminent" (last updated at the 9/4 screen), but the Phase 3 REZILIENT3 interim-analysis data presentation at WCLC's Presidential Symposium is confirmed for Monday 9/14 (3 trading days out) — inside the 5-trading-day window. Not actionable via this research-only workflow; flagging again for the next screen refresh to update the documented catalyst. **ZYME** — conference appearances 9/14-15 (Morgan Stanley, H.C. Wainwright) are IR follow-through on the already-resolved 8/25 Ziihera approval, not a new binary event; consistent with WATCHLIST.md's existing "resolved, standard 7.5% cap" treatment, no change needed. **SHIP, GNK** — no catalyst inside the 5-trading-day window; both benefit from the Baltic Dry Index surge (see Market Context) but that's a sector tailwind, not a documented single-name catalyst event — no entry signal change. **PLPC** — no catalyst inside the window, no adverse news.

### Risk Factors
- **Middle East / oil supply shock, ongoing and severe** (see Market Context) — Strait of Hormuz shipping nearly halted, Brent >$100/bbl. Continuation of the story already notified on 9/10, not treated as a fresh trigger today (see Notification below), but the fresh detail (heaviest single day of shipping attacks, Saudi facility strikes with casualties) confirms this is not de-escalating.
- Elevated Fed rate-hike odds (~70%) into the 9/15-16 FOMC, with August CPI releasing this morning as the last major data point — a headwind for long-duration tech (any core rebuild candidate, and today's CRWD/CRM/WDAY/VEEV softness) and for held SPHY's high-yield-credit exposure; a tailwind for held SGOV and for rate-sensitive core names (STT, ADP, TRV).
- No held position is near its hard-cut: MNKD -5.25%, RIGL +12.98% (both vs. -15% satellite cut), EDGX -0.76%, SGOV +0.05%, SPHY -1.54% (all vs. -5% income cut).
- Cash 20.43%, at/above the 20% floor — no sweep action needed this session.
- CGEM's near-dated (9/14) satellite catalyst remains unreflected in WATCHLIST.md — flagged again for the next screen refresh, not actionable today.
- Gemini's report started mid-document (sections 1-2 missing) — filled via WebSearch, see Data-quality notes; no fabricated figures used.

### Decision
**HOLD.** No core or satellite trade sourced this session — valid per the Patience Rule; core exposure remains at 0% (all 6 slots open) and rebuilding it from a fresh momentum/FCF/rating-qualified candidate stays the top priority, but this research-only workflow does not place trades regardless. Top items for today's market-open session: (1) re-verify momentum/FCF/rating live at the buy-side gate for any core candidate before considering an entry — none pre-approved; (2) watch this morning's CPI print and its effect on Fed-hike odds and the Nasdaq-heavy software names (CRWD, CRM, WDAY, VEEV); (3) continue monitoring the Middle East/Hormuz situation for further escalation or any de-escalation signal; (4) continue watching RIGL's approach toward its +25% satellite tighten threshold; (5) be aware CGEM's confirmed 9/14 catalyst for the next screen refresh.

### Notification
**Not sent.** No held position is near its hard-cut, no satellite catalyst resolved negatively overnight, and while the Middle East/oil situation remains severe, it is a continuation of the story already notified on 9/10 (Brent breaking $100/bbl, fresh US-Iran exchange) rather than a new discrete trigger — today's WebSearch turned up sharper detail on the same ongoing conflict, not a new escalation event dated overnight 9/10→9/11. Per STEP 5, staying silent this session; will re-notify immediately if a position approaches its hard-cut, a satellite catalyst resolves negatively, or the conflict produces a genuinely new discrete escalation (e.g., a formal Hormuz closure declaration, direct attack on a portfolio-adjacent asset class beyond the already-priced oil/rate channel).

### 9:30 AM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG entry — per STEP 3 this window trades from that list only, so no buys placed. Checked live spreads on the 9 core names flagged pre-market for a first look (CRWD, HUM, IQV, MPC, VLO, PSX, STT, ADP, TRV): most remain wide/unstable at the open (CRWD ~9.7%, HUM ~6.7%, IQV ~13.1%, VLO ~9.4%, PSX ~5.5%, TRV ~12.0%), only MPC (~0.66%) and STT (~0.53%) quoted tight, ADP borderline (~2.9%) — moot regardless since no approved-trades list existed to act on. This week (Sep 8 start) stays at core 0/6, satellite 0/4 new trades; today is the week's last session.
- **Risk re-check:** account equity $93,548.27, cash $19,083.81 (20.40%, at/above the 20% floor) — no sweep needed. All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial ~$74 dust flagged every prior sweep, not a new gap). No core positions open (0 of 6 slots). MNKD +2.65% intraday, RIGL +1.24% intraday, both well clear of the -15% satellite hard-cut; no stop tightening triggered (RIGL still below +25%). No stop moved down.
- Pattern worth flagging for Friday's review: wide/unstable opening-auction spreads on large, normally-liquid core names have now persisted three sessions running (9/9, 9/10, 9/11), with the specific name set shifting slightly each day — worth checking at the next weekly review whether an 11am/later requote is consistently tighter, or whether this is a first-few-minutes feed artifact rather than genuine illiquidity.

### 11 AM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG entry — per STEP 3 no buys placed. This week (Sep 8 start, last session of the week) stays at core 0/6, satellite 0/4 new trades.
- **Risk re-check:** account equity $93,431.63, cash $19,083.81 (20.42%, at/above the 20% floor) — no sweep needed. All 5 open positions (MNKD, RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing (SGOV's 0.740468683-sh fractional remainder stays unstopped, same immaterial dust flagged every prior sweep). No core positions open (0 of 6 slots). Satellite: MNKD -5.38% (vs -15% cut, not close), RIGL +13.12% (vs -15% cut; below the +25% tighten threshold, no action). No stop moved down, no tightening triggered. No thesis break on either held satellite name — no adverse news since the 9:30am check; RIGL's post-VEPPANU-launch thesis and MNKD's post-FUROSCIX-approval thesis both intact. Nothing moved sharply with no obvious cause, so no ad-hoc Gemini call this window. 2-strike sub-sector tracker unchanged: biotech 1/2 (OCUL 7/23), industrials 0/2.
- No lesson pattern beyond the opening-spread item already flagged above.
- SendGrid still flagged broken (401, first caught 9/10) — no notification sent this window since no trade fired; owner action to rotate the key remains outstanding.

### 3 PM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section existed anywhere in today's RESEARCH-LOG entry — per STEP 3 no buys placed. This week (Sep 8 start) closes at core 0/6, satellite 0/4 new trades.
- **MNKD stopped out mechanically between 11am and 3pm** (15% GTC trailing stop filled 11:51am ET at $3.74, realized -$315.90/-6.50%) — no session caught it live since none ran in that window; no adverse news found, thesis was intact right up to the stop. This is the second consecutive biotech-sub-sector satellite loss (after OCUL 7/23, no intervening biotech close between them) — **triggers the 2-strike sub-sector cooldown: no new satellite biotech entries until 2026-09-25.** Industrials sub-sector unaffected (0 strikes).
- **Risk sweep:** RIGL (satellite, only remaining Core/Satellite position) +13.53% unrealized, well clear of the -15% hard-cut; below the +25% tighten threshold, no stop tightening. EDGX +0.17%, SPHY -1.50%, both well inside the 5% income stop. All 4 remaining open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh positions`/`orders` carrying live GTC trailing stops, none missing. No stop moved down. No core positions open (0 of 6 slots).
...
## 2026-09-13 — Pre-market Research

**Scheduling note:** today (Sunday 2026-09-13) is a non-trading day — confirmed via `date`. This is the **second consecutive non-trading-day firing** of this workflow (after Saturday 2026-09-12) — CLAUDE.md documents five scheduled runs per *trading* day, so two weekend fires in a row is a real scheduler-config item, not a one-off, and worth escalating to the owner rather than just re-flagging silently. Since this workflow is research-only (never places trades) there's no downside to running it; treating this entry as further prep for the next trading session (Mon 2026-09-14 pre-market/open), not as "today's" market open.
...
- Income exposure: $69,294.05 (74.14% — EDGX $19,953.71/-0.12%, SGOV $29,652.34/+0.05%, SPHY $19,688.00/-1.54%).
- All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.494 on 294 covered sh (hwm $100.52; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep, not a new gap).
- This week (Sep 8 start) closed Friday at core 0/6, satellite 0/4 new trades. New week (Sep 14 start) resets both counters to 0.
- Biotech satellite 2-strike cooldown remains ACTIVE through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries regardless of screen results. Industrials sub-sector unaffected (0 strikes).

### Data-quality flag — Gemini failed outright this session (new failure mode)
The single consolidated Gemini Deep Research call **never completed**: polled for the full 1200s (20 min) budget and returned `status=in_progress` at timeout, exiting code 4 with zero usable content — no truncated-but-partial report to salvage this time, unlike every prior week's pattern (mid-document truncation, citations-only, etc.). Per STEP 3, fell back entirely to native WebSearch for all four sections below. Nothing in this entry is sourced from Gemini.

### Market Context (WebSearch fallback)
- **VIX futures ~16.75-18.17 range** (Barchart, still-live quote as of Friday's close read-through) — no fear spike, consistent with Friday's post-CPI settle.
- **FOMC Wed 9/16, 2pm:** a hike is now described by CNBC as "seems certain" after the latest inflation data — odds have continued climbing through the week (August 7: ~44%; September 8: ~61%; by 9/9-9/11 various trackers 85-90%+). This is a genuine, escalating hawkish repricing, not a one-day blip. Headwind for core's long-duration/software cluster (CRM, WDAY, CRWD, VEEV) and held SPHY's high-yield credit; tailwind for held SGOV. SEP/dot-plot also lands same meeting.
- **Week-ahead calendar (CNBC, eOption):** Monday 9/14 has no major US economic data. Tuesday 9/15: ADP weekly employment, Empire State Index. Wednesday 9/16: Export/Import Price Index, Retail Sales, Business Inventories, NAHB Housing Index, plus the FOMC decision. H.C. Wainwright 28th Annual Global Investment Conference and Morgan Stanley 24th Annual Global Healthcare Conference both run 9/14-9/16 (relevant to RIGL's Tue 9/15 appearance, see below).
- **Middle East / Strait of Hormuz:** no fresh discrete escalation found for this specific weekend (9/12-9/13) beyond the same ongoing-conflict backdrop already notified 9/10 and carried in every session since — search results returned only older (August-and-earlier) escalation coverage, nothing new dated this weekend.

### Held-Position Thesis Check
- **RIGL (satellite, held, +12.90% unrealized as of Friday's close):** No adverse news. WebSearch confirms CFO Dean Schorno presents at the H.C. Wainwright 28th Annual Global Investment Conference **Tuesday 9/15, 10:00am ET** (not Monday as some earlier logs abbreviated — conference itself runs 9/14-9/16, his slot is 9/15) — same non-binary IR pattern as Citi 9/9 and Cantor 9/10. Thesis intact. Continuing to approach (not yet at) the satellite +25% tighten threshold.
- **EDGX (income, held, -0.12% unrealized):** No idiosyncratic news found.
- **SGOV (income, held, +0.05% unrealized):** No idiosyncratic news; thesis (flight-to-safety/short-duration) reinforced by the FOMC hike odds.
- **SPHY (income, held, -1.54% unrealized):** No idiosyncratic news; well inside its 5% stop. Standing watch item unchanged — a hawkish FOMC outcome 9/16 pressures high-yield credit spreads, not yet a break.

### Core Trade Ideas (from current WATCHLIST.md core list)
None sourced — Gemini's per-ticker coverage did not come through at all this session (see Data-quality flag), and given the full outright failure (not a partial truncation), no individual core-name WebSearch spot-checks were run beyond the macro/rate-headwind context above — the 24-name list is too broad to cover one-by-one via WebSearch inside this session's scope, and full live momentum/FCF/rating reverification at the buy-side gate is required before any entry regardless. No core name is pre-approved.

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
- **RIGL** (held) — see Held-Position Thesis Check, no new entry signal.
- **CGEM — important, not held, cannot be newly entered regardless (biotech cooldown active through 2026-09-25):** WebSearch confirms the Phase 3 REZILIENT3 trial (zipalertinib + chemotherapy vs. chemotherapy alone, first-line EGFR exon20ins NSCLC, n=285) **met its primary progression-free-survival endpoint** at the planned interim analysis. Data presented in Presidential Symposium 2 at IASLC WCLC 2026 (Seoul, conference runs 9/12-9/15) — consistent with the previously-logged Sunday 9/13 presentation date. Cullinan Therapeutics (developed with Taiho Oncology/Taiho Pharmaceutical) is hosting a virtual analyst/investor event **Monday 9/14** to discuss the data, as previously logged. **Market reaction is not yet observable** — US markets have been closed since Friday 9/11's close and CGEM hasn't traded since; nothing to report on price action until Monday's open. Recent analyst tone ahead of this readout: Wall Street Zen upgraded CGEM Sell→Hold 9/5; Wedbush raised PT $39→$42 (Outperform) 8/13 — both pre-date the actual data, so not a reaction to it. Flagging for Monday market-open awareness only (in case a large favorable move prompts owner questions) — not sourced as an approved idea, and not entering under any circumstance while the cooldown is active.
- **PLPC, SHIP, GNK** — no Gemini coverage this session (outright failure); no adverse news found via targeted WebSearch for the broader market-news queries run above (none of the three surfaced). Catalyst-proximity unchanged from 9/12's Gemini-confirmed read (PLPC: no scheduled events; SHIP: next catalyst Q3 earnings 11/12, nearest corporate action 9/25 ex-dividend; GNK: no scheduled catalyst) — not independently re-verified this session given the full Gemini failure, carried forward as the best available information.

### Risk Factors
- FOMC Wed 9/16 hike odds have continued climbing through the week and are now described as "seems certain" — headwind for core's long-duration/software cluster and held SPHY; tailwind for held SGOV. SEP/dot-plot also lands same meeting — a source of guidance-driven volatility even after the decision itself.
- Middle East/Hormuz conflict continues — no fresh discrete escalation found this specific weekend, same ongoing backdrop as prior sessions.
- **CGEM's Phase 3 REZILIENT3 data met its primary endpoint** (real, positive, near-dated) — but CGEM is not held and the biotech cooldown blocks any new entry regardless; watch Monday's market reaction for awareness only.
- No held position is near its hard-cut: RIGL +12.90% (vs -15% satellite cut), EDGX -0.12%, SGOV +0.05%, SPHY -1.54% (all vs -5% income cut).
- Cash 20.09%, at/above the 20% floor — no sweep action needed.
- **Gemini Deep Research failed outright this session** (20-min timeout, zero content) — a more severe failure mode than the truncation pattern logged nearly every week since late July. Combined with two consecutive weekend firings, this is worth flagging to the owner as an operational/infra reliability item, not just a data-quality footnote.
- SendGrid was flagged broken (401 Unauthorized) as of 9/10-9/11; not re-tested this session (no urgent trigger requires a real send — see Notification below) and not re-tested 9/12 either. Two sessions running with this unresolved — owner action to rotate the key remains outstanding.

### Decision
**HOLD.** No core or satellite trade sourced this session — valid per the Patience Rule; core exposure remains at 0% (all 6 slots open) and rebuilding it from a fresh momentum/FCF/rating-qualified candidate stays the top priority once Monday's live buy-side gate can be run, but this research-only workflow does not place trades regardless. Top items for Monday's pre-market/market-open sessions: (1) run a full live momentum/FCF/rating recheck at the buy-side gate for core watchlist names — this session's outright Gemini failure means Monday's pass has zero carry-forward per-ticker coverage from this weekend, not just a thinner one; (2) watch CGEM's market reaction to Sunday's positive Phase 3 data and Monday's investor call for awareness (informational only — cooldown blocks entry regardless); (3) FOMC (Wed 9/16) is the week's dominant scheduled event, with hike odds now near-certain — monitor positioning into it; (4) continue watching RIGL's approach toward its +25% satellite tighten threshold; (5) re-test SendGrid and escalate to the owner if still broken — two sessions unresolved now; (6) flag the two-consecutive-weekend-firing scheduler pattern to the owner.

### Notification
**Not sent via SendGrid** (per STEP 5 — silent unless urgent; no held position is near its hard-cut, no satellite catalyst resolved negatively for a held name, and the Middle East/oil situation is an unchanged continuation, not a fresh discrete escalation) and SendGrid's own status remains unverified/possibly still broken. A separate owner-facing notification was sent this session outside the SendGrid path to flag the scheduler and Gemini-reliability operational items — see session record.
...
### 9:30 AM Session Note
- **HOLD, no trades.** Pre-market's Decision already concluded no core or satellite candidate was approved this session (no "Approved Trades (verified)" list) — core watchlist re-verification was flagged as a to-do but never produces a pre-approved candidate list this workflow can act on live, and satellite's only non-held idea (CGEM) stays blocked by the active biotech 2-strike cooldown (through 2026-09-25) regardless of today's investor-event reaction. Live check: account $93,278.77 equity, cash $18,777.57 (20.13%, at/above the 20% floor, no sweep needed); positions unchanged from pre-market (RIGL satellite +12.9%, EDGX/SGOV/SPHY income) — core still 0/6, satellite 1/4. This week (Sep 14 start) opens at core 0/6, satellite 0/4 new trades.
- Pattern to flag for Friday's review: core sleeve has now sat at 0/6 open positions for 5 consecutive sessions (since APA's 9/8 exit) with no live buy-side gate run producing a candidate — the gap isn't the gate rejecting names, it's that the research workflow hasn't been surfacing a pre-approved core candidate list at all recently (Gemini truncation 3 sessions running). Worth checking at the weekly refresh whether the research→approved-list handoff needs a fallback path for when Gemini's per-ticker section doesn't come through.
- SendGrid still 401 (6th consecutive window unresolved) — no new information this window, already escalated pre-market.

### 11 AM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market, 9:30am both HOLD) — per STEP 3 no buys placed this window either. This week (Sep 14 start) stays core 0/6, satellite 0/4 new trades — valid per the Patience Rule.
- **No cuts, no tightens.** Live check: $93,053.97 equity, cash $18,777.57 (20.18%, at/above the 20% floor, no sweep needed). Core has 0 open positions (0/6 slots). Satellite: RIGL +8.22% unrealized ($41.84 -> $45.28, down from +12.9% at 9:30am on today's broader risk-off tape) — well clear of the -15% hard-cut and still below the +25% tighten threshold, no action. Income: EDGX -0.78%, SGOV +0.05%, SPHY -1.78%, all well inside the 5% stop. All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing, none moved down.
- Thesis check: RIGL intact, no adverse news since 9:30am; H.C. Wainwright conference presentation is tomorrow (9/15), not today — non-binary IR, no gap-risk sizing action due this window. Income positions have no thesis to break.
- RIGL's intraday pullback (-4.15%) reads as broad-market risk-off (S&P futures/cash -0.8%+, Nasdaq -1.8%+ on the oil shock + AI-pause chip selloff already flagged pre-market), not an idiosyncratic move — no ad hoc Gemini call warranted per STEP 7.
- 2-strike sub-sector tracker unchanged: biotech 2/2 cooldown active through 2026-09-25 (OCUL 7/23, MNKD 9/11); industrials 0/2.
- No account activity this window (no buys, sells, tightens, or sweeps). SendGrid still 401 (7th consecutive window) — no new information, already escalated; no notification sent this window since no action was taken (STEP 9).

### 3 PM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market, 9:30am, 11am all HOLD) — per STEP 3 no buys placed this window either. This week (Sep 14 start) closes at core 0/6, satellite 0/4 new trades — valid per the Patience Rule.
- **Risk sweep (STEP 4):** Live check via `alpaca.sh account`/`positions`/`orders`: $93,158.76 equity, cash $18,777.57 (20.16%, at/above the 20% floor, no sweep needed). Core has 0 open positions (0/6 slots) — nothing to check. Satellite: RIGL +8.31% unrealized ($41.84 -> $45.3152), well clear of the -15% hard-cut. Income: EDGX -0.52%, SGOV +0.06%, SPHY -1.54%, all well inside the 5% stop. All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed carrying live GTC trailing stops via `alpaca.sh orders`, none missing, none moved down. No position at/beyond its hard-cut; no unprotected position found.
- **Gap-risk check:** RIGL's H.C. Wainwright conference presentation is tomorrow (9/15, 10:00am ET) — inside the 1-2 trading-day window, but this is a non-binary IR appearance (same pattern as every prior conference: Citi 9/9, Cantor 9/10), not a binary catalyst date (VEPPANU's own binary PDUFA event already resolved 8/13). The binary-catalyst 5%-cap/max-loss-documentation rule does not apply; holding through at standard sizing is valid — stated explicitly per STEP 4.
- **Thesis check:** Fresh WebSearch on RIGL found no adverse news — today's intraday pullback ($47.24 -> $45.3152, -4.08%) reads as broad market risk-off (oil-shock/AI-pause selloff, flagged pre-market and 11am), not an idiosyncratic break. VEPPANU launch on track, guidance reaffirmed, 8 consecutive profitable quarters. Thesis intact, no close warranted. Income positions have no thesis to break; no adverse news found for EDGX/SGOV/SPHY.
- **No stop tightening this window** — RIGL (+8.31%) still well below the satellite +25% threshold; Income sleeve has no gain-based tightening schedule.
- 2-strike sub-sector tracker unchanged: biotech 2/2 cooldown active through 2026-09-25 (OCUL 7/23, MNKD 9/11); industrials 0/2.
- Risky positions closed today: none.
- No account activity today at all (no buys, sells, tightens, or sweeps). SendGrid still 401 (8th consecutive window, 5th session unresolved since first caught 9/10) — escalating again; content sent via local fallback.

## 2026-09-15 — Pre-market Research

### Account
- Equity: $93,128.43 | Cash: $18,777.57 (20.16%) | No auth errors on account/positions/orders calls.
- Core exposure: $0.00 (0.00% — 0 of 6 slots open, unchanged since APA's 9/8 thesis-broken exit) | Satellite exposure: $5,169.90 (5.55%: RIGL, 114 sh, +8.39% unrealized) | Income exposure: $69,180.96 (74.29%: EDGX $19,861.88/-0.58%, SGOV $29,656.77/+0.07%, SPHY $19,662.32/-1.67%)
- Cash 20.16%, at/above the 20% floor — no sweep needed.
- All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.5035 on 294 covered sh (hwm $100.53; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep, not a new gap).
- This week (Sep 14 start): core 0/6, satellite 0/4 new trades so far.
- Biotech satellite 2-strike cooldown remains ACTIVE through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries regardless of screen results. Industrials sub-sector unaffected (0 strikes).

### Data-quality notes
The single consolidated Gemini Deep Research call completed (exit 0) but **the captured report is truncated and starts mid-document** — it opens directly at a shipping-sector section (SHIP/GNK/PLPC), then core-watchlist sector commentary, then a synthesis/sources section. Section 1 (explicit S&P 500 futures %/VIX level), section 2 (a clean, itemized catalyst/econ-release list), and section 3 (held-ticker overnight news for RIGL/EDGX/SGOV/SPHY specifically, addressed only for RIGL) never came through in the expected structured form. Per STEP 3's data-quality guard, no figure for the missing sections was taken from Gemini as-is; independently verified/filled via WebSearch instead (see Market Context below). This is at least the fourth session with a Gemini data-quality issue in the past two weeks (9/12 truncated, 9/13 failed outright, 9/14 truncated, 9/15 truncated again) — worth a structural look (e.g. requesting numbered section headers explicitly, or splitting into submit/poll sub-queries) at the next weekly review.
- **Material factual error caught and corrected, not carried forward:** Gemini's report repeatedly describes the FOMC rate decision, an "EIA Petroleum Status Report," and Retail Sales data as releasing **today (9/15)** ("if the Fed executes this hike today," "the 2:00 PM FOMC rate hike," "Retail Sales data" at "8:30 AM ET" today, EIA report "10:30 AM ET" today). Independently verified via WebSearch (fedratecalc.com, Census Bureau release schedule, EIA's own schedule page): the FOMC meeting is a two-day meeting Sep 15-16, with the **rate decision, dot plot, and press conference all on Wednesday 9/16 at 2:00/2:30 PM ET**, and both the EIA Weekly Petroleum Status Report and the Census Bureau's August Retail Sales report are also scheduled for **9/16, not today**. Gemini appears to have conflated tomorrow's date with today's. None of these three events are treated as today's catalysts below.
- **Unverified/likely-incorrect claim not carried forward:** Gemini's synthesis section describes a "6.3% destruction of equity value" in Cullinan Therapeutics (CGEM) following a mortality-risk (HR 0.50) trial readout. Independent WebSearch found the opposite: CGEM's actual REZILIENT3 Phase 3 data (zipalertinib + chemo, first-line EGFR exon20ins NSCLC, presented 9/13 at IASLC WCLC, investor event 9/14) showed a positive PFS result (14.5mo vs 8.5mo), BTIG raised its price target (to $40) with a maintained Buy rating, and CGEM has +215% over the past year — no mortality/overall-survival figure or negative price reaction was found anywhere in the search results. Per TRADING-STRATEGY.md's pending-catalyst guard, this claim is treated as unconfirmed/likely erroneous and is not used. CGEM is not held and cannot be newly entered regardless (biotech cooldown through 2026-09-25), so this has no trading impact today, but flagging so a future session doesn't inherit a fabricated negative data point.

### Market Context
- **S&P 500 (E-mini) futures +0.35%, Nasdaq (E-mini) futures +0.94%** pre-market (WebSearch-confirmed, Barchart/Investing.com) — supported by weakness in crude oil pulling bond yields lower.
- **VIX:** October VIX futures (front contract as of the 9/13 roll) opened 18.83, range 18.70-19.11, prior close 18.36 (WebSearch-confirmed) — modestly elevated vs. the 9/11 close (15.84) but not a fresh spike, consistent with the oil/Hormuz situation staying an ongoing story rather than escalating further overnight.
- **Oil:** WTI down >3% today, easing inflation expectations and pulling the 10-year Treasury yield down ~4bp to 4.66% (WebSearch-confirmed) — a pullback from the elevated levels flagged 9/11-9/14, not a new escalation. Separately confirmed via WebSearch: no new incident at the Saudi East-West pipeline or in the Strait of Hormuz since the weekend; Saudi Arabia is actively working to boost throughput via Hormuz to offset the still-shut pipeline. Same ongoing story as 9/10-9/14, easing rather than escalating — does not clear STEP 5's "major geopolitical event" bar for a fresh notification.
- **FOMC:** two-day meeting today/tomorrow (9/15-16); rate decision, dot plot, and press conference are all **tomorrow, Wed 9/16, 2:00/2:30pm ET** — see Data-quality notes above for the correction from Gemini's mistaken "today" framing. Current target range 3.50-3.75%; markets pricing a 25bp hike to 3.75-4.00% as more likely than not (WebSearch-confirmed) — consistent with the ~86-87% hike-odds figures logged all week.
- **Retail Sales (August) and the EIA Weekly Petroleum Status Report are also tomorrow (9/16), not today** — see Data-quality notes.
- No confirmed idiosyncratic overnight news breaking the thesis for any of the 24 core watchlist names (DELL, HPE, HUM, MU, STX, MPC, ZBRA, CRWD, VLO, CRL, NTAP, VEEV, PSX, IQV, HOOD, STT, WDAY, CRM, GPC, BBY, ABNB, BNY, RVTY, MET) — Gemini's per-name commentary (CRM: ~$2B Listen Labs acquisition talks reported by Business Insider, contributing to a ~2% pullback despite a strong Q2 beat; STX: bear-case notes on Mozaic 4 CSP-qualification pace) is sector color, not thesis-breaking, and none is independently verified as new-overnight. Given the FOMC/retail-sales dating error above, no core-name-specific claim from Gemini is treated as confirmed without independent corroboration; none was material enough to chase down individually this session.

### Held-Position Thesis Check
- **RIGL (satellite, held, +8.39% unrealized: $41.84 → $45.35):** No adverse news. WebSearch-confirmed: CFO Dean Schorno presents (one-on-one meetings + company overview) at the H.C. Wainwright 28th Annual Global Investment Conference **today, 9/15, 10:00am ET** in New York — same non-binary IR pattern as every prior conference appearance this month (Citi 9/9, Cantor 9/10, flagged again 9/14). Thesis intact, no gap-risk sizing action (VEPPANU's binary PDUFA event already resolved 8/13). Continuing to sit below the satellite +25% tighten threshold.
- **EDGX, SGOV, SPHY (income, held):** Gemini's held-ticker section did not come through in usable form for any of the three. WebSearch found no idiosyncratic news for EDGX or SPHY specifically. EDGX -0.58% unrealized, no thesis to break (Income sleeve has none). SGOV +0.07%, thesis (flight-to-safety/short-duration) intact, reinforced by today's risk-off-adjacent tape. SPHY -1.67%, well inside its 5% stop; today's oil pullback/yield decline is a mild tailwind for credit spreads, not a concern.

### Core Trade Ideas (from current WATCHLIST.md core list)
None sourced as approved — this workflow does not place trades regardless, and Gemini's per-ticker section for the 24-name core list came through only as broad sector commentary, not individual news items with dates/sources (see Data-quality notes and Market Context). No name shows a confirmed, verified overnight development that would change its watchlist status. Full live momentum/FCF/rating reverification at the buy-side gate remains required before any entry regardless — none is pre-approved. Rebuilding core exposure from a fresh qualified candidate remains the top standing priority (0 of 6 slots open, 7 consecutive sessions since APA's 9/8 exit).

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
- **RIGL** (held) — see Held-Position Thesis Check, no new entry signal.
- **CGEM** — not held, cannot be newly entered regardless (biotech cooldown active through 2026-09-25). See Data-quality notes: Gemini's claimed negative ("6.3% destruction of equity value") reaction to CGEM's REZILIENT3 data is contradicted by WebSearch (positive PFS readout, price-target raise, no mortality/OS figure found) and is not used. WATCHLIST.md's catalyst date still reads "target action Feb 27, 2027 (confirmed, not imminent)" for the separate zipalertinib NDA action date — that entry is unaffected by this week's REZILIENT3 data event, which was a trial-data presentation, not the NDA decision itself. No entry-signal change (blocked by cooldown regardless).
- **PLPC** — Gemini confirms no catalyst inside the 5-day window (last catalyst, Q2 print, resolved). No new information this session; no entry-signal change.
- **SHIP, GNK** — Gemini confirms no company-specific catalyst inside the 5-day window for either (SHIP's dividend ex-date is 9/25 per WATCHLIST.md, not yet inside the window; GNK's Q3 dividend declaration still expected ~early Nov). Gemini's fundamental commentary (SHIP's Q2 record net income/TCE, GNK's Q3 TCE guidance and new Series B preferred "poison pill" defensive rights plan expiring Sept 2026) is consistent with prior weeks' screen and not thesis-breaking. No entry-signal change for either name.

### Risk Factors
- Gemini's report was truncated again this session and contained at least two material factual errors (FOMC/retail-sales/EIA dating; CGEM reaction direction) — both caught and corrected via WebSearch, neither carried forward. Fourth consecutive-ish session with a Gemini data-quality issue; worth a structural fix at the next weekly review.
- FOMC decision is tomorrow (9/16, 2pm ET), not today — hike odds ~86-87% per WebSearch. Headwind for core's long-duration/software cluster and held SPHY's high-yield credit exposure if realized; tailwind for held SGOV. No action due today; flag for tomorrow's sessions.
- Oil/Hormuz situation unchanged from 9/10-9/14 (no new incident found), and today's tape (WTI -3%, 10yr -4bp) is actually easing, not escalating — does not meet STEP 5's urgent-notification bar.
- No held position is near its hard-cut: RIGL +8.39% (vs -15% satellite cut), EDGX -0.58%, SGOV +0.07%, SPHY -1.67% (all vs -5% income cut).
- Cash 20.16%, at/above the 20% floor — no sweep action needed.
- RIGL's H.C. Wainwright conference today (10am ET) is non-binary IR; watch for any forward R289 commentary, no sizing action expected.
- SendGrid status not yet rechecked this session (see Notification below) — was broken (401) for 8 consecutive windows / 5 sessions as of yesterday's 3pm close; still needs owner-side key rotation regardless of today's send outcome.

### Decision
**HOLD.** No core or satellite trade sourced this session — valid per the Patience Rule; core exposure remains at 0% (7 consecutive sessions since APA's 9/8 exit) and rebuilding it from a fresh momentum/FCF/rating-qualified candidate stays the top priority once a live buy-side gate can run, but this research-only workflow does not place trades regardless. Nothing today qualifies as urgent per STEP 5 (no held position near a hard-cut, no satellite catalyst resolved negatively overnight, oil/Hormuz situation easing not escalating) — no notification planned this session absent a change. Top items for today: (1) FOMC begins today, decision tomorrow 9/16 2pm ET — monitor positioning, do not act on today's mistaken "decision is today" framing from Gemini; (2) RIGL's H.C. Wainwright presentation, 10am ET, non-binary, watch for R289 color; (3) continue tracking RIGL's approach toward its +25% satellite tighten threshold; (4) escalate SendGrid key rotation to the owner if still broken at this session's send attempt; (5) flag Gemini's recurring data-quality issues (truncation + this session's two factual errors) for the next weekly review; (6) core sleeve still needs a fresh qualified candidate — no change in 7 sessions.
...
### 11 AM Session Note
- **HELD, no new trades.** No "### Approved Trades (verified)" section in today's RESEARCH-LOG (pre-market/9:30am both HOLD, Gemini truncated again) — per STEP 3 buy side stays HOLD; core 0/6 open (8th consecutive session since APA's 9/8 exit), satellite 1/4 (RIGL only).
- Live re-check via `alpaca.sh account`/`positions`/`orders`: equity $92,960.24, cash $18,777.57 (20.20%, at/above the 20% floor, no sweep needed). All 4 GTC trailing stops (RIGL 15%/$42.415 hwm $49.90, EDGX 5%/$25.9825 hwm $27.35, SPHY 5%/$22.23 hwm $23.40, SGOV 5%/$95.513 on 294 covered sh hwm $100.54) confirmed live, none missing or moved down.
- No cuts: RIGL +7.72% unrealized (114 sh, $41.84 -> $45.07), nowhere near the -15% satellite hard-cut; no core positions to check. No tightens: RIGL still well below the +25% satellite tighten threshold; Income sleeve has no gain-based schedule. EDGX -1.18% and SPHY -1.76%, both well inside their 5% stops — no cuts.
- RIGL's H.C. Wainwright conference (10am ET) fell inside this window; price action mild (-0.62% intraday, in line with broad tape) with nothing indicating adverse commentary — no thesis break, no action. No sharp unexplained move on any held name, so no STEP 7 Gemini call warranted this window.
- 2-strike tracker unchanged: biotech cooldown active through 2026-09-25 (OCUL 7/23, MNKD 9/11); industrials 0/2. No account activity this window (no buys, sells, sweeps, or tightens) — valid per the Patience Rule.

### 3 PM Session Note
- **HELD, no new trades; no closes; no tightens.** No "### Approved Trades (verified)" section existed in today's RESEARCH-LOG at any window — per STEP 3 buy side stayed HOLD all session; core 0/6 open (8th consecutive session since APA's 9/8 exit), satellite 1/4 (RIGL only). This week (Sep 14 start) closes at core 0/6, satellite 0/4 new trades — valid per the Patience Rule.
- Risk sweep (3pm): no core positions to check. RIGL +6.92% unrealized (114 sh, $41.84 -> $44.735), well clear of the -15% satellite hard-cut and below the +25% tighten threshold — no cut, no tighten. EDGX -0.90% and SPHY -1.78%, both well inside their 5% income stops. SGOV +0.07%, flat. All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing, none moved down.
- Fresh WebSearch thesis check on RIGL post-H.C.-Wainwright (10am ET): no adverse news — VEPPANU launch tracking (potential >$1B opportunity per Rigel's own framing), 8 consecutive profitable quarters, FY26 guidance reaffirmed $285-295M, R289 data still guided for later this year. Non-binary IR appearance, no gap-risk sizing action (binary PDUFA already resolved 8/13). Today's intraday pullback (+7.72% at 11am -> +6.92% now) reads as broad-tape drift (FOMC positioning into tomorrow's 2pm decision), not idiosyncratic.
- 2-strike sub-sector tracker unchanged: biotech cooldown active through 2026-09-25 (OCUL 7/23, MNKD 9/11); industrials 0/2. Cash 20.19%, at/above the 20% floor, no sweep needed. Risky positions closed today: none.
- Pattern flag carried to Friday's weekly review: Gemini Deep Research has now truncated or errored in every session for two straight weeks running (9/12-9/15 all truncated, 9/13 failed outright) — worth a structural fix (explicit numbered section headers, or splitting into smaller submit/poll sub-queries). Core sleeve still needs a fresh qualified candidate — 8 consecutive sessions at 0/6. SendGrid 401 status to be reconfirmed at this session's send attempt (broken 8 consecutive windows / 5 sessions as of yesterday's close).

## 2026-09-16 — Pre-market Research

### Account
- Equity: $92,871.08 | Cash: $18,777.57 (20.22%) | No auth errors on account/positions/orders calls.
- Core exposure: $0.00 (0.00% — 0 of 6 slots open, unchanged since APA's 9/8 thesis-broken exit, 9th consecutive session) | Satellite exposure: $5,057.04 (5.45%: RIGL, 114 sh, +6.02% unrealized) | Income exposure: $69,036.47 (74.33%: EDGX $19,758.71/-1.10%, SGOV $29,658.24/+0.07%, SPHY $19,619.52/-1.88%)
- Cash 20.22%, at/above the 20% floor — no sweep needed.
- All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.513 on 294 covered sh (hwm $100.54; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep, not a new gap). No stop moved down.
- This week (Sep 14 start): core 0/6, satellite 0/4 new trades so far.
- Biotech satellite 2-strike cooldown remains ACTIVE through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries regardless of screen results. Industrials sub-sector unaffected (0 strikes).

### Data-quality notes
The single consolidated Gemini Deep Research call completed cleanly this session — all four requested sections (futures/VIX, catalysts, held-ticker news, watchlist news) came through in full, numbered form, with no truncation. First fully clean, non-truncated report in at least 5 consecutive sessions (9/12 truncated, 9/13 failed outright, 9/14 truncated, 9/15 truncated, 9/16 clean).
- **Tooling note (my own error, not Gemini's — flagging so a future session doesn't misdiagnose it):** my first capture attempt this session piped the command through `tail -c 20000`, which discarded everything before the last ~20KB and lost sections 1-2 and part of section 3. Caught it before treating the gap as a Gemini truncation, resubmitted cleanly to a file with no truncating pipe, and captured the full report used below. Same failure class already flagged 9/14 ("my own first attempt... was separately truncated by a mishandled shell pipe").
- **Possible stale-data flag, not carried forward as a fresh catalyst:** Gemini's DELL/HPE section describes a "record Q2" revenue beat ($46.97B) and a "$95 billion AI-server backlog" driving a claimed +15.76% overnight move. The $95B backlog figure is identical to the one already logged in this week's WATCHLIST.md screen (DELL's Q2 FY26 AI-server-backlog inventory-build narrative, screened 9/11) and in the 9/14 research log's AI-hardware-selloff note — this may be Gemini re-presenting an already-priced-in, weeks-old earnings event as if it were today's overnight news, the same anachronism pattern caught 9/15 (FOMC/retail-sales date confusion). Not independently re-verified via WebSearch this session; treated as sector color only, not a confirmed fresh catalyst — no trading impact today since core isn't live-gated in this workflow regardless.
- **Unconfirmed rumor, not carried forward as fact:** Gemini's WDAY item cites unnamed "financial media reports" of preliminary Silver Lake buyout talks (~$43B) with no primary source beyond secondary aggregator links. Flagged as unconfirmed per the same standard applied to prior weeks' unverified claims.
- **FOMC hike-odds figure doesn't reconcile with yesterday's log:** this session's report cites CME FedWatch hike odds at 66.4% (up from a claimed 41.4% a week ago); 9/15's entries logged ~86-87% hike odds via independent WebSearch. Flagging the inconsistency rather than resolving it (would need a dedicated WebSearch beyond today's budget) — either way a hike is priced as more likely than not, and the decision/press conference at 2:00/2:30pm ET remains today's dominant event regardless of the exact probability.

### Market Context
- **S&P 500 futures -0.50%** (ESZ26 ~7,654.00), **SPY -0.45%** (~$757.49) pre-market — driven by a sharp global sovereign-yield spike, not a new geopolitical shock.
- **VIX:** spot printing 17.10-17.20 (pre-market range 16.79-18.03), still below its long-run mean (~18.61); October VIX futures at a premium (18.45-18.63), term structure in contango ahead of the FOMC.
- **10-year Treasury yield at 5.04%, a 19-year high** — global sovereign yields moving in sync (German Bund 3.57%/17-yr high, JGB 3.04%/30-yr high). Primary driver of today's equity softness, compressing valuation multiples broadly and pressuring long-duration/high-multiple names specifically.
- **FOMC: rate decision 2:00pm ET, dot plot + Chair Kevin Warsh press conference 2:30pm ET** — hike priced more likely than not (see Data-quality notes on the odds-figure discrepancy vs. yesterday). Warsh's Jackson Hole remarks reportedly hawkish (inflation "stubbornly above" the 2% mandate).
- **August Retail Sales (8:30am) and the EIA Weekly Petroleum Status Report** also release today — both correctly scheduled this session (no repeat of 9/15's date-mixup error). A hot retail print or a bullish (drawdown) EIA read would reinforce the hawkish FOMC setup; a soft retail print against elevated PPI raises stagflation risk.
- No oil-shock/Strait-of-Hormuz story surfaced this session (the dominant risk factor 9/10-9/15) — absence noted, not confirmed resolved; today's macro driver has shifted entirely to the bond-yield/FOMC story.
- Sector read: AI-hardware cluster (DELL, HPE) shown sharply higher pre-market (see Data-quality notes re: possibly stale data); VEEV up on a clean Q2 beat (EPS $2.35 vs $2.10 est., rev +17.6%); RVTY up on a KeyBanc PT raise plus a Human Cell Design acquisition; CRWD down ~3.4% on competitive-pressure/valuation concerns; refiners (MPC, PSX, VLO) firm on tightening crude supply ahead of the EIA report.

### Held-Position Thesis Check
- **RIGL (satellite, held, +6.02% unrealized: $41.84 → $44.36):** No adverse news. Gemini confirms the H.C. Wainwright presentation (9/15) reaffirmed FY26 revenue guidance of $285-295M and 8 consecutive profitable quarters. Recent Form 4 filings are routine tax-withholding on vested options, not open-market insider selling. Thesis intact, well below the +25% satellite tighten threshold.
- **EDGX (income, held):** Declared a $0.0463/share dividend, ex-date 9/14 (already passed), payable 9/17 — reinvests to SGOV per the Income sleeve's fixed dividend-reinvestment rule once received. No thesis to break (Income sleeve has none). -1.10% unrealized, well inside the 5% stop.
- **SGOV (income, held):** NAV stable ~$100.52-100.53. Thesis (capital preservation + capturing front-end yield) reinforced by today's rate/yield backdrop. +0.07% unrealized.
- **SPHY (income, held):** No idiosyncratic news. Structurally exposed to credit-spread widening if the Fed hikes/holds hawkish, but no break — -1.88% unrealized, well inside the 5% stop.

### Core Trade Ideas (from current WATCHLIST.md core list)
None sourced as approved — this research-only workflow does not place trades regardless; a full live momentum/FCF/rating reverification at the buy-side gate is required before any entry. Sector color from today's (clean, non-truncated) report: DELL/HPE AI-hardware strength (treat cautiously per Data-quality notes — may be stale Q2 data re-presented as fresh), VEEV/RVTY showing genuine fresh positive catalysts (earnings beat; M&A + analyst upgrade respectively), CRWD showing fresh competitive-pressure weakness, WDAY buyout rumor unconfirmed. None of this is pre-approved; rebuilding core exposure from a fresh qualified candidate remains the top standing priority (0 of 6 slots open, 9th consecutive session since APA's 9/8 exit).

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
- **RIGL** (held) — see Held-Position Thesis Check, no new entry signal.
- **CGEM** — not held, cannot be newly entered regardless (biotech cooldown active through 2026-09-25). Gemini confirms no catalyst inside the 5-day window; NDA target action date remains Feb 27, 2027. No entry-signal change.
- **PLPC** — Gemini confirms catalyst resolved (Q2 print), no new event inside the 5-day window. No entry-signal change.
- **SHIP** — dividend ex-date 9/25 confirmed 7 trading days out, outside the 5-day window. No entry-signal change.
- **GNK** — Q3 dividend declaration still expected early November (Gemini estimates a resulting ex-date ~11/16-11/17), well outside the window. No entry-signal change.

### Risk Factors
- **New macro driver: synchronized global sovereign-bond selloff** — 10yr Treasury at a 19-year high (5.04%), German Bund and JGB also at multi-decade highs. Real, broad-based valuation-multiple headwind for long-duration/high-multiple core names, distinct from the oil/Hormuz story that dominated 9/10-9/15 (which did not resurface this session).
- FOMC decision, dot plot, and press conference today at 2:00/2:30pm ET — hike priced as more likely than not; see Data-quality notes on the odds-figure discrepancy vs. yesterday's log. Headwind for SPHY's high-yield credit exposure if hawkish, tailwind for SGOV.
- No held position is near its hard-cut: RIGL +6.02% (vs -15% satellite cut), EDGX -1.10%, SGOV +0.07%, SPHY -1.88% (all vs -5% income cut).
- Cash 20.22%, at/above the 20% floor — no sweep action needed.
- Core sleeve has now sat at 0% for 9 consecutive sessions since APA's 9/8 exit — today's clean (non-truncated) Gemini report removes one possible explanation (data-quality gaps) for the drought; worth checking at the next weekly review whether the gap is really "no qualifying candidate" vs. a process gap between research and the live buy-side gate.
- Data-quality items flagged above (my own truncating pipe on first capture; DELL's possibly-stale AI-backlog figure; WDAY's unconfirmed buyout rumor; the FOMC-odds discrepancy) — none blocks today's HOLD decision.
- SendGrid status not tested this session (no urgent condition triggered STEP 5) — was broken (401) for 8 consecutive windows / 5 sessions as of 9/15's 3pm close; still needs owner-side key rotation, unverified whether resolved.

### Decision
**HOLD.** No core or satellite trade sourced this session — valid per the Patience Rule; core exposure remains at 0% (9th consecutive session since APA's 9/8 exit) and rebuilding it from a fresh qualified candidate stays the top priority once a live buy-side gate can run, but this research-only workflow does not place trades regardless. Nothing today qualifies as urgent per STEP 5 (no held position near a hard-cut, no satellite catalyst resolved negatively, no fresh geopolitical shock — today's dominant story is a synchronized bond-yield spike into the FOMC, not a sudden overnight event). Top items for today: (1) FOMC decision, dot plot, and Chair Warsh press conference at 2:00/2:30pm ET — the dominant scheduled event; (2) August Retail Sales (8:30am) and the EIA Weekly Petroleum Status Report, both correctly scheduled today; (3) verify DELL/HPE's claimed pre-market strength independently before treating it as a fresh catalyst — may be stale Q2 data; (4) treat WDAY's Silver Lake buyout rumor as unconfirmed; (5) core sleeve still needs a fresh qualified candidate — 9 sessions running; (6) recheck SendGrid key status when a notification is next due.
...
### 11 AM Session Note
- No "### Approved Trades (verified)" list existed anywhere in today's RESEARCH-LOG (pre-market/9:30am both HOLD) — per STEP 3 buy side stays HOLD; core 0/6 open (9th consecutive session since APA's 9/8 exit), satellite 1/4 (RIGL only). No buys this window.
- Live re-check via `alpaca.sh account`/`positions`/`orders`: equity $93,085.51, cash $18,777.57 (20.17%, at/above the 20% floor, no sweep needed). All 4 GTC trailing stops confirmed live, none missing, none moved down: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.5225 on 294 covered sh (hwm $100.55).
- No cuts: RIGL +6.97% unrealized, well clear of the -15% satellite hard-cut; EDGX -0.60% and SPHY -1.54%, both well inside the 5% income stop; SGOV +0.07%. No core positions to check. No tightens: RIGL still well below the +25% satellite tighten threshold; Income sleeve has no gain-based schedule.
- Thesis check: no adverse news on any held name since the 9:30am note. No sharp unexplained intraday move on any position — today's dominant driver (synchronized bond-yield spike into the 2pm/2:30pm ET FOMC decision) is broad macro, not idiosyncratic, so no STEP 7 Gemini call warranted this window. 2-strike tracker unchanged: biotech cooldown active through 2026-09-25 (OCUL 7/23, MNKD 9/11); industrials 0/2.
- No account activity this window (no buys, sells, sweeps, or tightens) — valid per the Patience Rule. Top watch item into the close: FOMC decision/dot plot/press conference at 2:00/2:30pm ET, the week's dominant scheduled event.

### 3 PM Session Note
- No "### Approved Trades (verified)" list existed anywhere in today's RESEARCH-LOG (pre-market/9:30am/11am all HOLD) — per STEP 3 buy side stays HOLD all session; core 0/6 open (9th consecutive session since APA's 9/8 exit), satellite 1/4 (RIGL only). No buys this window or all day.
- FOMC decision landed as priced: 25bp hike to 3.75%-4.00% (first hike since 2023), 12-0 vote; updated dot plot shows 16 of 18 participants expecting at least one more hike this year. Live re-check via `alpaca.sh account`/`positions`/`orders`: equity $93,001.44, cash $18,777.57 (20.19%, at/above the 20% floor, no sweep needed). All 4 GTC trailing stops confirmed live, none missing, none moved down: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.5225 on 294 covered sh (hwm $100.55; 0.989473856-sh fractional remainder stays unstopped, same immaterial dust flagged every prior sweep).
- Risk sweep (STEP 4): core has 0 open positions, nothing to check. Satellite RIGL +6.93%, well clear of the -15% hard-cut, no gap-risk catalyst inside the 1-2 trading-day window (Wainwright conference already resolved 9/15, non-binary). Income EDGX -0.77% and SPHY -1.78%, both well inside the 5% stop; SGOV +0.07%. Thesis check: no adverse news on any held name post-FOMC — the hike/hawkish-dot-plot outcome is priced macro, not idiosyncratic to any position; SPHY (high-yield credit) and SGOV (front-end yield) are the two names structurally exposed to the outcome but neither shows a break, both remain well inside their 5% stop/thesis. No closes needed.
- No tightens this window: RIGL (+6.93%) still well below the satellite +25% threshold; Income sleeve has no gain-based schedule. No stop moved down. 2-strike tracker unchanged: biotech cooldown active through 2026-09-25 (OCUL 7/23, MNKD 9/11); industrials 0/2.
- Risky positions closed today: none. No account activity all session (no buys, sells, sweeps, or tightens) — valid per the Patience Rule. Core sleeve still needs a fresh qualified candidate (9 consecutive sessions since APA's 9/8 exit) — top standing priority for tomorrow's pre-market research.


## 2026-09-17 — Pre-market Research

### Account
- Equity: $92,852.83 | Cash: $18,777.57 (20.22%) | Buying power: $282,521.02 (margin-inflated by the standing 4x multiplier flag — no margin/leverage used or planned) | Daytrade count: not present in account payload (same pattern as prior sessions)
- Core exposure: $0.00 (0.00% — 0 of 6 slots open, unchanged since APA's 9/8 thesis-broken exit, 10th consecutive session) | Satellite exposure: $5,070.72 (5.46%: RIGL, 114 sh, +6.31% unrealized) | Income exposure: $69,004.54 (74.32%: EDGX $19,686.64/-1.46%, SGOV $29,664.14/+0.09%, SPHY $19,653.76/-1.71%)
- Cash 20.22%, at/above the 20% floor — no sweep needed.
- All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing, no auth errors on account/positions/orders calls: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.5225 on 294 covered sh (hwm $100.55; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep). No stop moved down.
- This week (Sep 14 start): core 0/6, satellite 0/4 new trades so far.
- Biotech satellite 2-strike cooldown remains ACTIVE through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries regardless of screen results. Industrials sub-sector unaffected (0 strikes).

### Data-quality notes
**Section 1 (S&P 500 futures direction / VIX level) is missing entirely from this session's Gemini report** — the captured output starts directly at "## Top Market-Moving Catalysts and Economic Releases Today" (section 2) with no preceding futures/VIX section anywhere in the document; confirmed via `grep -i "vix\|futures"` against the full raw capture, which returns zero explicit futures-% or VIX-level lines. Per the data-quality guard, no futures/VIX figure was fabricated from Gemini's output. Filled the gap with a single native WebSearch query (not a second Gemini call, per CLAUDE.md's guidance against serial small queries) — see Market Context below for the WebSearch-sourced figures, flagged as such.
- Sections 2-4 (catalysts, held-ticker news, watchlist news, satellite catalyst-proximity table) all came through complete and non-truncated.
- **Notable finding: SPHY thesis-strain flag.** Gemini's report surfaces a real, sourced technical/fundamental deterioration in SPHY — monthly distributions declining every month in 2026 vs. prior-year comps (latest ex-date payment $0.135652 vs. $0.146775 a year earlier), price below its 50-day MA since 9/10, Momentum Indicator below zero since 9/8, MACD histogram negative. Not a full thesis break (no spike in corporate default rates, no credit event) and the Income sleeve has no thesis-break exit regardless (5% trailing stop and the buy-funding sweep are its only exit paths, per TRADING-STRATEGY.md) — logged as a watch item, no action taken; SPHY is -1.71% unrealized, well inside its 5% stop.

### Market Context
- **S&P 500 futures +0.3%, VIX ~17.05 (-0.87%)** — sourced via WebSearch fallback (see Data-quality notes; Gemini's report omitted this section). Modestly calm/positive premarket tape, a bit at odds with the "aggressively defensive" tone of Gemini's own strategic conclusion — noted, not reconciled further; treat the WebSearch figures as the best-available numbers for today, not independently cross-verified beyond the single query.
- **Economic calendar (all pre-open):** 7:30am — Initial Jobless Claims (207K consensus vs. 206K prior), Philly Fed Manufacturing Index (31.3 consensus vs. 47.4 prior — a large forecast deceleration), Building Permits (1.400M vs. 1.433M prior), Housing Starts (1.320M vs. 1.239M prior). 9:00am — Pending Home Sales (2.0% consensus vs. -2.3% prior). All read against yesterday's 25bp FOMC hike (3.75%-4.00%, 12-0, hawkish dot plot) and a 10-year Treasury yield at a 19-year high (5.04%) — a hot claims/Philly-Fed print would likely extend the bond selloff; a soft print would offer equities a reprieve.
- Sector read: semis/AI-hardware cluster showing sharp divergence — **MU (core watchlist)** strongly positive (new India assembly facility live and shipping to DELL, HBM capacity ramp to 100K wafers/month by year-end, Sept 30 FY26 Q4 earnings ahead, premarket +1%+); **STX (not on current watchlist)** down ~15% over a 5-day stretch on heavy coordinated 10b5-1 insider selling by the CEO/CFO, fundamentals otherwise intact per Gemini. **RVTY** +9.1% to a new 52-week high (raised guidance + Human Cell Design acquisition). **IQV** near 12-month highs (earnings beat, $2B buyback authorized). **CRWD** +3.34% premarket (flight-to-quality bid into cybersecurity). **MPC** +3.90% premarket (refining/crack-spread strength). **BNY** strong ROTCE (24% vs. 15.5% industry) though -1.78% recently on broad macro pressure. WDAY/CRM/other high-multiple SaaS names flagged as structurally exposed to the elevated 10-year yield (DCF discount-rate pressure), no company-specific news.

### Held-Position Thesis Check
- **RIGL (satellite, held, +6.31% unrealized):** No adverse news, no company-specific overnight developments. Thesis intact, well below the +25% satellite tighten threshold and the -15% hard-cut.
- **EDGX (income, held, -1.46% unrealized):** Thesis intact and arguably augmented — Cboe has proposed rule changes to extend EDGX equity/derivatives trading to 23 hours/day, 5 days/week, plus a new Data Vendor Program and Clock Service fees (revenue-accretive). No thesis to break for Income sleeve regardless; well inside the 5% stop.
- **SGOV (income, held, +0.09% unrealized):** Thesis intact and reinforced — yesterday's 25bp hike continues to lift yields on SGOV's underlying short-duration paper; NAV stable ~$100.54-100.56.
- **SPHY (income, held, -1.71% unrealized):** See Data-quality notes — real distribution-decline and technical-breakdown flags, elevated to a watch item, not a break. No thesis-break exit exists for Income sleeve; well inside the 5% stop.

### Core Trade Ideas (from current WATCHLIST.md core list)
None sourced as approved — this research-only workflow does not place trades regardless; a full live momentum/FCF/rating reverification at the buy-side gate is required before any entry. Sector color from today's report: MU showing genuine fresh operational strength (India fab live, HBM ramp, Sept 30 earnings catalyst ahead) — worth a fresh look at the live buy-side gate; RVTY and IQV both showing fresh, real positive catalysts (guidance raise + M&A; earnings beat + buyback). STX is not on the current watchlist (didn't clear the 9/11 screen) so its sharp insider-selling-driven decline is informational only, not actionable. None of this is pre-approved; rebuilding core exposure from a fresh qualified candidate remains the top standing priority (0 of 6 slots open, 10th consecutive session since APA's 9/8 exit).

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
- **RIGL** (held) — see Held-Position Thesis Check, no new entry signal.
- **CGEM** — not held, cannot be newly entered regardless (biotech cooldown active through 2026-09-25). Gemini confirms no catalyst inside the 5-day window; NDA target action date remains Feb 27, 2027 (>5 months out). No entry-signal change.
- **PLPC** — Gemini confirms catalyst resolved (Q2 earnings, 9/3), no new event inside the 5-day window. Note: consensus price targets recently cut ~11% (to $35.36 avg) on lack of near-term profitability despite the revenue beat — informational, not a thesis break for a position not held. No entry-signal change.
- **SHIP** — dividend ex-date 9/25 confirmed 6 trading days out, just outside the 5-day window. No entry-signal change.
- **GNK** — no near-term catalyst scheduled; Gemini notes a new 52-week high ($28.22) and an 11.4% annualized yield with an elevated 351% payout ratio — informational only. No entry-signal change.

### Risk Factors
- **SPHY thesis-strain watch item** (see Data-quality notes / Held-Position Thesis Check) — declining distributions, below 50-day MA since 9/10, negative MACD — not yet a break, no action available beyond the existing 5% stop per Income sleeve rules, but worth tracking at each subsequent window this week.
- 10-year Treasury yield remains at a 19-year high (5.04%) following yesterday's hawkish FOMC hike (3.75%-4.00%, dot plot showing 16 of 18 participants expecting at least one more hike this year) — continued headwind for high-multiple/long-duration core names and for SPHY's high-yield credit exposure; continued tailwind for SGOV.
- No held position is near its hard-cut: RIGL +6.31% (vs. -15% satellite cut), EDGX -1.46%, SGOV +0.09%, SPHY -1.71% (all vs. -5% income cut).
- Cash 20.22%, at/above the 20% floor — no sweep action needed.
- Core sleeve has now sat at 0% for 10 consecutive sessions since APA's 9/8 exit.
- Gemini's section-1 omission (futures/VIX) flagged above — filled via a single WebSearch fallback, not independently cross-verified beyond that one query.
- SendGrid status: last confirmed broken (401 Unauthorized) as of 9/15's close, not retested 9/16 (no urgent condition triggered a send attempt that session) — will be tested this session's STEP 5 attempt below if a notification were due; since none is due today, status remains unconfirmed until next actual send attempt.

### Decision
**HOLD.** No core or satellite trade sourced this session — valid per the Patience Rule; core exposure remains at 0% (10th consecutive session since APA's 9/8 exit) and rebuilding it from a fresh qualified candidate stays the top priority once a live buy-side gate can run, but this research-only workflow does not place trades regardless. Nothing today qualifies as urgent per STEP 5 (no held position near a hard-cut, no satellite catalyst resolved negatively — none of CGEM/PLPC/SHIP/GNK are inside their catalyst window today, no major geopolitical event — today's dominant story is the post-FOMC bond-yield/economic-data backdrop, already known). Top items for today: (1) 7:30am/9:00am economic releases (jobless claims, Philly Fed, housing data, pending home sales) against the hawkish FOMC backdrop; (2) track SPHY's distribution-decline/technical-breakdown watch item at subsequent windows; (3) MU as a potential fresh core research angle ahead of its Sept 30 earnings, subject to a full live buy-side gate check; (4) core sleeve still needs a fresh qualified candidate — 10 sessions running; (5) confirm SendGrid status next time a notification is actually due.

### Notification
**Silent — no urgent condition met.** Per STEP 5, notification is silent unless a held position is already below its hard-cut pre-market, a satellite catalyst resolved negatively overnight, or a major geopolitical event occurred. None applies: all 4 positions are well clear of their stops (SPHY's thesis-strain flag is a watch item, not a hard-cut breach), no satellite catalyst is inside its 5-day window, and no fresh geopolitical shock occurred overnight. No email sent this session.

### 9:30 AM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market decision above was HOLD) — per STEP 3 this window trades from that list only, so no buys placed. This week (Sep 14 start) stays at core 0/6, satellite 0/4 new trades.
- **Account/positions/stops:** Equity $93,021.80, cash $18,777.57 (20.18%, at/above the 20% floor — no sweep needed). 4 open positions unchanged from pre-market (RIGL satellite, EDGX/SGOV/SPHY income), all 4 confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing, none moved down: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.532 on 294 covered sh (hwm $100.56).
- **Risk sweep:** no position near a hard-cut — RIGL +8.75% (vs. -15% satellite cut, and still below the +25% tighten threshold so no tightening applies), EDGX -1.46% and SPHY -1.43% (both well inside the 5% income stop). Core has 0 open positions, nothing to check. 2-strike sub-sector tracker unchanged: biotech 2/2 cooldown active through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries; industrials 0/2.
- **Lesson for Friday's review:** core sleeve has now sat at 0% for 11 consecutive sessions since APA's 9/8 exit — worth an explicit discussion of whether the buy-side gate criteria are too tight given how many names have cleared momentum but stalled on fundamentals/sector-cap in recent screens (DELL/HPE/MU cluster, GEN/ANET sector-capped alternates).

### 11 AM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market and 9:30am both HOLD) — per STEP 3 this window trades from that list only, so no buys placed. This week (Sep 14 start) stays at core 0/6, satellite 0/4 new trades.
- **Account/positions/stops:** Equity $93,269.19, cash $18,777.57 (20.13%, at/above the 20% floor — no sweep needed). 4 open positions unchanged from 9:30am (RIGL satellite, EDGX/SGOV/SPHY income), all 4 confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing, none moved down: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.532 on 294 covered sh (hwm $100.56).
- **Risk sweep:** no position near a hard-cut — RIGL +9.56% (vs. -15% satellite cut, still below the +25% tighten threshold so no tightening applies), EDGX -0.39% and SPHY -1.46% (both well inside the 5% income stop). Core has 0 open positions, nothing to check. No cuts. 2-strike sub-sector tracker unchanged: biotech 2/2 cooldown active through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries; industrials 0/2.
- **Thesis check:** RIGL — no adverse news since 9:30am, continued intraday strength (+3.06%), no gap-risk catalyst inside window. EDGX/SGOV/SPHY — no thesis to break; SPHY's distribution-decline/technical-breakdown flag (from pre-market) remains a watch item, not a break, well inside its 5% stop. No Gemini follow-up — no position moved sharply without an already-documented cause.
- No action taken this window — zero trades, zero cuts, zero tightens, zero stop moves.

### 3 PM Session Note
- **Buy-side: HOLD.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market, 9:30am, 11am, 3pm all HOLD) — per STEP 3 no buys placed this window or all session. This week (Sep 14 start) closes today at core 0/6, satellite 0/4 new trades so far.
- **Account/positions/stops:** Equity $93,366.36, cash $18,777.57 (20.11%, at/above the 20% floor — no sweep needed). 4 open positions unchanged all day (RIGL satellite, EDGX/SGOV/SPHY income), all 4 reconfirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing, none moved down: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.532 on 294 covered sh (hwm $100.554; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep, not a new gap).
- **End-of-session risk sweep (STEP 4):** no position at/beyond its sleeve hard-cut — core has 0 open positions, nothing to check; RIGL +10.05% (vs. -15% satellite hard-cut, not close). No open position missing a stop. No satellite catalyst date inside the 1-2 trading-day gap-risk window (RIGL's own binary PDUFA event resolved 8/13; no new near-term event surfaced today). Thesis check: RIGL intact, no adverse news found today, continued intraday strength since 11am (+10.05% vs. +9.56% at 11am); EDGX/SGOV/SPHY have no thesis to break under Income sleeve rules (5% trailing stop is the only exit path) — SPHY's distribution-decline/technical-breakdown watch item (flagged pre-market) persists but remains a watch item, not a break, well inside its 5% stop (-1.41%). **Risky positions closed today: none.**
- **Stop tightening (STEP 5):** none — RIGL (+10.05%) still well below the satellite +25% tighten threshold; Income sleeve has no gain-based tightening schedule; core has no open positions. No stop moved down.
- **2-strike sub-sector tracker:** unchanged — biotech 2/2, cooldown active through 2026-09-25 (OCUL 7/23, MNKD 9/11), no new satellite biotech entries; industrials 0/2.
- **Lesson for Friday's review:** core sleeve has now sat at 0% for 12 consecutive sessions since APA's 9/8 exit — same standing item flagged 9/17 9:30am; worth revisiting whether the buy-side gate criteria are too tight given repeated names clearing momentum but stalling on fundamentals/sector-cap (DELL/HPE/MU cluster, GEN/ANET sector-capped alternates) or on wide/stale opening spreads (DELL, CRWD, DXCM, NTAP, MPC, VLO, PSX per 9/9-9/10).


## 2026-09-18 — Pre-market Research

### Account
- Equity: $93,319.70 | Cash: $18,777.57 (20.12%) | Buying power: $283,828.25 (margin-inflated by the standing 4x multiplier flag — no margin/leverage used or planned) | Daytrade count: not present in account payload (same pattern as prior sessions)
- Core exposure: $0.00 (0.00% — 0 of 6 slots open, unchanged since APA's 9/8 thesis-broken exit, 13th consecutive session) | Satellite exposure: $5,226.90 (5.60%: RIGL, 114 sh, +9.58% unrealized) | Income exposure: $69,315.23 (74.28%: EDGX $19,930.51/-0.24%, SGOV $29,671.04/+0.11%, SPHY $19,713.68/-1.41%)
- Cash 20.12%, at/above the 20% floor — no sweep needed.
- All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing, no auth errors on account/positions/orders calls: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.532 on 294 covered sh (hwm $100.56; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep). No stop moved down.
- This week (Sep 14 start): core 0/6, satellite 0/4 new trades so far.
- Biotech satellite 2-strike cooldown remains ACTIVE through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries regardless of screen results. Industrials sub-sector unaffected (0 strikes).

### Data-quality notes
**This session's Gemini Deep Research report was severely truncated — the captured output contains only a "## Conclusion" paragraph and a citation list; sections 1-4 (S&P futures/VIX, market-moving catalysts, held-ticker overnight news, watchlist overnight news/catalyst-proximity) are entirely absent from the document, a new and more severe variant of prior weeks' "truncated mid-paragraph" and "citations only" failure modes.** Per the data-quality guard, nothing was taken from the missing sections — no figure or headline below is sourced from that gap. Rather than a same-query resubmit (a single retry risks the same multi-section loss and this workflow's time budget), fell back to targeted native WebSearch queries covering the highest-priority items: futures/VIX, the day's top catalyst, the one held satellite name (RIGL), and the one satellite watchlist name flagged by Gemini's own (surviving) conclusion text as time-sensitive (SHIP). The Conclusion paragraph itself was usable and is treated as a real, if unsupported-by-body, signal: it flagged SHIP's ex-dividend date as sitting "exactly at the edge of the five-trading-day action window" — confirmed independently via WebSearch below.
- **Gap not filled this session:** ticker-by-ticker overnight news for the other 23 core watchlist names and CGEM/PLPC/GNK satellite names was not independently re-verified via WebSearch (no held position or near-term catalyst makes this urgent today, and serial WebSearch across ~26 individual names is disproportionate for a HOLD-default session) — treated as data-unavailable for those names specifically, not as "no news."
- SendGrid status: last confirmed broken (401 Unauthorized) as of 9/15's close, not retested since (no urgent condition triggered a send attempt 9/16 or 9/17) — will be tested this session's STEP 5 attempt below only if a notification is actually due.

### Market Context
- **S&P 500 futures +1.94%, Nasdaq futures +2.62%, Dow futures +1.36%, Russell futures +1.14%, VIX ~15.41 (down from ~17.05 yesterday)** — sourced via WebSearch fallback (see Data-quality notes; Gemini's report omitted this section entirely). A strong, broad risk-on premarket tone as the market continues digesting Wednesday's 25bp FOMC hike; not independently cross-verified beyond the single query.
- **Catalysts:** Bank of Japan rate decision and August industrial production/Conference Board leading indicators are today's scheduled macro events. Crude oil fell on reports of Saudi Arabia moving to bring more supply online, which is providing secondary equity support by easing inflation-expectation pressure — consistent with the risk-on futures tone above. No major scheduled US catalyst on the scale of Wednesday's FOMC decision.
- Sector color: none independently re-verified this session beyond the held/flagged names below (see Data-quality notes gap).

### Held-Position Thesis Check
- **RIGL (satellite, held, +9.58% unrealized):** No new company-specific news since 9/17. WebSearch confirms Rigel's September investor-conference participation (Citi 9/9, Cantor 9/10, H.C. Wainwright 9/15) is already-known, non-binary IR activity, all now in the past — no new item. Thesis intact, well below the +25% satellite tighten threshold and the -15% hard-cut.
- **EDGX (income, held, -0.24% unrealized):** No adverse news found. No thesis to break for Income sleeve regardless; well inside the 5% stop.
- **SGOV (income, held, +0.11% unrealized):** No adverse news found; short-duration T-bill fund continues to benefit from this week's rate backdrop. Well inside the 5% stop.
- **SPHY (income, held, -1.41% unrealized):** WebSearch found no new negative development beyond the already-logged distribution-decline/technical-breakdown watch item (flagged 9/17) — most recent confirmed distribution (ex-date 9/1, paid 9/4) is consistent with the prior pattern, no fresh credit-event or halt news. Remains a watch item, not a break; well inside the 5% stop.

### Core Trade Ideas (from current WATCHLIST.md core list)
None sourced as approved — this research-only workflow does not place trades regardless; a full live momentum/FCF/rating reverification at the buy-side gate is required before any entry. No ticker-by-ticker overnight news re-check was performed this session for the 24-name core list (see Data-quality notes) — nothing here should be read as a clean bill of health, only as "not screened today." Core sleeve still needs a fresh qualified candidate; rebuilding core exposure from a fresh momentum/FCF/rating-qualified candidate stays the top standing priority (0 of 6 slots open, 13th consecutive session since APA's 9/8 exit).

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
- **RIGL** (held) — see Held-Position Thesis Check, no new entry signal.
- **CGEM** — not held, cannot be newly entered regardless (biotech cooldown active through 2026-09-25). Not independently re-checked this session (see Data-quality notes gap); NDA target action date remains Feb 27, 2027 per last week's confirmed data (>5 months out), so no plausible change to entry-signal status regardless.
- **PLPC** — not independently re-checked this session; catalyst (Q2 earnings) was already resolved as of last refresh, no new event expected. No entry-signal change.
- **SHIP** — **catalyst inside the window: dividend ex-date confirmed via WebSearch as 2026-09-25 ($0.35/share), which lands exactly at the 5-trading-day boundary from today (9/18: Mon 9/21, Tue 9/22, Wed 9/23, Thu 9/24, Fri 9/25 = 5 trading days out).** Gemini's surviving Conclusion paragraph independently flagged this same item despite the body truncation. Industrials sub-sector is unaffected by the biotech cooldown. This is informational for a potential future entry, not an approved trade — this workflow does not place trades; any new SHIP entry would still need a full live buy-side gate pass (momentum, YoY growth, rating, and — given the near-dated non-binary ex-date catalyst — sizing/documentation per the satellite gap-risk rule) at a trade window.
- **GNK** — not independently re-checked this session; no near-term catalyst was scheduled as of last refresh. No entry-signal change.

### Risk Factors
- **Gemini Deep Research infrastructure degraded again this session** — see Data-quality notes; this is the most severe truncation variant yet (entire numbered body missing, only Conclusion + sources survived). Worth flagging for Friday's review alongside the standing Gemini-reliability pattern.
- SPHY thesis-strain watch item persists (distribution decline, sub-50-day-MA) — not yet a break, well inside its 5% stop.
- No held position is near its hard-cut: RIGL +9.58% (vs. -15% satellite cut), EDGX -0.24%, SGOV +0.11%, SPHY -1.41% (all vs. -5% income cut).
- Cash 20.12%, at/above the 20% floor — no sweep action needed.
- Core sleeve has now sat at 0% for 13 consecutive sessions since APA's 9/8 exit.
- SHIP's near-dated (5-trading-day-boundary) dividend ex-date is a real, confirmed item for the next screen/trade-window discussion, not a today-action item under this research-only workflow.
- SendGrid status unconfirmed since 9/15 (last known 401) — will be tested only if STEP 5 below actually attempts a send.

### Decision
**HOLD.** No core or satellite trade sourced this session — valid per the Patience Rule; core exposure remains at 0% (13th consecutive session since APA's 9/8 exit) and rebuilding it from a fresh qualified candidate stays the top priority once a live buy-side gate can run, but this research-only workflow does not place trades regardless. Nothing today qualifies as urgent per STEP 5 (no held position near a hard-cut, no satellite catalyst resolved negatively, no major geopolitical event — today's dominant story is a continued risk-on relief rally post-FOMC with falling oil and a VIX crush). Top items for today: (1) Gemini's severe truncation this session — flag for Friday's review; (2) SHIP's ex-dividend date lands exactly at the 5-trading-day catalyst-window boundary (9/25) — worth a closer look at a live trade window given industrials is unaffected by the biotech cooldown; (3) core sleeve still needs a fresh qualified candidate — 13 sessions running; (4) confirm SendGrid status next time a notification is actually due.
...
### 11 AM Session Note
- **HOLD, no trades.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market, 9:30am both HOLD) — per STEP 3 this window trades from that list only, so no buys placed. This week (Sep 14 start) stays at core 0/6, satellite 0/4.
- Live `alpaca.sh account`/`positions`/`orders` reconfirmed: equity $93,256.25, cash $18,777.57 (20.13%, at/above the 20% floor, no sweep needed). Core 0.00% (0/6, 15th consecutive session since APA's 9/8 exit). Satellite: RIGL 114 sh, +9.29% unrealized — well clear of the -15% cut and still below the +25% tighten threshold, no action; no gap-risk catalyst inside the 1-2 trading-day window. Income: EDGX -0.24%, SGOV +0.11%, SPHY -1.65% — all well inside the 5% stop. All 4 positions confirmed carrying live GTC trailing stops (RIGL 15%/$42.415 hwm $49.90, EDGX 5%/$25.9825 hwm $27.35, SPHY 5%/$22.23 hwm $23.40, SGOV 5%/$95.5605 on 294 covered sh hwm $100.59), none missing, no stop moved down.
- No losers to cut (no core positions; RIGL +9.29%, not a loser). No thesis breaks: RIGL intact (no new adverse news since pre-market/9:30 checks), Income sleeve has no thesis to break. Nothing moved sharply intraday (largest intraday move RIGL -0.27%) — no Gemini consolidated-move query warranted. 2-strike biotech cooldown unchanged (active through 09-25); industrials 0/2.
- Nothing to notify on this window (no action taken).

### 3 PM Session Note
- **HOLD, no trades.** No "### Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market, 9:30am, 11am all HOLD) — per STEP 3 this window trades from that list only, so no buys placed. This week (Sep 14 start) closes today at core 0/6, satellite 0/4 new trades — valid per the Patience Rule.
- Live `alpaca.sh account`/`positions`/`orders` reconfirmed: equity $93,300.19, cash $18,777.57 (20.13%, at/above the 20% floor, no sweep needed). Core 0.00% (0/6 slots, 16th consecutive session since APA's 9/8 exit). Satellite: RIGL 114 sh, +10.06% unrealized — well clear of the -15% hard-cut and still below the +25% tighten threshold, no action; RIGL's own binary PDUFA event resolved 8/13, no satellite catalyst inside the 1-2 trading-day gap-risk window (SHIP's 9/25 ex-date is informational only, not held). Income: EDGX -0.28%, SGOV +0.11%, SPHY -1.58% — all well inside the 5% stop.
- **Risk sweep (STEP 4):** all 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing (RIGL 15%/$42.415 hwm $49.90, EDGX 5%/$25.9825 hwm $27.35, SPHY 5%/$22.23 hwm $23.40, SGOV 5%/$95.5605 on 294 covered sh hwm $100.59; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep). No position at/beyond its sleeve hard-cut. No thesis breaks: RIGL intact (no new adverse news since 11am), Income sleeve has no thesis to break (SPHY's distribution-decline/technical-breakdown watch item persists as a watch item, not a break). No stop moved down. 2-strike biotech cooldown unchanged (active through 09-25); industrials 0/2. **Risky positions closed today: none.**
- **STEP 5 (tighten):** no action — RIGL (+10.06%) still below the satellite +25% tighten threshold; Income sleeve has no gain-based schedule; no core positions.
- Day P&L -$66.17 (-0.07%) vs. yesterday's (9/17) 3pm snapshot ($93,366.36) — a modest pullback despite RIGL's continued gain, driven by SPHY/EDGX softness on the day. Phase P&L -$6,699.81 (-6.70%) since Day 0 ($100,000 -> $93,300.19). Core exposure remains 0.00% (0/6, 16 consecutive sessions since APA's 9/8 exit) — rebuilding it stays the top priority; MU (flagged 9/17, Sept 30 earnings ahead) remains the standing angle for the next research window. Satellite steady at 5.63% (1/4 slots: RIGL only; biotech cooldown active through 9/25, industrials 0/2).
- **SendGrid tested this window (STEP 9): still 401 Unauthorized (12th consecutive window / 9th session since first caught 9/10).** `sendgrid.sh` only falls back to a local file on missing credentials, not on an API-level 401, so the send aborted without auto-writing a fallback; content preserved by manually appending to DAILY-SUMMARY.md in the same fallback format used since 9/10. Owner action to rotate the key remains overdue and is escalated again this window.
- Tomorrow: re-verify momentum/FCF/rating live at the buy-side gate for a core rebuild candidate; watch RIGL's continued approach toward the +25% satellite tighten threshold; SHIP's 9/25 dividend ex-date is the next satellite item worth a closer look once the biotech cooldown context is moot (industrials unaffected); Friday's weekly-review/watchlist screen-refresh is the next major workflow, with the 16-consecutive-session core drought as a standing discussion item.


## 2026-09-19 — Pre-market Research

**Note: 2026-09-19 is a Saturday — cash equity markets are closed all day (confirmed via `date`).** This is a non-trading day; no buy-side gate runs today regardless of what this research surfaces. Entry produced per the standing pre-market schedule for completeness/monitoring.

### Account
- Equity: $93,431.20 | Cash: $18,777.57 (20.10%) | Buying power: $284,140.45 (margin-inflated by the standing 4x multiplier flag — no margin/leverage used or planned) | Daytrade count: not present in account payload (same pattern as prior sessions)
- Core exposure: $0.00 (0.00% — 0 of 6 slots open, unchanged since APA's 9/8 thesis-broken exit, 16 consecutive trading sessions as of Friday 9/18's close; today doesn't add to that count since markets are closed) | Satellite exposure: $5,316.96 (5.69%: RIGL, 114 sh, +11.47% unrealized) | Income exposure: $69,336.67 (74.21%: EDGX $19,992.80/+0.08%, SGOV $29,672.99/+0.12%, SPHY $19,670.88/-1.63%)
- Cash 20.10%, at/above the 20% floor — no sweep needed.
- All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing, no auth errors on account/positions/orders calls: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.5605 on 294 covered sh (hwm $100.59; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep). No stop moved down. Prices unchanged from Friday's close (change_today: 0 on every position) — expected on a non-trading day.
- This week (Sep 14 start) closed at core 0/6, satellite 0/4 new trades — valid per the Patience Rule.
- Biotech satellite 2-strike cooldown remains ACTIVE through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries regardless of screen results. Industrials sub-sector unaffected (0 strikes).

### Data-quality notes
**This session's Gemini Deep Research report is missing its dedicated Part I (S&P 500 futures direction / VIX level) section entirely — the captured output starts directly at "## Part II: Top Market-Moving Catalysts."** Per the data-quality guard, no futures/VIX figure below is asserted from a missing section; the one VIX figure used (15.51) was recovered incidentally from the report's own later "Strategic Outlook" paragraph, not fabricated. No explicit S&P futures % figure was recoverable anywhere in the document — that data point is reported as unavailable, not invented. Given today is a Saturday, a literal "futures move right now" reading is inherently limited anyway (equity index futures are closed over the weekend and reopen Sunday evening); the report's own text explicitly notes markets are closed today. Sections on catalysts, held-ticker news, and watchlist news (Parts II-V) all came through complete and well-sourced (44 citations), no truncation observed in those sections. Did not resubmit given the useful data recovered and this session's non-trading-day status.
- Not independently WebSearch-verified beyond what Gemini itself sourced — no second-opinion query run this session given the non-trading-day context and the absence of any live buy-side gate.

### Market Context
- **VIX ~15.51** (per Gemini's Strategic Outlook text — suppressed level, described as masking underlying stress). S&P 500 described as "near all-time highs." No explicit futures % move recovered (see Data-quality notes) — reasonable given today is a non-trading Saturday.
- **Major overnight/weekend geopolitical development: Houthi rebel attack on Saudi oil infrastructure, with a tanker reported on fire, has materially repriced energy risk.** Brent crude has decisively broken above $100/bbl (~$102+), and the 3:2:1 refining crack spread has surged toward ~$70/bbl — a level the report characterizes as historically rare, driven by both the supply-risk premium and structurally inelastic distillate demand (AI-datacenter diesel-generator deployment amid grid constraints, plus limited U.S. refining capacity). This is bullish/neutral for the portfolio (no held position is short energy or a refiner), and directly relevant to three core watchlist names — VLO, MPC, PSX — all refiners already flagged in recent weeks for strong FCF/momentum. Not independently cross-verified beyond Gemini's own citations (multiple financial-news sources cited); flagged as the session's most notable single item.
- 10-year Treasury yield continues grinding toward 5% — a structural headwind for high-multiple/long-duration tech (CRWD specifically flagged near its 52-week low on this dynamic) and a watch item for SPHY (held, income) if the 5% level is decisively breached (could widen high-yield credit spreads); not yet a break, SPHY's spreads remain tight per the report.
- Monday 9/21 calendar: Chicago Fed National Activity Index (7:30am), Fed speaker Goolsbee (10:30am), 3-/6-month T-bill auctions (10:30am). Later in the week: flash PMIs (Eurozone/UK/US), SNB and Riksbank rate decisions, continued BOJ-hike absorption, quarter-end window-dressing flows.
- Sector color: ZBRA showing standout relative strength (flat near $344-345 despite the broader oil-driven macro noise) on physical-AI/RFID retail-automation thesis, a raised 2026 price-realization outlook (+$90M), and a major University Health contract win; Zacks raised FY2026 EPS estimates +9%. NTAP near its 52-week high (enterprise AI-training data storage demand) vs. CRWD near its 52-week low (duration-driven multiple compression despite intact fundamentals) — a clean bifurcation example. TMO and CRL both trading near 52-week highs (bioprocessing/CRO recovery). No idiosyncratic overnight news for DELL, HPE, AMD, MRVL, INTC, PANW, HUM, CNC, INCY, STT, CPAY, NUE.

### Held-Position Thesis Check
- **RIGL (satellite, held, +11.47% unrealized):** Thesis intact, no adverse news. VEPPANU (vepdegestrant) commercial launch continues per plan — 8th consecutive profitable quarter, 76% gross margin TTM, $95M cash at Q2-end; R289 (oral IRAK1/4 inhibitor) Phase 1b dose-expansion on track for an H2 2026 Phase 2 dose selection. Still well below the +25% satellite tighten threshold and the -15% hard-cut.
- **EDGX (income, held, +0.08% unrealized):** No adverse news found; no thesis to break for Income sleeve regardless. Well inside the 5% stop.
- **SGOV (income, held, +0.12% unrealized):** Thesis intact/reinforced — short-term T-bill yields (3M/6M auctions near 4.00%) remain elevated with the Fed still restrictive. Well inside the 5% stop.
- **SPHY (income, held, -1.63% unrealized):** No new negative development beyond the standing distribution-decline/technical watch item (flagged 9/17); credit spreads remain tight for now per this session's report, but flagged again as sensitive if the 10-year yield cleanly breaches 5%. Remains a watch item, not a break; well inside the 5% stop.

### Core Trade Ideas (from current WATCHLIST.md core list)
None approved — this research-only workflow does not place trades regardless, and today is a non-trading Saturday with no buy-side gate window anyway. Of the current 24-name core list, today's report gives the strongest fresh color to **VLO, MPC, PSX** (historic refining crack-spread expansion off the Saudi-attack-driven oil spike; VLO specifically priced $850M in new senior notes to refinance higher-cost 2026 debt, a balance-sheet-optimization positive) and **ZBRA** (fresh contract win, raised price-realization outlook, analyst EPS estimate raise). **NTAP** flagged near its 52-week high; **CRWD** flagged near its 52-week low (duration risk, not a fundamentals concern) — worth watching, not actionable today. Core sleeve still needs a fresh qualified candidate — top standing priority, subject to a full live buy-side gate re-verification at the next trade window (Monday 9/21).

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
- **RIGL** (held) — see Held-Position Thesis Check, no new entry signal.
- **CGEM** — not held, cannot be newly entered regardless (biotech cooldown active through 2026-09-25). Note for the record: CGEM's catalyst has already resolved very favorably — Phase 3 REZILIENT3 interim data presented 9/13-14 at WCLC (zipalertinib + chemo vs. chemo alone in EGFR exon 20 NSCLC): 50% reduction in progression/death risk, PFS 14.5mo vs. 8.5mo, OS HR 0.72, 31.3% intracranial response rate. Next documented catalyst is Q3 earnings ~Nov 5, 2026 — not inside the 5-day window. Genuinely strong data, but the cooldown mechanically blocks new entry regardless of catalyst quality.
- **PLPC** — no catalyst inside the 5-day window (Q2 print already resolved). No entry-signal change.
- **SHIP** — **dividend ex-date 2026-09-25 remains at the 5-trading-day boundary from today** (Mon 9/21 - Fri 9/25 = 5 trading days), same item flagged 9/18; this session's Gemini report did not independently re-confirm the date but flagged general macro sensitivity (bunker-fuel costs rising with crude). Industrials unaffected by the biotech cooldown. Informational for a potential Monday buy-side-gate look, not an approved trade.
- **GNK** — no near-term catalyst (Q3 dividend declaration expected ~early Nov). No entry-signal change.
- **ALNT** — Q2 earnings catalyst already resolved, no new event inside the window. No entry-signal change.
- **FEIM** — Sept 10 earnings catalyst already resolved; stock has drifted ~13.4% higher since, trading ~$63.64, no new event inside the window. No entry-signal change.

### Risk Factors
- **Major overnight/weekend geopolitical event: Houthi attack on Saudi oil infrastructure (tanker fire), Brent >$102/bbl, refining crack spreads near record ~$70/bbl** — see Market Context. No held position threatened; bullish context for core watchlist refiners (VLO/MPC/PSX). Flagged as this session's most notable single item — see Notification below.
- Gemini's Part I (futures/VIX) section missing again this session (see Data-quality notes) — a recurring reliability pattern (truncation variants seen 9/11, 9/18, now a missing-section variant 9/19); worth aggregating for Friday's review alongside the standing pattern.
- 10-year Treasury yield grinding toward 5% remains a headwind for high-multiple tech and a watch item for SPHY's credit-spread exposure if breached — not yet a break.
- No held position is near its hard-cut: RIGL +11.47% (vs. -15% satellite cut), EDGX +0.08%, SGOV +0.12%, SPHY -1.63% (all vs. -5% income cut).
- Cash 20.10%, at/above the 20% floor — no sweep action needed.
- Core sleeve has now sat at 0% for 16 consecutive trading sessions since APA's 9/8 exit (unchanged today — non-trading day).
- SHIP's near-dated (5-trading-day-boundary) dividend ex-date (9/25) is a real, confirmed item for Monday's trade-window discussion, not a today-action item.
- SendGrid status: last confirmed broken (401 Unauthorized) as of 9/18's 3pm test (12th consecutive window / 9th session). Tested again this session per STEP 5 below given the notification triggered.

### Decision
**HOLD.** Today is a non-trading Saturday — no buy-side gate runs regardless of what this research surfaces, so no core or satellite trade is sourced or actionable. Core exposure remains at 0% (16 consecutive trading sessions since APA's 9/8 exit); rebuilding it from a fresh qualified candidate stays the top priority for Monday's pre-market/9:30am window, with VLO/MPC/PSX (refining) and ZBRA carrying the strongest fresh color from today's research, subject to a full live buy-side gate pass. Top items for Monday: (1) re-verify momentum/FCF/rating live for a core rebuild candidate, with the refiners and ZBRA as specific angles; (2) SHIP's 9/25 dividend ex-date sits at the 5-day catalyst-window boundary — worth a closer look at the Monday buy-side gate (industrials unaffected by the biotech cooldown); (3) core sleeve still needs a fresh qualified candidate — 16 sessions running; (4) continue monitoring the 10-year yield's approach to 5% for its SPHY credit-spread implications; (5) confirm SendGrid status via this session's actual send attempt (see Notification).

### Notification
**Triggered — major geopolitical event.** Per STEP 5, the Houthi attack on Saudi oil infrastructure (tanker fire), the resulting break above $100/bbl Brent, and the resulting historic refining crack-spread expansion (~$70/bbl) constitute a major geopolitical/macro development worth flagging even though no held position is directly threatened (see Market Context / Risk Factors). No held position is below its hard-cut and no satellite catalyst resolved negatively overnight, so this notification is sent on the geopolitical-event criterion alone. **SendGrid send attempted, still 401 Unauthorized (13th consecutive window / 10th session since first caught 9/10)** — `sendgrid.sh` only auto-falls-back on missing credentials, not an API-level 401, so the send aborted without auto-writing a fallback; content preserved by manually appending to DAILY-SUMMARY.md in the same fallback format used since 9/10. Owner action to rotate the key remains overdue and is escalated again this session.

## 2026-09-20 — Pre-market Research

**Note: 2026-09-20 is a Sunday — cash equity markets are closed all day (confirmed via `date`).** This is a non-trading day; no buy-side gate runs today regardless of what this research surfaces. Entry produced per the standing pre-market schedule for completeness/monitoring, consistent with 9/19's precedent.

### Account
- Equity: $93,431.20 | Cash: $18,777.57 (20.10%) | Buying power: $284,140.45 (margin-inflated by the standing 4x multiplier flag — no margin/leverage used or planned) | Daytrade count: not present in account payload (same pattern as prior sessions)
- Core exposure: $0.00 (0.00% — 0 of 6 slots open, unchanged since APA's 9/8 thesis-broken exit, 16 consecutive trading sessions as of Friday 9/18's close; today doesn't add to that count since markets are closed) | Satellite exposure: $5,316.96 (5.69%: RIGL, 114 sh, +11.47% unrealized) | Income exposure: $69,336.67 (74.21%: EDGX $19,992.80/+0.07%, SGOV $29,672.99/+0.12%, SPHY $19,670.88/-1.63%)
- Cash 20.10%, at/above the 20% floor — no sweep needed.
- All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing, no auth errors on account/positions/orders calls: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.5605 on 294 covered sh (hwm $100.59; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep). No stop moved down. Prices unchanged from Friday's close (change_today: 0 on every position) — expected on a non-trading day.
- This week (Sep 14 start) closed Friday at core 0/6, satellite 0/4 new trades — valid per the Patience Rule.
- Biotech satellite 2-strike cooldown remains ACTIVE through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries regardless of screen results. Industrials sub-sector unaffected (0 strikes).

### Data-quality notes
**This session's Gemini Deep Research report was truncated at the start — the captured output begins mid-document directly at the EDGX held-position deep-dive, with no preceding Part I (S&P 500 futures/VIX) or Part II (top market-moving catalysts/economic releases) sections anywhere in the document.** Per the data-quality guard, no futures/VIX figure or economic-calendar item below is asserted from a missing section. The captured output also stops after RIGL in the held-position deep-dive — **SGOV and SPHY did not get their own dedicated thesis-check paragraphs this session** (unlike prior weeks); their live prices/P&L are confirmed via `alpaca.sh positions` above (no gap, no anomaly), but no independent overnight-news check was returned for either from Gemini this session. Rather than a same-query resubmit (this workflow's time budget), filled the futures/VIX gap with a single native WebSearch query per CLAUDE.md guidance (see Market Context) and treated SGOV/SPHY as data-unavailable for overnight-news purposes, not as "no news."
- Sections covering held EDGX/RIGL detail, the full core-watchlist overnight sweep, and the satellite catalyst-proximity table all came through complete and well-sourced (60 citations), no truncation observed in those sections.
- SendGrid status: last confirmed broken (401 Unauthorized) as of 9/19's actual send attempt (13th consecutive window / 10th session since first caught 9/10) — not retested this session since no urgent condition triggers a send attempt today (see Notification below).

### Market Context
- **S&P 500 closed Friday 9/18 at 7651 (+0.17%), VIX ~14.81 at Friday's close (VIX futures ~17.95); equity futures pointing higher into Monday's open** — sourced via WebSearch fallback (see Data-quality notes; Gemini's report omitted this section entirely). Gemini's own Strategic Conclusion (recovered from the surviving portion of the report) separately cites a suppressed VIX reading of 15.51, in the same low/complacent ballpark as the WebSearch figure — not independently reconciled beyond noting both point to low near-term realized volatility.
- **Gemini's Strategic Conclusion flags a significant macro divergence for the week ahead:** the 10-year Treasury yield sits at 5.00% following continued hawkish Fed and Bank of Japan action, driving what it characterizes as "severe institutional outflows from high-yield corporate credit" — explicitly named as a "definitive thesis-breaker for risk-on fixed income instruments like SPHY," with capital rotating into short-duration government paper like SGOV. A corporate buyback blackout period into quarter-end is also flagged as a liquidity headwind, with Friday's Core PCE inflation print called out as the key event risk that could reprice equities against this fixed-income backdrop. This is Gemini's own framing/conclusion, not independently cross-verified — logged as a risk factor below, not acted on (Income sleeve has no thesis-break exit per TRADING-STRATEGY.md; 5% trailing stop and the buy-funding sweep are its only exit paths).
- **AI infrastructure buildout continues to dominate positive sector-level news:** AMD closed the week near $545.09 on gigawatt-scale Helios rack-scale platform commitments from Meta/OpenAI/Oracle, plus a RISC-V/SiFive ROCm port; MRVL closed at $244.25 on an expanded GlobalFoundries SiGe agreement and a Google deal with warrants tied to up to $120B in potential orders through FY2033; INTC +8.62% late-week on broad semiconductor-sector momentum. Cybersecurity also surged on analyst upgrades: CRWD +13.85% (Wedbush Outperform initiation), PANW +13.09% (added to Wedbush Best Ideas List).
- No explicit weekly economic-calendar detail (beyond the Friday Core PCE flag above) was recovered this session — treated as a data gap, not fabricated.

### Held-Position Thesis Check
- **EDGX (income, held, +0.07% unrealized):** Thesis intact. Declared a $0.0462/share dividend, ex-date 9/21, payable 9/24 (reinvests to SGOV per the Income sleeve rule). Cboe's proposed near-24x5 equities trading extension is a modest structural positive for liquidity. One nuance flagged: the currently-suppressed VIX mathematically caps the options-overlay premium EDGX can generate relative to high-volatility regimes — not a thesis break, just a lower-income-environment note.
- **RIGL (satellite, held, +11.47% unrealized):** Thesis intact, no adverse news. Weekend coverage was a review of already-known Cantor/Wainwright conference remarks (8 consecutive profitable quarters, $95M cash post the $70M VEPPANU upfront payment). R289 MDS dose-selection data still guided for H2 2026. Still well below the +25% satellite tighten threshold and the -15% hard-cut.
- **SGOV / SPHY (income, held):** No dedicated overnight-news paragraph returned this session (see Data-quality notes) — live prices/stops confirmed via `alpaca.sh`, no anomaly (SGOV +0.12%, SPHY -1.63%, both well inside their 5% stops). SPHY's standing distribution-decline/technical-breakdown watch item (flagged since 9/17) is echoed and sharpened by this session's Strategic Conclusion (see Market Context) — still a watch item under Income sleeve rules (no thesis-break exit exists), not a stop-triggering event today.

### Core Trade Ideas (from current WATCHLIST.md core list)
None approved — this research-only workflow does not place trades regardless, and today is a non-trading Sunday with no buy-side gate window anyway. Fresh positive color this session on **AMD, MRVL, INTC** (AI-infrastructure/semiconductor momentum), **CRWD, PANW** (cybersecurity analyst-upgrade surge), **STT** (executive reshuffle to accelerate the Alpha platform), **CPAY** (FTC settlement cleared, refocusing capital), **INCY** (minor Canadian reimbursement win for MINJUVI), **VEEV** (Vault AI rollout), **WST** (Q2 beat still supporting the thesis) — none of this is pre-approved; a full live momentum/FCF/rating reverification at the buy-side gate is required before any entry. No material overnight/weekend news for DELL, HPE, MPC, VLO, CRL, HUM, PSX, RVTY, IQV, ZBRA, CNC, TMO, NUE, NTAP (the one late-week NTAP hit is a CMS reimbursement-pathway acronym collision, not the company — flagged explicitly by Gemini as unrelated). Core sleeve still needs a fresh qualified candidate — top standing priority, subject to a full live buy-side gate re-verification at Monday's pre-market/9:30am window (17th consecutive trading session at 0% since APA's 9/8 exit, as of Monday).

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
- **RIGL** (held) — see Held-Position Thesis Check, no new entry signal.
- **CGEM** — not held, cannot be newly entered regardless (biotech cooldown active through 2026-09-25). Confirmed: its Phase 3 REZILIENT3 catalyst already resolved positively on 9/13 (PFS 14.5mo vs 8.5mo, OS HR 0.72), prompting Stifel/UBS price-target upgrades — genuinely strong data, but the cooldown mechanically blocks new entry regardless. No entry-signal change.
- **PLPC, GNK** — Gemini confirms no identifiable corporate event or catalyst scheduled for the week of 9/21-9/25 for either. No entry-signal change.
- **SHIP** — **confirmed within the 5-trading-day catalyst window: dividend ex-date 2026-09-25 lands exactly on the 5th trading day from today** (Mon 9/21 through Fri 9/25). Industrials unaffected by the biotech cooldown. Informational for a potential Monday buy-side-gate look, not an approved trade — any entry would still need a full live pass (momentum, YoY growth, rating) plus sizing/documentation per the satellite gap-risk rule given the near-dated (non-binary) catalyst.
- **ALNT** — Q3 earnings catalyst confirmed for Nov 4, 2026, well outside the window. No entry-signal change.
- **FEIM** — Q1 FY27 earnings catalyst (9/10) already resolved; no new event inside the window. No entry-signal change.

### Risk Factors
- **Gemini's own Strategic Conclusion explicitly frames current high-yield-credit outflows as a "definitive thesis-breaker" for SPHY specifically** (see Market Context) — a stronger characterization than prior weeks' "watch item" framing. Per TRADING-STRATEGY.md, Income sleeve has no thesis-break exit mechanism regardless (5% trailing stop and the buy-funding sweep are its only exit paths), so no action is available or required beyond continued monitoring of the existing stop; flagged here for visibility given the escalation in Gemini's own language.
- 10-year Treasury yield at 5.00%, hawkish Fed/BOJ backdrop, and an approaching corporate buyback blackout into quarter-end are flagged as broad headwinds; Friday's Core PCE print is called out as the week's key event risk.
- Gemini truncation this session lost both the futures/VIX section and the SGOV/SPHY held-position deep-dives (see Data-quality notes) — a continuation of the recurring reliability pattern seen 9/11, 9/18, 9/19; worth aggregating for Friday's review.
- No held position is near its hard-cut: RIGL +11.47% (vs. -15% satellite cut), EDGX +0.07%, SGOV +0.12%, SPHY -1.63% (all vs. -5% income cut).
- Cash 20.10%, at/above the 20% floor — no sweep action needed.
- Core sleeve will reach 17 consecutive trading sessions at 0% exposure as of Monday's open (since APA's 9/8 exit).
- SHIP's confirmed 5-day-window dividend ex-date (9/25) is a real item for Monday's trade-window discussion, not a today-action item.
- SendGrid status: still broken as of 9/19's last actual send attempt (401 Unauthorized, 13th consecutive window / 10th session since first caught 9/10) — not retested this session since no urgent condition triggers a send attempt today. **This is now a 10-day-plus outage of the account's only owner-facing alert channel; owner action to rotate the key remains overdue.**

### Decision
**HOLD.** Today is a non-trading Sunday — no buy-side gate runs regardless of what this research surfaces, so no core or satellite trade is sourced or actionable. Core exposure remains at 0% (16 consecutive trading sessions since APA's 9/8 exit as of Friday's close); rebuilding it from a fresh qualified candidate stays the top priority for Monday's pre-market/9:30am window, with the AI-infrastructure/semiconductor cluster (AMD, MRVL, INTC) and cybersecurity (CRWD, PANW) carrying the strongest fresh positive color from today's research, subject to a full live buy-side gate pass. Top items for Monday: (1) re-verify momentum/FCF/rating live for a core rebuild candidate; (2) SHIP's 9/25 dividend ex-date is now confirmed inside the 5-day catalyst window — worth a closer look at Monday's buy-side gate (industrials unaffected by the biotech cooldown); (3) core sleeve still needs a fresh qualified candidate — 16 sessions running as of Friday; (4) monitor SPHY given Gemini's sharpened "thesis-breaker" language on high-yield credit outflows, though no action is available under Income sleeve rules; (5) confirm SendGrid status next time a notification is actually due — outage is now 10+ days old.

### Notification
**Silent — no urgent condition met.** Per STEP 5, notification is silent unless a held position is already below its hard-cut pre-market, a satellite catalyst resolved negatively overnight, or a major geopolitical event occurred. None applies: all 4 positions are well clear of their stops (SPHY's sharpened thesis-strain language is a watch item under Income rules, not a hard-cut breach or an actionable break), no satellite catalyst resolved negatively (CGEM resolved positively 9/13; SHIP's ex-date is upcoming, not resolved), and no new geopolitical shock was reported overnight beyond the standing post-FOMC/high-yield-credit macro backdrop already logged. No email sent this session.

## 2026-09-21 — Pre-market Research

Monday, a live trading day (confirmed via `date`).

### Account
- Equity: $93,359.72 | Cash: $18,777.57 (20.11%) | Buying power: $283,940.29 (margin-inflated by the standing 4x multiplier — no margin/leverage used or planned). `account`/`positions`/`orders` all returned cleanly, no auth errors.
- Core exposure: $0.00 (0.00% — 0 of 6 slots open, unchanged since APA's 9/8 thesis-broken exit; this is the 17th consecutive trading session at 0% as of today's open) | Satellite exposure: $5,244.00 (5.62%: RIGL, 114 sh, +9.94% unrealized) | Income exposure: $69,338.15 (74.27%: EDGX $19,992.80/+0.08%, SGOV $29,674.47/+0.12%, SPHY $19,670.88/-1.63%)
- Cash 20.11%, at/above the 20% floor — no sweep needed.
- All 4 open positions (RIGL, EDGX, SGOV, SPHY) confirmed via `alpaca.sh orders` carrying live GTC trailing stops, none missing: RIGL 15%/$42.415 (hwm $49.90), EDGX 5%/$25.9825 (hwm $27.35), SPHY 5%/$22.23 (hwm $23.40), SGOV 5%/$95.5605 on 294 covered sh (hwm $100.59; 0.989473856-sh fractional remainder stays unstopped, same immaterial ~$99 dust flagged every prior sweep). No stop moved down.
- Last week (Sep 14 start) closed at core 0/6, satellite 0/4 new trades — valid per the Patience Rule. New week starts today at core 0/6, satellite 0/4.
- Biotech satellite 2-strike cooldown remains ACTIVE through 2026-09-25 (OCUL 7/23, MNKD 9/11) — no new satellite biotech entries this week regardless of screen results; lifts Friday. Industrials sub-sector unaffected (0 strikes).

### Data-quality notes
**This session's first consolidated Gemini Deep Research call came back severely truncated — the captured output contained only Section 5 (satellite catalyst-date verification), with Sections 1-4 (futures/VIX, market-moving catalysts, held-ticker overnight news, core+satellite watchlist overnight news) entirely missing.** Per the data-quality guard, nothing was taken from the missing sections. Given today is a live trading Monday (unlike the weekend runs on 9/19-9/20), a same-query resubmit was run rather than falling back to a single WebSearch query — it came back complete and well-sourced (63 citations) covering the core/satellite watchlist and a macro synthesis, though it still did not return a dedicated per-ticker held-position thesis-check paragraph for EDGX/RIGL/SGOV/SPHY the way prior sessions' reports have (only a summary line in its Strategic Outlook — see Held-Position Thesis Check below) and it did not return a dedicated daily economic-calendar list. The economic-calendar items carried below are sourced from Saturday's (9/19) research, which already specifically flagged today's (9/21) calendar and remains the most recent available data point for it.
- **FEIM data-quality flag — resolved via independent verification, not taken at face value.** The resubmit's FEIM paragraph asserted a "severe Q2 earnings miss" (loss of $0.50/share vs. an expected $0.26 profit, revenue miss $15.4M vs. $18.57M consensus) and an approximate 15% stock decline, in the same paragraph as a "record $111 million funded backlog" — internally inconsistent, and flatly contradicts WATCHLIST.md's already-verified FEIM entry (Q1 FY27 beat reported 9/10: revenue $23.5M/+70% YoY, EPS $0.41 vs. ~$0.15 consensus, backlog $129M/+82% YoY, 1.76x book-to-bill). Ran an independent WebSearch to adjudicate rather than log either figure on faith: confirmed FEIM's actual 9/10 Q1 FY27 print was the clean beat already on WATCHLIST.md (record $23.5M revenue +70% YoY, $0.41 EPS, $129M backlog +82% YoY/1.76x book-to-bill, per stockanalysis.com/Yahoo Finance/GuruFocus/Motley Fool earnings-call coverage) — no Q2 report exists yet, no miss, no ~15% decline. This is a fabricated/hallucinated data point from this session's Gemini report, not a real overnight development; treated as noise, not logged as a thesis change. Consistent with TRADING-STRATEGY.md's standing "Pending-catalyst guard" caution about not trusting Gemini's framing at face value. Worth aggregating for Friday's review alongside the recurring truncation pattern.

### Market Context
- **S&P 500 futures +0.41%, described as "structurally sound but strictly bounded."** VIX resistance flagged at 15.18 and 10-year Treasury yield resistance at 4.969% — the report frames a breach of either as the trigger for risk-off rotation; absent that, the AI-led rally has room to run. 10-year yield is otherwise described as grinding toward/at the 5.00% level (hawkish Fed/BOJ backdrop, carried from 9/20's research).
- **Day's key scheduled event: GCC-Iran diplomatic talks in Oman regarding Strait of Hormuz shipping-lane tensions** — a potential de-escalation signal following the weekend's Houthi-attack-driven oil spike (Brent >$100/bbl, historic refining crack spreads), not a new negative shock.
- **Economic calendar (carried from 9/19's research, most recent available for today):** Chicago Fed National Activity Index (7:30am ET), Fed speaker Goolsbee (10:30am ET), 3-/6-month T-bill auctions (10:30am ET). Later in the week: flash PMIs, SNB/Riksbank rate decisions, quarter-end window-dressing flows.
- **AI infrastructure buildout dominates positive sector news:** DELL posted a blowout Q2 (revenue $47.0B/+58% YoY, adj. EPS $7.04 vs. $4.87 consensus; AI server revenue +757% to $16.1B; record $51.3B AI backlog; FY guide raised to $192B from $167B) — stock +10%+ after-hours to a fresh 52-week high $588.40, analyst PTs raised to $460-$735 range, 0% bearish among 19 analysts. HPE rallying 6-12% in sympathy (Barclays PT raised to $27). MRVL +3% premarket on a Google custom-silicon agreement (warrants tied to up to $120B in potential orders through FY2033) and an S&P debt upgrade to BBB. INTC downgraded to Sell by at least one analyst overnight on Nvidia's RTX Spark superchip threatening Intel's client-compute moat. AMD +2.7% on broad SMH strength. PANW +3% on a beat-and-raise FQ4 (EPS $1.02/rev $3.41B, FY27 guide $14.1-14.2B above estimates). CRWD +8.3% on AI-conviction spillover from Snowflake's earnings beat.
- **Refining sector (MPC/VLO/PSX) continues its historic crack-spread windfall** flagged over the weekend — MPC Q2 net income $5.1B ($36.33/bbl margin, doubled YoY), VLO $3.7B net income, PSX EPS $9.41 vs. $7.50 consensus; YTD gains >150% for MPC/VLO.
- RVTY +9% on KeyBanc PT hike to $165 (AI-driven screening demand) plus a pending Human Cell Design (GLP-1 research) acquisition. IQV +60% over 6 months, FY26 EPS guide raised to $12.80-$13.00, 294 AI agents deployed across 90 client use cases. ZBRA getting analyst PT hikes (Needham $435, Stephens $450) on a raised revenue/EPS outlook. No material overnight news for CRL, HUM, CNC, TMO, NUE, INCY, WST, STT, CPAY, VEEV, NTAP.

### Held-Position Thesis Check
- **RIGL (satellite, held, +9.94% unrealized):** No adverse news surfaced this session (satellite catalyst-verification section confirms no near-term catalyst — Q3 earnings estimated Nov 3-6, R289 data still guided "end of 2026"). Thesis intact, well below the +25% satellite tighten threshold and the -15% hard-cut.
- **EDGX / SGOV / SPHY (income, held):** No dedicated per-ticker overnight-news paragraph returned this session (see Data-quality notes) — live prices/stops confirmed via `alpaca.sh`, no anomaly, no gap. This session's Strategic Outlook does flag the sleeve generally as "insulated from acute overnight shocks," but singles out **SPHY's diminishing margin of safety** (OAS compression to 267bps, declining monthly distributions) as making high-yield credit "structurally less attractive" versus SGOV's short-duration safety — a continuation/sharpening of the same watch item flagged 9/17 through 9/20, not a new development. Not a thesis break under Income sleeve rules (no thesis-break exit exists); well inside its 5% stop.

### Core Trade Ideas (from current WATCHLIST.md core list)
None approved — this pre-market research workflow does not run a live buy-side gate; any entry requires full live momentum/FCF/rating re-verification at a trade window. Strongest fresh positive color this session: **DELL** (blowout Q2, record $51.3B AI backlog, FY guide raised, fresh 52-week high) and **HPE** (sympathy rally, Barclays PT raise) lead the AI-infrastructure cluster; **MRVL** (Google custom-silicon deal, S&P debt upgrade) and **PANW**/**CRWD** (beat-and-raise, AI-conviction spillover) also stand out. **RVTY, IQV, ZBRA** carry fresh analyst-upgrade momentum. **INTC** flagged with a fresh Sell-side downgrade (Nvidia RTX Spark threat to its client-compute moat) — a caution item, not currently held, would need to clear the momentum/FCF/rating gate fresh regardless. **MPC/VLO/PSX** continue the multi-week refining crack-spread thesis already flagged. Core sleeve remains at 0% for a 17th consecutive session — rebuilding it from a fresh qualified candidate stays the top priority for a live trade window, subject to a full buy-side gate pass (this workflow does not execute trades).

### Satellite Trade Ideas (from current WATCHLIST.md satellite list)
- **RIGL** (held) — see Held-Position Thesis Check, no new entry signal.
- **CGEM** — not held, cannot be newly entered regardless (biotech cooldown active through 2026-09-25, lifts Friday). No documented near-term catalyst per this session's satellite verification table. No entry-signal change.
- **SHIP — confirmed the only satellite ticker with a catalyst inside the 5-trading-day window: ex-dividend date Friday 2026-09-25, $0.35/share (~9.71% annualized yield, ~43.21% payout ratio).** Q2 EPS $1.32 beat estimates by 23.5% (+633% YoY), revenue $55.69M (+48.5% YoY), Strong Buy consensus with a $25 PT. Industrials unaffected by the biotech cooldown. Informational for a potential live trade-window look, not an approved trade — any entry would still need a full live momentum/growth/rating pass plus sizing/documentation per the satellite gap-risk rule given the near-dated (non-binary) catalyst.
- **GNK** — Q2 dividend ($0.80/share, +433% YoY) already paid/ex-dated; Q3 distribution guided >$1.00/share but not expected until ~early Nov. No catalyst inside the window.
- **PLPC** — no identifiable catalyst inside the window (next ex-div Oct 1, 10 calendar days out — fails the 5-trading-day proximity check).
- **ALNT** — no catalyst inside the window (Q3 earnings estimated Nov 4).
- **FEIM** — no catalyst inside the window (Q1 FY27 catalyst already resolved 9/10). See Data-quality notes: this session's claim of a fresh Q2 miss/price decline is confirmed false via independent WebSearch — not a real development, no entry-signal change from WATCHLIST.md's standing (positive) view.

### Risk Factors
- 10-year Treasury yield grinding toward/at 5.00% remains a headwind for high-multiple tech and a credit-spread risk for SPHY specifically (sharpened language again this session) — no action available under Income sleeve rules, watch item only.
- VIX/yield levels flagged as the pivot for risk-on continuation (VIX <15.18, 10yr <4.969%) versus a risk-off rotation if breached — worth monitoring intraday.
- No held position is near its hard-cut: RIGL +9.94% (vs. -15% satellite cut), EDGX +0.08%, SGOV +0.12%, SPHY -1.63% (all vs. -5% income cut).
- Cash 20.11%, at/above the 20% floor — no sweep action needed.
- Core sleeve reaches 17 consecutive trading sessions at 0% exposure today (since APA's 9/8 exit) — standing discussion item.
- SHIP's confirmed 5-day-window dividend ex-date (9/25) is a real item for a live trade-window look this week, not a today-only action item.
- First Gemini call this session was severely truncated (Sections 1-4 entirely missing) and required a resubmit; the resubmit's FEIM paragraph contained a fabricated/internally-inconsistent earnings-miss claim caught and disproven via independent WebSearch before logging. Both are worth aggregating for Friday's review alongside the recurring truncation pattern (9/11, 9/18, 9/19, 9/20, now 9/21).
- SendGrid status: last confirmed broken (401 Unauthorized) as of 9/18's 3pm test — not retested this session since no urgent condition triggers a send attempt today (see Notification below); this is now a 13+ day outage of the account's only owner-facing alert channel, owner action to rotate the key remains overdue.

### Decision
**HOLD.** This pre-market research workflow does not execute trades regardless of what it surfaces. Core exposure remains at 0% (17 consecutive trading sessions since APA's 9/8 exit) — rebuilding it from a fresh qualified candidate stays the top priority for the next live trade window, with DELL/HPE (AI-infrastructure momentum) and MRVL/PANW/CRWD (custom-silicon/beat-and-raise/AI-conviction) carrying the strongest fresh color, subject to a full live buy-side gate pass. Top items for the next trade window: (1) re-verify momentum/FCF/rating live for a core rebuild candidate; (2) SHIP's confirmed 5-day-window dividend ex-date (9/25) is worth a closer look (industrials unaffected by the biotech cooldown, which itself lifts Friday); (3) continue monitoring SPHY given the sharpened high-yield-credit-outflow language, though no action is available under Income sleeve rules; (4) confirm SendGrid status next time a notification is actually due — outage is now 13+ days old; (5) log the FEIM Gemini-hallucination incident and the Sections-1-4 truncation for Friday's weekly-review reliability discussion.

### Notification
**Silent — no urgent condition met.** Per STEP 5, notification is silent unless a held position is already below its hard-cut pre-market, a satellite catalyst resolved negatively overnight, or a major geopolitical event occurred. None applies: all 4 positions are well clear of their stops (RIGL +9.94%, Income names all within their 5% stops; SPHY's sharpened thesis-strain language is a watch item under Income rules, not a hard-cut breach), no satellite catalyst resolved negatively (SHIP's ex-date is upcoming, not resolved; CGEM has no near-term catalyst), and the day's most notable scheduled event (GCC-Iran diplomatic talks on Strait of Hormuz shipping lanes) is a potential de-escalation signal, not a new shock. No email sent this session.

### 9:30 AM Session Note
- **HOLD both sleeves.** Live-rechecked momentum on CRWD (rank 4, above both 50d/200d MA, 3M rel. +36.8%, 6M rel. +106.6% vs SPY per fresh Alpaca bars) and VLO (rank 8, above both MAs, 3M rel. +72.9%, 6M rel. +55.3%, no corporate-action artifact) — both cleared momentum/FCF/rating cleanly, but neither cleared the live spread check.
- **Recurring wide/erratic opening-spread data-feed anomaly reproduced broadly this window.** Polled DELL, HPE, MRVL, PANW, CRWD, MPC, VLO, PSX, RVTY, IQV, ZBRA, TMO, WST (core) across 2-5 rounds each (~0-90s apart): every name showed either a sustained wide spread (5-12%) or genuine flicker between tight (~0.5%) and wide (3-5.6%) on consecutive polls with no stable settle — CRWD and VLO specifically looked clean on 2-3 consecutive polls before re-widening on the next check, so neither was treated as confirmed stable. Satellite: SHIP's quote stayed frozen at bid $16.04/ask $21.28 (~24.6% spread, unchanged timestamp-to-timestamp for several minutes) — same stale/illiquid pattern as the 9/2 EDGX incident, not transacted against; CGEM also wide (~24.9%) and moot regardless (biotech cooldown active through 9/25). Per STEP 2's skip-wide-or-zero-spread rule, no buy was placed on any name.
- No account activity this window (no buys, sells, sweeps, tightens). Core stays 0/6 positions, 0/6 trades this week; satellite stays 1/4 (RIGL only), 0/4 trades this week — valid per the Patience Rule. Cash unchanged at ~20.08-20.11% of equity, at/above the floor.
- **Pattern worth flagging for Friday's review:** this is at least the 4th-5th documented instance (8/24-8/28, 8/25 DELL abort, and now 9/21) where the spread anomaly resolves as a *flicker* between tight and wide on the same name across close-spaced polls rather than a simple sustained-wide read — worth deciding whether the workflow should require more polls (e.g. 5 over 2+ minutes) before treating any name as confirmed tradeable, given CRWD/VLO both looked clean 2-3 times running before re-widening today.

### 11 AM Session Note
- **HOLD both sleeves — no action taken.** No "Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market logged none; 9:30 AM's live spread check failed on every polled core/satellite name) — per STEP 3 that's treated as "Approved: none," so no buys placed this window; this window did not re-screen or re-verify candidates from scratch (out of scope for the 11am window).
- Current state confirmed via fresh `alpaca.sh account`/`positions`/`orders`: equity $93,550.00, cash $18,777.57 (20.07%, at/above floor). Core 0/6 positions, 0/6 trades this week. Satellite 1/4 (RIGL, +11.66% unrealized, plpc well above the -15% cut and below the +25% tighten threshold — no cut, no tighten). Income: EDGX +0.37%, SGOV +0.13%, SPHY -1.39% — all well inside their 5% stops, no cut. All 4 GTC trailing stops confirmed live and unchanged (RIGL 15%/$42.415, EDGX 5%/$25.9825, SPHY 5%/$22.23, SGOV 5%/$95.57) — none moved down.
- Thesis check: RIGL intact (no adverse news, catalyst already resolved 8/13); EDGX/SGOV/SPHY have no thesis-break exit under Income sleeve rules, SPHY's sharpened high-yield-credit-outflow language remains a watch item only. No day-P&L anomaly on any held name warranting a Gemini sharp-move query. 2-strike biotech cooldown unchanged (active through 2026-09-25); industrials 0/2.
- No memory files besides this note changed this window — nothing to persist to main.

### Notification
Silent — no buys, sells, cuts, or tightens this window (pure HOLD); per STEP 9, notification is sent only if action was taken.

### 3 PM Session Note
- **HOLD both sleeves — no action taken.** No "Approved Trades (verified)" section exists anywhere in today's RESEARCH-LOG (pre-market logged none; 9:30 AM's live spread check failed on every polled name; 11 AM did not re-screen) — per STEP 3 that's "Approved: none," so no buys placed this window either. This week (Sep 21 start, new week) closes today's window at core 0/6, satellite 0/4 new trades.
- Current state confirmed via fresh `alpaca.sh account`/`positions`/`orders`: equity $93,625.35, cash $18,777.57 (20.06%, at/above the 20% floor, no sweep needed). Core 0/6 positions (18th consecutive session at 0% since APA's 9/8 exit). Satellite 1/4 (RIGL, +11.93% unrealized — well clear of the -15% hard-cut and still below the +25% tighten threshold, no cut/tighten). Income: EDGX +0.71%, SGOV +0.13%, SPHY -1.41% — all well inside their 5% stops, no cut. All 4 open positions carry live GTC trailing stops confirmed via `orders` (RIGL 15%/$42.415, EDGX 5%/$25.9825, SPHY 5%/$22.23, SGOV 5%/$95.57 on 294 covered sh — 0.989473856-sh fractional remainder stays unstopped, same immaterial dust flagged every prior sweep), none missing, none moved down.
- Risk sweep: no position at/beyond its hard-cut; no unprotected position; RIGL's catalyst (VEPPANU launch) resolved 8/13 with no new near-term binary event inside 1-2 trading days, so the gap-risk close-or-hold check doesn't apply. Thesis check: RIGL intact (no adverse news since 11am); EDGX/SGOV/SPHY have no thesis-break exit under Income sleeve rules, SPHY's high-yield-credit-outflow watch item persists but is not a break, well inside its 5% stop. 2-strike biotech cooldown unchanged (active through 2026-09-25, lifts Friday); industrials 0/2. **Risky positions closed today: none.**
- No stop tightened this window (RIGL below the +25% satellite threshold; Income has no gain-based schedule). Day P&L +$325.16 (+0.35%) vs Friday 9/18's $93,300.19 close. Phase P&L -6.37% since Day 0 ($100,000 -> $93,625.35). Core exposure remains the top standing priority (18 consecutive sessions at 0%); tomorrow: re-verify momentum/FCF/rating live at the buy-side gate for a core rebuild candidate (DELL/HPE/MRVL/PANW/CRWD carry the freshest color per this morning's research), and take a closer live look at SHIP ahead of its 9/25 dividend ex-date.
